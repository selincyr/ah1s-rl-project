...
RESULT_DIR = Path(
    "results_stage4_low_alt_physical_collective_id_v1"
)
...
# =====================================================================
# STAGE-4 LOW-ALTITUDE PHYSICAL COLLECTIVE IDENTIFICATION V1
# =====================================================================
...
PHYS_SUSTAINED_SECONDS = 20.0

PHYS_DIAG_MIN_AGL_FT = 12.0
PHYS_DIAG_MAX_DOWN_VS_FPS = -0.65
...
rule("B — NATIVE ACTION0=-1 LOW-ALTITUDE EQUILIBRIUM")
trigger_run = build_native_min_equilibrium()
trig = trigger_run["trigger_state"]
baseline_collective = trigger_run["baseline_physical_collective"]
...
print(
    ...
    f"A0=-1.000000 | "
    f"COLLbaseline={baseline_collective:+.6f}"
)
close_handoff(trigger_run)


rule("C — PHYSICAL COLLECTIVE AUTHORITY PULSES BELOW NATIVE A0=-1")
pulse_rows = []
pulse_trace_rows = []

for case_idx, delta_phys in enumerate(PHYSICAL_COLLECTIVE_DELTAS, start=1):
    ...
    baseline = run["baseline_physical_collective"]

    initial = snapshot(fdm, lat0, lon0, mission_heading)
    requested = float(baseline + delta_phys)

    safe = True
    term = "pulse_complete"
    physical_values = []

    for step in range(int(PHYS_PULSE_SECONDS / dt)):
        state_before = snapshot(fdm, lat0, lon0, mission_heading)

        # Recompute horizontal actions normally; force only collective physically.
        action, ctrl, vertical_bias = build_near_ground_action_low_alt_bias(
            state_before,
            target_alt_ft=LOW_ALT_TARGET_FT,
            hover_action0=hover_action0,
        ...
        state, used, mapped_elev, requested_collective = raw_physical_collective_cycle(
            env2,
            fdm,
            action,
        ...
        t = (step + 1) * dt
        physical_values.append(state["physical_collective_cmd"])

        pulse_trace_rows.append({
            "case": int(case_idx),
            "physical_collective_delta": float(delta_phys),
            "time_s": float(t),
            "baseline_physical_collective": float(baseline),
            "requested_physical_collective": float(requested_collective),
        ...
        reason = physical_diag_safety_reason(state)
        if reason:
            safe = False
            term = reason
            break
    ...
    row = {
        ...
        "baseline_physical_collective": float(baseline),
        "requested_physical_collective": float(requested),
        "actual_physical_collective_mean": float(
            np.mean(physical_values)
        ) if physical_values else float("nan"),
        "delta_vertical_speed_fps": float(
            final["vertical_speed_fps"] - initial["vertical_speed_fps"]
        ),
        "delta_altitude_ft": float(
            final["altitude_ft"] - initial["altitude_ft"]
        ),
        "delta_forward_speed_fps": float(
            final["forward_speed_fps"] - initial["forward_speed_fps"]
        ),
        "delta_cross_track_ft": float(
            final["cross_track_ft"] - initial["cross_track_ft"]
        ),
    ...
    print(
        f"Case {case_idx:02d} dCOLL={delta_phys:+.5f} | "
        f"COLL={row['actual_physical_collective_mean']:+.6f} | "
        f"SAFE={safe} TERM={term} | "
        f"dVS@2s={row['delta_vertical_speed_fps']:+.4f} "
        f"dALT={row['delta_altitude_ft']:+.3f} | "
        f"dVfwd={row['delta_forward_speed_fps']:+.4f} "
        f"dX={row['delta_cross_track_ft']:+.3f} | "
...
if len(safe_pulses) >= 3:
    x = np.asarray(
        [r["physical_collective_delta"] for r in safe_pulses],
        dtype=float,
    )
    y = np.asarray(
        [r["delta_vertical_speed_fps"] for r in safe_pulses],
        dtype=float,
    )
    ...
    print()
    print(
        f"FIT dVS(2s) ~= {fit_slope:+.6f} * physical_collective_delta "
        f"{fit_intercept:+.6f}   (R^2={fit_r2:.4f})"
    )


rule("D — SUSTAINED PHYSICAL COLLECTIVE CHECKS")
sustained_rows = []
sustained_trace_rows = []

for case_idx, delta_phys in enumerate(
    SUSTAINED_PHYSICAL_COLLECTIVE_DELTAS,
    start=1,
):
    run = build_native_min_equilibrium()
    ...
    for step in range(int(PHYS_SUSTAINED_SECONDS / dt)):
        state_before = snapshot(fdm, lat0, lon0, mission_heading)
        ...
        if t >= PHYS_SUSTAINED_SECONDS - 2.0:
...
rule("E — LOW-ALT PHYSICAL COLLECTIVE ID CONCLUSION")
useful = bool(
    any(
        r["safe"]
        and r["physical_collective_delta"] < 0.0
        and r["delta_vertical_speed_fps"] <= -0.04
        for r in pulse_rows
    )
)
...
safe_sustained = [
    ...
    if r["safe"]
    and r["corridor"]
    and r["final_altitude_ft"] >= PHYS_DIAG_MIN_AGL_FT
    and np.isfinite(r["vs_mean_last2s"])
    and r["vs_mean_last2s"] < -0.04
]
...
if safe_sustained:
    preferred = min(
        safe_sustained,
        key=lambda r: (
            abs(r["vs_mean_last2s"] + 0.20),
            abs(r["final_position_error_ft"]),
            abs(r["final_cross_track_ft"]),
            abs(r["physical_collective_delta"]),
        ),
    )
...
print(f"additional lower physical collective authority useful: {useful}")
print(f"pulse-response R^2: {fit_r2:.4f}")
print(f"safe sustained candidate inside corridor: {preferred is not None}")

if preferred is not None:
    print(
        "preferred sustained evidence case: "
        f"dCOLL={preferred['physical_collective_delta']:+.5f}, "
        f"COLL={preferred['actual_physical_collective_mean']:+.6f}, "
...
print()
print(
    "READY FOR STAGE-4 LOW-ALT PHYSICAL COLLECTIVE MAPPING "
    f"CALIBRATION: {ready}"
)

save_rows_csv(
    RESULT_DIR / "physical_collective_pulse_sweep.csv",
    pulse_rows,
)
save_rows_csv(
    RESULT_DIR / "physical_collective_pulse_traces.csv",
    pulse_trace_rows,
)
save_rows_csv(
    RESULT_DIR / "physical_collective_sustained_sweep.csv",
    sustained_rows,
)
save_rows_csv(
    RESULT_DIR / "physical_collective_sustained_traces.csv",
    sustained_trace_rows,
)
...
summary = {
    ...
    "baseline_native_action0": -1.0,
    "physical_collective_deltas": PHYSICAL_COLLECTIVE_DELTAS,
    "pulse_seconds": PHYS_PULSE_SECONDS,
    "sustained_physical_collective_deltas": (
        SUSTAINED_PHYSICAL_COLLECTIVE_DELTAS
    ),
    "sustained_seconds": PHYS_SUSTAINED_SECONDS,
    "fit_slope": float(fit_slope),
    "fit_intercept": float(fit_intercept),
    "fit_r2": float(fit_r2),
    "additional_lower_physical_collective_authority_useful": bool(useful),
    "preferred_sustained_case": preferred,
    "ready_for_stage4_low_alt_physical_collective_mapping_calibration": bool(
        ready
    ),
...
print("Saved:")
print(f"  {RESULT_DIR / 'physical_collective_pulse_sweep.csv'}")
print(f"  {RESULT_DIR / 'physical_collective_pulse_traces.csv'}")
print(f"  {RESULT_DIR / 'physical_collective_sustained_sweep.csv'}")
print(f"  {RESULT_DIR / 'physical_collective_sustained_traces.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. This is identification only. "
    "No final low-alt collective mapping is created until the measured "
    "physical authority is shown safe and useful."
)
