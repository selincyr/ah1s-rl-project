...
RESULT_DIR = Path(
    "results_stage4_low_alt_bias_v1"
)
...
print("Stage-4 low-altitude bias V1: test the diagnosed fade-to-zero failure without changing horizontal control or actuator authority")
...
rule("B — LOW-ALTITUDE COLLECTIVE FEEDFORWARD FLOOR SWEEP")
print("Previous PI V1 reached a repeatable equilibrium at ~35.6 ft because the adaptive bias was blended to ZERO at 30 ft.")
print("This experiment changes only that low-altitude endpoint of the blend.")
print(f"VS_Ki fixed at {LOCKED_VS_KI:.4f}; Vmax={LOCKED_DESCENT_VMAX:.2f}; VS_Kp={LOCKED_VS_KP:.2f}.")
print(f"Bias state remains bounded [{VERTICAL_BIAS_MIN:+.2f},{VERTICAL_BIAS_MAX:+.2f}].")
print(f"Low-altitude floor candidates: {LOW_ALT_BIAS_FLOOR_CANDIDATES}")
print(f"Qualified collective residual envelope remains [{COLLECTIVE_RESIDUAL_MIN:+.2f},{COLLECTIVE_RESIDUAL_MAX:+.2f}].")
print(f"Horizontal fixed: LONG_Kpos={LOCKED_LONG_KPOS_PHYS:.4f}, LONG_Kv={LOCKED_LONG_KV_PHYS:.3f}; lateral Kp={LOCKED_LATERAL_KP:.2f}, Kd={LOCKED_LATERAL_KD:.2f}.")

candidate_rows=[]
all_candidate_traces=[]
for idx, floor in enumerate(LOW_ALT_BIAS_FLOOR_CANDIDATES, start=1):
    print(f"Candidate {idx:2d}/{len(LOW_ALT_BIAS_FLOOR_CANDIDATES)} | low_alt_bias_floor={floor:+.3f}")
    result, trace = run_teacher_case(low_alt_bias_floor=floor, candidate_index=idx, detailed=False)
    ...
    print(
        ...
        f"VBias=[{result.get('vertical_bias_min_seen',0):+.3f},{result.get('vertical_bias_max_seen',0):+.3f}] | "
        f"dA0=[{result.get('collective_residual_min_seen',0):+.3f},{result.get('collective_residual_max_seen',0):+.3f}]"
    )

write_csv(RESULT_DIR / "low_alt_bias_sweep.csv", candidate_rows)
write_csv(RESULT_DIR / "low_alt_bias_candidate_traces.csv", all_candidate_traces)
...
for rank,row in enumerate(ranked[:min(6,len(ranked))],start=1):
    print(
        ...
        f"floor={row.get('low_alt_bias_floor', float('nan')):+.3f} | t={row['duration_s']:.1f}s | "
        f"ALT={row['final_altitude_ft']:.2f} VS={row['final_vertical_speed_fps']:+.3f} | "
...
if not passing:
    ...
    diagnostic={
        "ready":False,
        "reason":"No low-altitude feedforward-floor candidate passed the full 300->30 ft descent. Do not train Stage 4 or tune touchdown yet.",
        "root_cause_hypothesis":"Previous bias fade-to-zero created a structural low-altitude equilibrium above the 30-ft target.",
    ...
    print("\nNO PASSING LOW-ALTITUDE BIAS CANDIDATE.")
    print("Do NOT widen actuator authority from this experiment. Review the low-altitude vertical trace first.")
    raise SystemExit(2)

best=sorted(passing,key=candidate_score)[0]
print(f"\nSELECTED: C{best['candidate_index']:02d} | low_alt_bias_floor={best['low_alt_bias_floor']:+.3f}")

rule("D — FRESH DETAILED REPEAT OF SELECTED LOW-ALTITUDE BIAS")
final_result, final_trace = run_teacher_case(
    low_alt_bias_floor=best["low_alt_bias_floor"],
...
final_summary={
    "stage":"Stage 4 low-altitude collective feedforward floor V1",
    ...
    "selected_parameters":{
        ...
        "vs_kp":float(LOCKED_VS_KP),
        "vs_ki":float(LOCKED_VS_KI),
        "low_alt_bias_floor":float(best["low_alt_bias_floor"]),
        "capture_blend_start_alt_ft":float(CAPTURE_BLEND_START_ALT_FT),
        "target_altitude_ft":float(DESCENT_TARGET_ALT_FT),
        "long_kpos_phys":float(LOCKED_LONG_KPOS_PHYS),
        "long_kv_phys":float(LOCKED_LONG_KV_PHYS),
        "lateral_kp":float(LOCKED_LATERAL_KP),
        "lateral_kd":float(LOCKED_LATERAL_KD),
        "elevator_physical_min":float(ELEVATOR_PHYSICAL_MIN),
        "elevator_physical_max":float(ELEVATOR_PHYSICAL_MAX),
    },
...
rule("STAGE-4 LOW-ALTITUDE BIAS V1 — RESULT")
print(
    f"PASS={final_result['teacher_pass']} SAFE={final_result['safe']} TERM={final_result['termination_reason']} | "
    f"ALT={final_result['final_altitude_ft']:.3f} VS={final_result['final_vertical_speed_fps']:+.3f} | "
    f"FWD={final_result['final_forward_ft']:.3f} err={final_result['final_position_error_ft']:+.3f} V={final_result['final_forward_speed_fps']:+.3f} | "
    f"X={final_result['final_cross_track_ft']:+.3f} LAT={final_result['final_lateral_speed_fps']:+.3f} | "
    f"MAXerr={final_result['max_abs_position_error_ft']:.3f} XMAX={final_result['max_abs_cross_track_ft']:.3f} | "
    f"HOLD={final_result['settle_hold_s']:.2f}s"
)
print("Saved:")
print(" ",RESULT_DIR / "low_alt_bias_sweep.csv")
print(" ",RESULT_DIR / "low_alt_bias_candidate_traces.csv")
print(" ",RESULT_DIR / "best_teacher_trace.csv")
print(" ",RESULT_DIR / "final_summary.json")
print("NEXT: only if PASS=True, treat 300->30 ft teacher as qualified and begin near-ground/touchdown identification. No PPO training yet.")
