...
"""
AH-1S / JSBSim
STAGE 4 LATERAL-DESCENT TEACHER RECALIBRATION V2
================================================
...
print("Stage-4 lateral teacher V2 target: keep the qualified descent inside the 5 ft cross-track corridor to stable 30 ft AGL hover")
...
rule("B — STAGE-4 LATERAL TEACHER V2 SWEEP DURING QUALIFIED DESCENT")
print("No Stage-4 PPO training. No Stage-4 runtime neural controller during descent.")
print("Mission-manager entry and A1=-1.0 braking are fixed from V3 qualification.")
print("Descent begins only after Stage-4 itself holds the original endpoint envelope for >=5 s.")
print("Vertical regime is fixed at Vmax=1.20 ft/s, VS_Kp=0.35. Only lateral Kp/Kd are swept.")
print(
...
idx = 0
for lateral_kp in LATERAL_KP_CANDIDATES:
    for lateral_kd in LATERAL_KD_CANDIDATES:
        idx += 1
        print(
            f"Candidate {idx:2d}/{len(LATERAL_KP_CANDIDATES)*len(LATERAL_KD_CANDIDATES)} | "
            f"LAT_Kp={lateral_kp:.3f} | LAT_Kd={lateral_kd:.2f}"
        )
        ...
        print(
            ...
            f"satHi={100.0*result.get('action2_upper_bound_fraction',0.0):.1f}%"
        )

write_csv(RESULT_DIR / "lateral_teacher_sweep.csv", candidate_rows)
write_csv(RESULT_DIR / "lateral_candidate_traces.csv", all_candidate_traces)

passing = [r for r in candidate_rows if r["teacher_pass"]]
ranked = sorted(candidate_rows, key=candidate_score)

rule("C — STAGE-4 LATERAL TEACHER SELECTION")
for rank, row in enumerate(ranked[: min(6, len(ranked))], start=1):
    print(
        ...
        f"LAT_Kp={row['lateral_kp']:.3f} LAT_Kd={row['lateral_kd']:.2f} | "
...
if not passing:
    ...
    diagnostic = {
        "ready": False,
        "reason": "No Stage-4 lateral teacher candidate passed during the locked descent regime. Do not distill or proceed to landing.",
...
print(
    "\nSELECTED: "
    f"C{best['candidate_index']:02d} | LAT_Kp={best['lateral_kp']:.3f} | "
    f"LAT_Kd={best['lateral_kd']:.2f}"
)

rule("D — FRESH DETAILED REPEAT OF SELECTED DESCENT TEACHER")
final_result, final_trace = run_teacher_case(
    lateral_kp=best["lateral_kp"],
    lateral_kd=best["lateral_kd"],
...
final_summary = {
    "stage": "Stage 4 lateral teacher V2: recalibrated cross-track hold during qualified 300->30 ft descent",
    ...
    "selected_parameters": {
        ...
        "fixed_brake_action1": FIXED_BRAKE_A1,
        "lateral_kp": float(best["lateral_kp"]),
        "lateral_kd": float(best["lateral_kd"]),
...
rule("STAGE 4 LATERAL TEACHER V2 — RESULT")
print(
...
print("Saved:")
print(" ", RESULT_DIR / "lateral_teacher_sweep.csv")
print(" ", RESULT_DIR / "lateral_candidate_traces.csv")
...
print("\nSTAGE-4 DESCENT LATERAL TEACHER QUALIFIED TO 30 FT AGL.")
