...
RESULT_DIR = Path(
    "results_stage4_near_ground_longitudinal_mapping_v1"
)
...
rule("B — NEAR-GROUND LONGITUDINAL MAPPING SWEEP (30 FT -> 9 FT)")
print(
    "Qualified 300->30 ft teacher is unchanged. "
    "Only the <30-ft physical elevator normalization and its "
    "longitudinal feedback gains are swept."
)
print(
...
print(
    "Measured physical delta authority about -0.180 baseline: "
    f"[{NEAR_BRAKE_DELTA_MIN:+.4f}, "
    f"{NEAR_RELEASE_DELTA_MAX:+.4f}]"
)
print(
    f"Targets: {NEAR_GROUND_TARGETS_FT}; "
    f"vertical max descent={NEAR_GROUND_VMAX_FPS:.2f} ft/s"
)
...
for kpos in NEAR_LONG_KPOS_GRID:
    for kv in NEAR_LONG_KV_GRID:
        ...
        print(
            ...
            f"NEAR_LONG_Kpos={kpos:.4f} | "
            f"NEAR_LONG_Kv={kv:.3f}"
        )

        result, trace = run_near_ground_candidate(
            long_kpos_phys=kpos,
            long_kv_phys=kv,
            detailed=False,
        )
        ...
        print(
            ...
            f"levels={result['levels_passed']}/{result['levels_total']} | "
            ...
            f"ELEV=[{result['physical_elevator_min']:+.6f},"
            f"{result['physical_elevator_max']:+.6f}] | "
            f"satBrake={100*result['brake_saturation_fraction']:.1f}% "
            f"satRelease={100*result['release_saturation_fraction']:.1f}%"
        )


rule("C — NEAR-GROUND LONGITUDINAL MAPPING SELECTION")
ranked = sorted(rows, key=candidate_sort_key)

for rank, row in enumerate(ranked, start=1):
    print(
        f"#{rank} C{row['case']:02d} | "
        f"PASS={row['teacher_pass']} SAFE={row['safe']} | "
        f"Kpos={row['long_kpos_phys']:.4f} "
        f"Kv={row['long_kv_phys']:.3f} | "
        f"levels={row['levels_passed']}/{row['levels_total']} | "
        ...
        f"satB={100*row['brake_saturation_fraction']:.1f}% "
        f"satR={100*row['release_saturation_fraction']:.1f}%"
    )

passing = [r for r in ranked if r["teacher_pass"]]
...
if passing:
    ...
    rule("D — FRESH DETAILED REPEAT OF SELECTED NEAR-GROUND MAPPING")
    fresh_result, fresh_trace = run_near_ground_candidate(
    ...
    print(
        "FRESH RESULT | "
        f"PASS={fresh_result['teacher_pass']} "
        f"SAFE={fresh_result['safe']} "
        f"TERM={fresh_result['termination_reason']} | "
        f"levels={fresh_result['levels_passed']}/"
        f"{fresh_result['levels_total']} | "
    ...
    print()
    print("NO PASSING NEAR-GROUND MAPPING CANDIDATE.")


rule("E — NEAR-GROUND MAPPING V1 RESULT")
ready = bool(
    fresh_result is not None
    and fresh_result["teacher_pass"]
    and fresh_result["safe"]
)

print(
    f"READY FOR TOUCHDOWN IDENTIFICATION: {ready}"
)

if fresh_result is not None:
    print(
        "Selected mapping authority | "
        f"action1=-1 -> {NEAR_ELEVATOR_PHYSICAL_MIN:+.6f}, "
        f"action1=+1 -> {NEAR_ELEVATOR_PHYSICAL_MAX:+.6f}"
    )
    print(
        "Selected feedback | "
        f"Kpos={fresh_result['long_kpos_phys']:.4f}, "
        f"Kv={fresh_result['long_kv_phys']:.3f}"
    )
    print(
        "Saturation | "
        f"brake={100*fresh_result['brake_saturation_fraction']:.1f}% "
        f"release={100*fresh_result['release_saturation_fraction']:.1f}%"
    )

save_rows_csv(
    RESULT_DIR / "near_ground_mapping_sweep.csv",
    [
        {k: v for k, v in r.items() if k != "level_results"}
        for r in rows
    ],
)
save_rows_csv(
    RESULT_DIR / "near_ground_mapping_traces.csv",
    all_traces,
)

if fresh_result is not None:
    save_rows_csv(
        RESULT_DIR / "best_near_ground_trace.csv",
        fresh_trace,
    )
    save_rows_csv(
        RESULT_DIR / "best_near_ground_level_summary.csv",
        fresh_result["level_results"],
    )

summary = {
    "stage4_training": "NONE",
    "high_altitude_300_to_30_teacher_changed": False,
    ...
    "near_ground_brake_delta_min": (
        NEAR_BRAKE_DELTA_MIN
    ),
    "near_ground_release_delta_max": (
        NEAR_RELEASE_DELTA_MAX
    ),
    "kpos_grid": NEAR_LONG_KPOS_GRID,
    "kv_grid": NEAR_LONG_KV_GRID,
    "near_ground_targets_ft": NEAR_GROUND_TARGETS_FT,
    "rows": rows,
    "fresh_result": fresh_result,
    "ready_for_touchdown_identification": bool(ready),
}
...
print("Saved:")
print(
    f"  {RESULT_DIR / 'near_ground_mapping_sweep.csv'}"
)
print(
    f"  {RESULT_DIR / 'near_ground_mapping_traces.csv'}"
)
if fresh_result is not None:
    print(
        f"  {RESULT_DIR / 'best_near_ground_trace.csv'}"
    )
    print(
        f"  {RESULT_DIR / 'best_near_ground_level_summary.csv'}"
    )
print(
...
print()
print(
    "Do NOT train Stage 4 here. If READY=True, the 30->9 ft "
    "near-ground regime is qualified and the next step is a separate "
    "WOW/compression-based touchdown identification."
)
