...
#   continuous touchdown approach based on measured lower ground-effect curve
#   actual WOW contact
...
def run_full_stage4_teacher(detailed=True):
    ...
    for step in range(int(TOUCHDOWN_MAX_TIME_S / dt)):
        ...
        ff_coll = lower_feedforward_collective(
            state_before["altitude_ft"],
            lower_curve["points"],
            touchdown_start_alt,
        ...
        desired_coll = float(np.clip(
            ff_coll + TOUCHDOWN_KVS * vs_error,
            TOUCHDOWN_COLL_MIN,
            TOUCHDOWN_COLL_MAX,
        ))
        ...
        append_trace(
            ...
            used,
            {
                "lower_feedforward_collective": ff_coll,
                "vs_des_fps": vs_des,
...
rule("A — FULL STAGE-4 TEACHER SAME-FDM VALIDATION")
result, trace = run_full_stage4_teacher(
    detailed=True
)


rule("B — FULL STAGE-4 TEACHER CONCLUSION")
print(
    f"PASS={result['pass']} | "
    f"same_fdm={result['same_fdm']} | "
    f"clock_reset={result['clock_reset_on_attach']} | "
    f"presentation_pass={result['presentation_pass']}"
)
...
print(
    "READY FOR STAGE-4 DISTILLATION DATA COLLECTION: "
    f"{result['pass']}"
)

save_rows_csv(
    FULL_RESULT_DIR / "full_teacher_trace.csv",
    trace,
)
...
print("Saved:")
print(f"  {FULL_RESULT_DIR / 'full_teacher_trace.csv'}")
print(f"  {FULL_RESULT_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT train Stage 4 in this script. "
    "If READY=True, this continuous same-FDM Stage-4 teacher is the source "
    "behavior for the next rollout-collection / policy-distillation step."
)
