...
MODEL_PATHS = [
    (
        "BEST",
        "models/task11_hover300/best/best_model"
    ),
    (
        "FINAL",
        "models/task11_hover300/ppo_ah1s_hover300_v11_final"
    )
]
...
def test_model(model_path, label):
    ...
    entered_hover_zone = False

    best_hover_snapshot = None
    ...
    for step in range(1, 5001):
        ...
        if 290.0 <= altitude <= 310.0:
            entered_hover_zone = True

        # ==========================================================
        # BEST HOVER SNAPSHOT
        # ==========================================================
        #
        # Hold artmasa bile hedefe en yakın durumu saklıyoruz.
        # Böylece V11'in ne kadar yaklaştığını görebiliriz.
        #
        # Daha düşük skor daha iyi.
        # ==========================================================

        hover_score = (
            abs(altitude - 300.0) / 10.0
            + abs(vertical_speed) / 2.0
            + abs(forward_velocity) / 8.0
            + abs(lateral_velocity) / 8.0
            + abs(heading_error) / 0.25
            + abs(roll) / 0.20
            + abs(pitch) / 0.20
        )

        if (
            280.0 <= altitude <= 320.0
        ):

            if (
                best_hover_snapshot is None
                or hover_score
                < best_hover_snapshot["score"]
            ):
                ...
                best_hover_snapshot = {
                    ...
                    "pitch": pitch,
                    "hold": hold,
                    "collective_action": float(
                        action[0]
                    ),
                    "elevator_action": float(
                        action[1]
                    ),
                    "aileron_action": float(
                        action[2]
                    ),
                    "rudder_action": float(
                        action[3]
        ...
        if step % 25 == 0:
            ...
            print(
                ...
                f"TgtVS {info['target_vertical_speed']:6.2f} | "
    ...
    print()
    print("BEST HOVER SNAPSHOT")

    if best_hover_snapshot is None:

        print(
            "No state entered the 280-320 ft analysis zone."
        )
        ...
        print(
            "Step:",
            best_hover_snapshot["step"]
        )

        print(
            "Score:",
            f"{best_hover_snapshot['score']:.3f}"
        )

        print(
            "Altitude:",
            f"{best_hover_snapshot['altitude']:.2f} ft"
        )

        print(
            "Vertical speed:",
            f"{best_hover_snapshot['vertical_speed']:.2f} ft/s"
        )

        print(
            "Forward velocity:",
            f"{best_hover_snapshot['forward_velocity']:.2f} ft/s"
        )

        print(
            "Lateral velocity:",
            f"{best_hover_snapshot['lateral_velocity']:.2f} ft/s"
        )

        print(
            "Heading error:",
            f"{best_hover_snapshot['heading_error']:.3f} rad"
        )

        print(
            "Roll:",
            f"{best_hover_snapshot['roll']:.3f} rad"
        )

        print(
            "Pitch:",
            f"{best_hover_snapshot['pitch']:.3f} rad"
        )

        print(
            "Hold:",
            f"{best_hover_snapshot['hold']}/100"
        )

        print(
            "Actions:",
            f"A0={best_hover_snapshot['collective_action']:+.3f} "
            f"A1={best_hover_snapshot['elevator_action']:+.3f} "
            f"A2={best_hover_snapshot['aileron_action']:+.3f} "
            f"A3={best_hover_snapshot['rudder_action']:+.3f}"
        )
