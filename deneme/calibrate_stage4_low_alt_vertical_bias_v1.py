...
RESULT_DIR = Path(
    "results_stage4_low_alt_vertical_bias_v1"
)
...
# =====================================================================
# STAGE-4 LOW-ALTITUDE VERTICAL-BIAS CALIBRATION V1
# =====================================================================
...
def final_settle_now(state):
    return bool(
        abs(state["altitude_ft"] - TARGET_ALT_FT) <= FINAL_ALT_TOL_FT
...
def rank_key(row):
    return (
        0 if row["pass"] else 1,
        0 if row["safe"] else 1,
        abs(row["final_altitude_ft"] - TARGET_ALT_FT),
        row["max_abs_position_error_ft"],
        row["max_abs_cross_track_ft"],
...
rule("B — LOW-ALTITUDE ADAPTIVE-BIAS MINIMUM SWEEP (30 FT -> 9 FT)")
print(
    "Qualified 300->30 ft teacher is unchanged. "
    "Below 25 ft only the adaptive vertical-bias lower bound is changed."
)
print(
    f"Fixed near-ground longitudinal operating point: "
    f"Kpos={FIXED_NEAR_LONG_KPOS:.4f}, "
    f"Kv={FIXED_NEAR_LONG_KV:.3f}"
)
print(
    f"Total collective residual remains hard-limited to "
    f"[{COLLECTIVE_RESIDUAL_MIN:+.2f},"
    f"{COLLECTIVE_RESIDUAL_MAX:+.2f}]"
)
print(
    f"Bias-min candidates below {LOW_ALT_BIAS_ONSET_FT:.1f} ft: "
    f"{LOW_ALT_BIAS_MIN_GRID}"
)

rows = []
all_traces = []

for case_idx, bias_min in enumerate(
    LOW_ALT_BIAS_MIN_GRID,
    start=1,
):
    print(
        f"Candidate {case_idx}/{len(LOW_ALT_BIAS_MIN_GRID)} | "
        f"LOW_ALT_BIAS_MIN={bias_min:+.3f}"
    )

    result, trace = run_bias_candidate(
        low_alt_bias_min=bias_min,
        detailed=False,
    )
    ...
    print(
        ...
        f"VBminObs={result['vertical_bias_min_observed']:+.3f} "
        f"dA0min={result['collective_residual_min']:+.3f} | "
        f"satTotal={100*result['total_residual_low_saturation_fraction']:.1f}% "
        f"satBias={100*result['bias_min_saturation_fraction']:.1f}% | "
        f"crossings={crossed or 'none'}"
    )


rule("C — LOW-ALTITUDE BIAS SELECTION")
...
for rank, row in enumerate(ranked, start=1):
    print(
        ...
        f"biasMin={row['low_alt_bias_min']:+.3f} | "
        ...
        f"dA0min={row['collective_residual_min']:+.3f} "
        f"satTotal={100*row['total_residual_low_saturation_fraction']:.1f}% "
        f"satBias={100*row['bias_min_saturation_fraction']:.1f}%"
    )
...
if passing:
    ...
    print(
        ...
        f"LOW_ALT_BIAS_MIN={selected['low_alt_bias_min']:+.3f}"
    )

    rule("D — FRESH DETAILED REPEAT")
    fresh_result, fresh_trace = run_bias_candidate(
        low_alt_bias_min=selected["low_alt_bias_min"],
        detailed=True,
    )
    ...
    print(
        ...
        f"VBminObs={fresh_result['vertical_bias_min_observed']:+.3f} "
        f"dA0min={fresh_result['collective_residual_min']:+.3f} | "
        f"HOLD={fresh_result['settle_hold_s']:.2f}s"
    )
else:
    print()
    print("NO PASSING LOW-ALTITUDE BIAS CANDIDATE.")


rule("E — LOW-ALTITUDE VERTICAL-BIAS V1 RESULT")
...
best = ranked[0]
ready_capture_tuning = bool(
    not ready_touchdown
    and best["safe"]
    and best["final_altitude_ft"] <= 10.5
...
print(
    f"READY FOR FINAL 9-FT CAPTURE TUNING: {ready_capture_tuning}"
)
print(
    "Best state | "
    ...
    f"VBminObs={best['vertical_bias_min_observed']:+.3f} "
    f"dA0min={best['collective_residual_min']:+.3f}"
)

save_rows_csv(
    RESULT_DIR / "low_alt_bias_sweep.csv",
    [
...
save_rows_csv(
    RESULT_DIR / "low_alt_bias_traces.csv",
    all_traces,
)

if fresh_result is not None:
    save_rows_csv(
        RESULT_DIR / "best_low_alt_bias_trace.csv",
        fresh_trace,
    )

summary = {
    "stage4_training": "NONE",
    "qualified_300_to_30_teacher_changed": False,
    "near_ground_longitudinal_kpos": FIXED_NEAR_LONG_KPOS,
    "near_ground_longitudinal_kv": FIXED_NEAR_LONG_KV,
    "low_alt_bias_onset_ft": LOW_ALT_BIAS_ONSET_FT,
    "low_alt_bias_min_grid": LOW_ALT_BIAS_MIN_GRID,
    "total_collective_residual_min": COLLECTIVE_RESIDUAL_MIN,
    "total_collective_residual_max": COLLECTIVE_RESIDUAL_MAX,
    "rows": rows,
    "fresh_result": fresh_result,
    "ready_for_touchdown_identification": bool(ready_touchdown),
    "ready_for_final_9ft_capture_tuning": bool(
        ready_capture_tuning
    ),
}
...
print("Saved:")
print(
    f"  {RESULT_DIR / 'low_alt_bias_sweep.csv'}"
)
print(
    f"  {RESULT_DIR / 'low_alt_bias_traces.csv'}"
)
if fresh_result is not None:
    print(
        f"  {RESULT_DIR / 'best_low_alt_bias_trace.csv'}"
    )
print(
...
print(
    ...
    "This calibration only allows the low-altitude integrator to use "
    "more of the collective residual authority already qualified by system ID."
)
