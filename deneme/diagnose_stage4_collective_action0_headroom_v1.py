...
RESULT_DIR = Path(
    "results_stage4_collective_action0_headroom_v1"
)
...
rule("B — NATIVE ACTION0 HEADROOM CHARACTERIZATION")
trigger_run = build_native_headroom_trigger()
trig = trigger_run["trigger_state"]
ctrl = trigger_run["trigger_ctrl"]
a0_native = trigger_run["native_action0"]
headroom = trigger_run["remaining_action0_headroom"]
...
print(
    ...
    f"A0native={a0_native:+.6f} | "
    f"headroom_to_-1={headroom:.6f} | "
    f"COLL={trig['physical_collective_cmd']:+.6f}"
)
close_handoff(trigger_run)


rule("C — NATIVE ACTION0 HEADROOM PULSES")
pulse_rows = []
pulse_trace_rows = []

for case_idx, frac in enumerate(HEADROOM_FRACTIONS, start=1):
    run = build_native_headroom_trigger()
    ...
    native_a0 = float(run["native_action0"])
    headroom = float(run["remaining_action0_headroom"])
    forced_a0 = float(native_a0 - float(frac) * headroom)
    ...
    for step in range(int(PULSE_SECONDS / dt)):
        ...
        t = (step + 1) * dt
        physical_collectives.append(state["physical_collective_cmd"])

        pulse_trace_rows.append({
            "case": int(case_idx),
            "headroom_fraction": float(frac),
            "time_s": float(t),
            "native_action0": float(native_a0),
            "forced_action0": float(forced_a0),
            "physical_collective_cmd": float(state["physical_collective_cmd"]),
            "mapped_physical_elevator": float(mapped_elev),
            **{k: float(v) for k, v in state.items()},
        })

        reason = diag_safety_reason(state)
        if reason:
            safe = False
            term = reason
            break
    ...
    row = {
        ...
        "native_action0": float(native_a0),
        "forced_action0": float(forced_a0),
        "delta_action0": float(forced_a0 - native_a0),
        "physical_collective_mean": float(np.mean(physical_collectives)) if physical_collectives else float("nan"),
        "delta_vertical_speed_fps": float(final["vertical_speed_fps"] - initial["vertical_speed_fps"]),
        "delta_altitude_ft": float(final["altitude_ft"] - initial["altitude_ft"]),
        "delta_forward_speed_fps": float(final["forward_speed_fps"] - initial["forward_speed_fps"]),
        "delta_cross_track_ft": float(final["cross_track_ft"] - initial["cross_track_ft"]),
    ...
    print(
        f"Case {case_idx:02d} frac={frac:.2f} | "
        f"A0={forced_a0:+.6f} "
        f"dA0={row['delta_action0']:+.6f} | "
        f"SAFE={safe} TERM={term} | "
        f"dVS@2s={row['delta_vertical_speed_fps']:+.4f} "
        f"dALT={row['delta_altitude_ft']:+.3f} | "
        f"COLLmean={row['physical_collective_mean']:+.6f} | "
        f"FWDerr={row['final_position_error_ft']:+.2f} "
        f"X={row['final_cross_track_ft']:+.2f}"
    )
...
if len(safe_pulses) >= 3:
    x = np.asarray([r["delta_action0"] for r in safe_pulses], dtype=float)
    y = np.asarray([r["delta_vertical_speed_fps"] for r in safe_pulses], dtype=float)
    ...
    print()
    print(
        f"FIT dVS(2s) ~= {fit_slope:+.6f} * delta_action0 "
        f"{fit_intercept:+.6f}   (R^2={fit_r2:.4f})"
    )


rule("D — SUSTAINED NATIVE ACTION0 HEADROOM CHECKS")
sustained_rows = []
sustained_trace_rows = []

for case_idx, frac in enumerate(SUSTAINED_HEADROOM_FRACTIONS, start=1):
    run = build_native_headroom_trigger()
    ...
    final = snapshot(fdm, lat0, lon0, mission_heading)
    vs_mean2 = float(np.mean(vs_tail)) if vs_tail else float("nan")
    ...
    print(
        ...
        f"FWDerr={final['position_error_ft']:+.2f} "
...
rule("E — NATIVE ACTION0 HEADROOM CONCLUSION")
full_native = next(
    (r for r in sustained_rows if abs(r["headroom_fraction"] - 1.0) < 1e-9),
    None,
)

native_headroom_useful = bool(
    full_native is not None
    and full_native["safe"]
    and full_native["corridor"]
    and np.isfinite(full_native["vs_mean_last2s"])
    and full_native["vs_mean_last2s"] < -0.05
)

native_headroom_likely_sufficient = bool(
    native_headroom_useful
    and full_native["final_altitude_ft"] <= 15.0
)

ready_physical_mapping_id = bool(
    not native_headroom_likely_sufficient
    and full_native is not None
    and full_native["safe"]
    and full_native["corridor"]
)

print(f"native action0 headroom useful: {native_headroom_useful}")
if full_native is not None:
    print(
        "full native minimum evidence: "
        f"A0={full_native['forced_action0']:+.6f}, "
        f"VSmean(last2s)={full_native['vs_mean_last2s']:+.3f}, "
        f"ALT={full_native['final_altitude_ft']:.2f}, "
        f"FWDerr={full_native['final_position_error_ft']:+.2f}, "
        f"X={full_native['final_cross_track_ft']:+.2f}, "
        f"COLLmean={full_native['physical_collective_mean']:+.6f}"
    )

print(f"NATIVE ACTION0 HEADROOM LIKELY SUFFICIENT: {native_headroom_likely_sufficient}")
print(f"READY FOR LOW-ALT PHYSICAL COLLECTIVE MAPPING ID: {ready_physical_mapping_id}")

save_rows_csv(
    RESULT_DIR / "native_action0_pulse_sweep.csv",
    pulse_rows,
)
save_rows_csv(
    RESULT_DIR / "native_action0_pulse_traces.csv",
    pulse_trace_rows,
)
save_rows_csv(
    RESULT_DIR / "native_action0_sustained_sweep.csv",
    sustained_rows,
)
save_rows_csv(
    RESULT_DIR / "native_action0_sustained_traces.csv",
    sustained_trace_rows,
)
...
summary = {
    ...
    "diag_low_alt_bias_min": DIAG_LOW_ALT_BIAS_MIN,
    "native_action0_pulse_headroom_fractions": HEADROOM_FRACTIONS,
    "native_action0_sustained_headroom_fractions": SUSTAINED_HEADROOM_FRACTIONS,
    "fit_slope": float(fit_slope),
    "fit_intercept": float(fit_intercept),
    "fit_r2": float(fit_r2),
    "native_action0_headroom_useful": bool(native_headroom_useful),
    "native_action0_headroom_likely_sufficient": bool(native_headroom_likely_sufficient),
    "ready_for_low_alt_physical_collective_mapping_id": bool(ready_physical_mapping_id),
    "pulse_rows": pulse_rows,
    "sustained_rows": sustained_rows,
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'native_action0_pulse_sweep.csv'}")
print(f"  {RESULT_DIR / 'native_action0_pulse_traces.csv'}")
print(f"  {RESULT_DIR / 'native_action0_sustained_sweep.csv'}")
print(f"  {RESULT_DIR / 'native_action0_sustained_traces.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. This diagnostic first exhausts the native "
    "normalized action0 range before any Stage-4-specific physical collective "
