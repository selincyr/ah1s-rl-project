...
OUT_DIR = Path("results_stage3_speed_scheduled_lateral_v4")
OUT_DIR.mkdir(parents=True, exist_ok=True)

SUMMARY_CSV = OUT_DIR / "stage3_speed_scheduled_lateral_v4_sweep.csv"
TRACE_CSV = OUT_DIR / "stage3_speed_scheduled_lateral_v4_best_trace.csv"
...
# ------------------------------------------------------------
# STAGE-3 SPEED-SCHEDULED PRE-ENDPOINT LATERAL HOLD
# ------------------------------------------------------------
# V3 showed that using a fixed -0.230 cruise aileron baseline throughout
# braking keeps a left-bias as forward speed falls.  The endpoint teacher
# immediately needs a positive/near-neutral aileron and then recovers, so V4
# keeps the SAME lateral PD correction but smoothly blends the baseline from
# the Stage-2 cruise value toward hover trim as forward speed decreases.
# Longitudinal braking and altitude control are unchanged.
CORRIDOR_KP = 0.020
CORRIDOR_KD = 0.180
MAX_CORRIDOR_CORR = 0.30

# Small, focused sweep: when should the cruise->hover trim blend begin?
# At <= LATERAL_ENABLE_SPEED_FPS the scheduled baseline has reached hover trim.
BLEND_START_SPEEDS_FPS = [3.0, 4.0, 5.0]
BLEND_END_SPEED_FPS = LATERAL_ENABLE_SPEED_FPS
...
def teacher_action(
    ...
    if lateral_active:
        ...
        # Airspeed-scheduled aileron baseline.  At high forward speed keep
        # the proven Stage-2 cruise trim (-0.230).  As the aircraft slows,
        # smoothly move the baseline toward hover trim before endpoint takeover.
        speed_for_schedule = abs(state["forward_speed_fps"])
        denom = max(
            blend_start_speed_fps - BLEND_END_SPEED_FPS,
            1e-6,
        )
        cruise_weight = float(
            np.clip(
                (speed_for_schedule - BLEND_END_SPEED_FPS) / denom,
                0.0,
                1.0,
            )
        )
        scheduled_trim = float(
            cruise_weight * CRUISE_AILERON_ACTION
            + (1.0 - cruise_weight) * hover_trim
        )

        action[2] = float(
            np.clip(
                scheduled_trim + lateral_corr,
                -1.0,
                +1.0,
            )
        )
        ...
        lateral_corr = 0.0
        scheduled_trim = CRUISE_AILERON_ACTION
        action[2] = CRUISE_AILERON_ACTION

    if lateral_active:
        scheduled_trim = hover_trim
        ...
        row.update(
            {
                ...
                "scheduled_aileron_trim": float(
                    scheduled_trim
                ),
                "blend_start_speed_fps": float(
                    blend_start_speed_fps
                ),
...
# ============================================================
# FOCUSED V4 SPEED-SCHEDULE SWEEP
# ============================================================

rule("STAGE 3 — SPEED-SCHEDULED LATERAL HOLD V4")

print("Stage 1 model: LOCKED")
print("Stage 2 model: HYBRID FINAL")
print("Stage-3 longitudinal parameters: UNCHANGED")
print("Altitude controller: UNCHANGED")
print("No PPO training in this diagnostic.")
print("V3 diagnosis: fixed -0.230 cruise aileron bias remains too long as speed falls.")
print("V4 change: smoothly blend cruise aileron trim toward hover trim during braking.")
print()
print(f"Corridor Kp / Kd : {CORRIDOR_KP:.3f} / {CORRIDOR_KD:.3f}")
print(f"Corridor max corr: {MAX_CORRIDOR_CORR:.2f}")
print(f"Blend end speed  : {BLEND_END_SPEED_FPS:.2f} ft/s")
print(f"Blend starts     : {BLEND_START_SPEEDS_FPS}")
...
for idx, blend_start in enumerate(BLEND_START_SPEEDS_FPS, start=1):
    result = run_case(
        DIAG_HOVER_TRIM,
        DIAG_LATERAL_KP,
        blend_start,
    ...
    print(
        f"S{idx:02d} | BLEND_START={blend_start:4.1f} | "
        f"SAFE={result['safe']} | "
        f"STOP={result['persistent_stop']} | "
        f"HOVER={result['endpoint_hover']} | "
        f"PRES={result['presentation_cross_pass']} | "
        ...
        f"MAXX={result['max_cross_ft']:5.2f} | "
...
def selection_key(result):
    complete = bool(
        result["endpoint_hover"]
        and result["presentation_cross_pass"]
        and result["safe"]
    )
    return (
...
rule("BEST V4 CASE — FULL DETAILED FLIGHT")

best_detailed = run_case(
    DIAG_HOVER_TRIM,
    DIAG_LATERAL_KP,
    best["blend_start_speed_fps"],
    detailed=True,
)
...
if best_detailed["trace"]:
    with TRACE_CSV.open(
        ...
        writer = csv.DictWriter(
            f,
            fieldnames=list(best_detailed["trace"][0].keys()),
        )
        writer.writeheader()
        writer.writerows(best_detailed["trace"])
...
rule("STAGE 3 SPEED-SCHEDULED LATERAL V4 — FINAL RESULT")
...
for key in [
    ...
    print(f"{key:31s}: {best_detailed[key]}")

print()

complete_pass = bool(
    best_detailed["safe"]
    and best_detailed["endpoint_hover"]
    and best_detailed["presentation_cross_pass"]
)

if complete_pass:
    print("ENDPOINT HOVER: TRUE")
    print("PRESENTATION GEOMETRY: TRUE")
    print("V4 passed functional hover and the full <=5 ft lateral geometry criterion.")
    print("NEXT: freeze this Stage-3 teacher, then distill -> PPO fine-tune -> teacher-OFF validation.")
elif best_detailed["endpoint_hover"]:
    print("ENDPOINT HOVER: TRUE")
    print("PRESENTATION GEOMETRY: FALSE")
    print("Functional pass remains, but do NOT lock Stage 3 yet.")
else:
    print("ENDPOINT HOVER: FALSE")
    print("PRESENTATION GEOMETRY: FALSE")
    print("Do not change longitudinal braking from this result; inspect the lateral schedule trace first.")

print()
print("IMPORTANT: Stage 3 locks only if safety + endpoint hover + full geometry all pass together.")
