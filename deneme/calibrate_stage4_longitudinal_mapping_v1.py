...
RESULT_DIR = Path(
    "results_stage4_longitudinal_mapping_v1"
)
...
LONG_KPOS_CANDIDATES = [0.0004, 0.0007, 0.0010]
LONG_KV_CANDIDATES = [0.002, 0.004, 0.006]
...
def build_stage4_teacher_action(
    ...
    # longitudinal: one-sided feedback within the sustained-tested band.
    ahead_ft = float(state["forward_ft"] - TARGET_FORWARD_FT)
    brake_demand = (
        float(long_kpos_phys) * ahead_ft
        + float(long_kv_phys) * float(state["forward_speed_fps"])
    )
    ...
    # lateral: locked C12 operating point from the preceding calibration.
    lateral_corr = (
    ...
    return action, {
        ...
        "brake_demand_phys": float(brake_demand),
    ...
    for step in range(total_steps):
        ...
        if detailed and t + 1e-9 >= next_print:
            print(
                ...
                f"A1={used_action[1]:+6.3f} ELEV={mapped_elevator_cmd:+.6f} | HOLD={settle_hold:4.2f}s"
            )
            next_print += 20.0
...
print("Stage-4 longitudinal mapping V1 target: full qualified 300->30 ft descent inside both 5 ft horizontal corridors")
...
rule("B — STAGE-4 LONGITUDINAL MAPPING V1 SWEEP DURING QUALIFIED DESCENT")
...
print("Vertical regime fixed: Vmax=1.20 ft/s, VS_Kp=0.35. Lateral fixed: Kp=0.10, Kd=0.35.")
print("Only Stage-4 descending longitudinal physical-feedback gains are swept.")
...
idx = 0
for long_kpos_phys in LONG_KPOS_CANDIDATES:
    for long_kv_phys in LONG_KV_CANDIDATES:
        idx += 1
        print(
            f"Candidate {idx:2d}/{len(LONG_KPOS_CANDIDATES)*len(LONG_KV_CANDIDATES)} | "
            f"LONG_Kpos={long_kpos_phys:.4f} | LONG_Kv={long_kv_phys:.3f}"
        )
        ...
        print(
            ...
            f"ELEV=[{result.get('physical_elevator_min',float('nan')):+.6f},{result.get('physical_elevator_max',float('nan')):+.6f}] "
            f"satBrake={100.0*result.get('physical_elevator_min_bound_fraction',0.0):.1f}% | "
            f"A2=[{result['action2_min']:+.3f},{result['action2_max']:+.3f}]"
        )

write_csv(RESULT_DIR / "longitudinal_mapping_sweep.csv", candidate_rows)
write_csv(RESULT_DIR / "longitudinal_mapping_candidate_traces.csv", all_candidate_traces)

passing = [r for r in candidate_rows if r["teacher_pass"]]
ranked = sorted(candidate_rows, key=candidate_score)

rule("C — STAGE-4 LONGITUDINAL MAPPING SELECTION")
for rank, row in enumerate(ranked[: min(6, len(ranked))], start=1):
    print(
        ...
        f"LONG_Kpos={row['long_kpos_phys']:.4f} LONG_Kv={row['long_kv_phys']:.3f} | "
...
if not passing:
    ...
    diagnostic = {
        "ready": False,
        "reason": "No Stage-4 longitudinal mapping candidate passed the full 300->30 ft descent. Do not distill or proceed to landing.",
    ...
    print("\nNO PASSING LONGITUDINAL-MAPPING CANDIDATE.")
...
print(
    "\nSELECTED: "
    f"C{best['candidate_index']:02d} | LONG_Kpos={best['long_kpos_phys']:.4f} | "
    f"LONG_Kv={best['long_kv_phys']:.3f}"
)

rule("D — FRESH DETAILED REPEAT OF SELECTED LONGITUDINAL MAPPING")
final_result, final_trace = run_teacher_case(
    long_kpos_phys=best["long_kpos_phys"],
    long_kv_phys=best["long_kv_phys"],
...
final_summary = {
    "stage": "Stage 4 longitudinal mapping V1: calibrated action1->physical elevator mapping during qualified 300->30 ft descent",
    ...
    "selected_parameters": {
        ...
        "long_kpos_phys": float(best["long_kpos_phys"]),
        "long_kv_phys": float(best["long_kv_phys"]),
...
rule("STAGE 4 LONGITUDINAL MAPPING V1 — RESULT")
print(
...
print("Saved:")
print(" ", RESULT_DIR / "longitudinal_mapping_sweep.csv")
print(" ", RESULT_DIR / "longitudinal_mapping_candidate_traces.csv")
