...
RESULT_DIR = Path(
    "results_stage4_low_alt_collective_diagnostic_v1"
)
...
# =====================================================================
# STAGE-4 LOW-ALTITUDE COLLECTIVE AUTHORITY DIAGNOSTIC V1
# =====================================================================

# Operating point only; NOT locked as final near-ground longitudinal gains.
# C03 from continuous V2 reached the lowest repeatable altitude with clean geometry.
DIAG_LONG_KPOS = 0.0005
DIAG_LONG_KV = 0.006

TRIGGER_ALT_FT = 21.0
TRIGGER_MAX_ABS_VS_FPS = 0.10
TRIGGER_MAX_TIME_S = 220.0

PULSE_SECONDS = 2.0
COLLECTIVE_EXTRA_RESIDUAL_PULSES = [0.00, -0.02, -0.04, -0.06, -0.08]

SUSTAINED_SECONDS = 12.0
COLLECTIVE_EXTRA_RESIDUAL_SUSTAINED = [-0.02, -0.04, -0.06, -0.08]

DIAG_MIN_AGL_FT = 13.0
...
def build_low_alt_trigger():
    """
    Rebuild the locked chain, reproduce the qualified 300->30 ft teacher,
    then use the continuous near-ground controller until ~21 ft where the
    low-altitude vertical steady-state error is already visible.

    Returns the SAME open FDM at the trigger state.
    """
    start = build_qualified_30ft_open(detailed=False)
    ...
    for step in range(int(TRIGGER_MAX_TIME_S / dt)):
        ...
        action, ctrl, vertical_bias = build_near_ground_action_mapped(
            state_before,
            target_alt_ft=CONTINUOUS_TARGET_ALT_FT,
            hover_action0=hover_action0,
            vertical_bias=vertical_bias,
            dt=dt,
            long_kpos_phys=DIAG_LONG_KPOS,
            long_kv_phys=DIAG_LONG_KV,
        )
        ...
        if reason:
            close_handoff(start)
            raise RuntimeError(
                f"Low-altitude trigger approach failed before trigger: {reason}"
            )

        if any_wow(contact_snapshot(fdm)):
            close_handoff(start)
            raise RuntimeError("Unexpected WOW before low-altitude trigger.")

        if (
            state["altitude_ft"] <= TRIGGER_ALT_FT
            and abs(state["vertical_speed_fps"]) <= TRIGGER_MAX_ABS_VS_FPS
        ):
            trigger_state = state.copy()
            trigger_ctrl = dict(ctrl)
            trigger_action = np.asarray(used, dtype=np.float32).copy()
            trigger_elev = float(mapped_elev)
            break

    if trigger_state is None:
        # Accept the final near-equilibrium state only if it is clearly the same
        # diagnosed low-altitude regime.
        state = snapshot(fdm, lat0, lon0, mission_heading)
        if (
            state["altitude_ft"] <= 21.5
            and abs(state["vertical_speed_fps"]) <= 0.12
        ):
            state_before = snapshot(fdm, lat0, lon0, mission_heading)
            action, ctrl, _ = build_near_ground_action_mapped(
                state_before,
                target_alt_ft=CONTINUOUS_TARGET_ALT_FT,
                hover_action0=hover_action0,
                vertical_bias=vertical_bias,
            ...
            trigger_state = state.copy()
            trigger_ctrl = dict(ctrl)
            trigger_action = np.asarray(action, dtype=np.float32).copy()
            trigger_elev = near_action1_to_physical(float(action[1]))
        else:
            close_handoff(start)
            raise RuntimeError(
                "Did not reach the diagnosed ~20.5-ft low-altitude equilibrium."
            )
    ...
    return {
        **start,
        ...
        "trigger_elevator": float(trigger_elev),
        "trigger_elapsed_s": float(elapsed),
    }


def raw_near_ground_collective_offset_cycle(
    env2,
    fdm,
    action,
    base_collective_residual,
    extra_collective_residual,
    hover_action0,
    lat0,
    lon0,
    mission_heading,
    ...
    Keep the verified near-ground elevator mapping and all horizontal controls.
    Only add a small extra normalized collective residual, constrained by the
    already-qualified total Stage-4 residual envelope [-0.50,+0.15].
    """
    action = np.asarray(action, dtype=np.float32).reshape(-1).copy()

    total_residual = float(np.clip(
        float(base_collective_residual) + float(extra_collective_residual),
        COLLECTIVE_RESIDUAL_MIN,
        COLLECTIVE_RESIDUAL_MAX,
    ))
    action[0] = float(np.clip(
        float(hover_action0) + total_residual,
        -1.0,
        +1.0,
    ))
    ...
    # Preserve the verified near-ground action1 physical mapping.
    mapped_elevator = near_action1_to_physical(float(action[1]))
    fdm["fcs/elevator-cmd-norm"] = mapped_elevator
    ...
    for _ in range(physics_steps(env2)):
        if not fdm.run():
            raise RuntimeError(
                "JSBSim stopped during low-altitude collective diagnostic."
            )
...
def diagnostic_safety_reason(state):
...
print(
    "HANDOFF | "
    f"PASS={audit['handoff_pass']} | "
    f"same_fdm={audit['same_fdm']} | "
    f"clock_reset={audit['clock_reset_on_attach']} | "
    f"FWD={audit_state['forward_ft']:.3f} "
    f"V={audit_state['forward_speed_fps']:+.3f} | "
    f"X={audit_state['cross_track_ft']:+.3f} "
    f"ALT={audit_state['altitude_ft']:.3f} | "
    f"HOLD={audit['stage3_hover_hold_s']:.2f}s"
)
if not audit["handoff_pass"]:
    close_handoff(audit)
    raise RuntimeError("Locked Stage1->2->3 qualification failed.")
close_handoff(audit)


rule("B — LOW-ALTITUDE EQUILIBRIUM CHARACTERIZATION")
trigger_run = build_low_alt_trigger()
trig = trigger_run["trigger_state"]
ctrl = trigger_run["trigger_ctrl"]

print(
    "TRIGGER | "
    f"t={trigger_run['trigger_elapsed_s']:.2f}s | "
    f"ALT={trig['altitude_ft']:.3f} "
    f"VS={trig['vertical_speed_fps']:+.4f} | "
    f"FWD={trig['forward_ft']:.3f} "
    f"err={trig['position_error_ft']:+.3f} "
    f"V={trig['forward_speed_fps']:+.4f} | "
    f"X={trig['cross_track_ft']:+.3f} "
    f"LAT={trig['lateral_speed_fps']:+.4f} | "
    f"VB={trigger_run['vertical_bias']:+.3f} | "
    f"VSdes={ctrl['vs_des_fps']:+.3f} "
    f"dA0={ctrl['collective_residual']:+.3f} | "
    f"COLL={trig['physical_collective_cmd']:+.6f}"
)
print(
    "Available qualified residual margin to lower bound: "
    f"{ctrl['collective_residual'] - COLLECTIVE_RESIDUAL_MIN:+.3f}"
)
close_handoff(trigger_run)


rule("C — LOW-ALTITUDE COLLECTIVE RESIDUAL PULSES")
pulse_rows = []
pulse_trace_rows = []

for case_idx, extra in enumerate(
    COLLECTIVE_EXTRA_RESIDUAL_PULSES,
    start=1,
):
    run = build_low_alt_trigger()
    ...
    physical_collective_values = []
    max_abs_pos = abs(initial["position_error_ft"])
    max_abs_cross = abs(initial["cross_track_ft"])
    ...
    for step in range(int(PULSE_SECONDS / dt)):
        ...
        state, used, total_residual, mapped_elev = (
            raw_near_ground_collective_offset_cycle(
                env2,
                fdm,
                action,
                base_collective_residual=ctrl["collective_residual"],
                extra_collective_residual=extra,
                hover_action0=hover_action0,
                lat0=lat0,
                lon0=lon0,
        ...
        t = (step + 1) * dt
        total_residual_values.append(total_residual)
        physical_collective_values.append(
            state["physical_collective_cmd"]
        )
        ...
        pulse_trace_rows.append({
            "case": int(case_idx),
            "extra_collective_residual": float(extra),
            "time_s": float(t),
            "base_collective_residual": float(
                ctrl["collective_residual"]
            ),
            "total_collective_residual": float(
                total_residual
            ),
            "physical_collective_cmd": float(
                state["physical_collective_cmd"]
        ...
        reason = diagnostic_safety_reason(state)
        if reason:
            safe = False
            term = reason
            break
    ...
    d_alt = final["altitude_ft"] - initial["altitude_ft"]
    d_vfwd = (
    ...
    row = {
        ...
        "trigger_altitude_ft": float(initial["altitude_ft"]),
        "trigger_vertical_speed_fps": float(
            initial["vertical_speed_fps"]
        ),
        "delta_vertical_speed_fps": float(d_vs),
        "delta_altitude_ft": float(d_alt),
        "delta_forward_speed_fps": float(d_vfwd),
        "delta_cross_track_ft": float(d_cross),
        "total_collective_residual_mean": float(
            np.mean(total_residual_values)
        ) if total_residual_values else float("nan"),
        "physical_collective_mean": float(
            np.mean(physical_collective_values)
        ) if physical_collective_values else float("nan"),
    ...
    print(
        f"Case {case_idx:02d} extra_dA0={extra:+.3f} | "
        f"SAFE={safe} TERM={term} | "
        f"dVS@2s={d_vs:+.4f} "
        f"dALT={d_alt:+.3f} | "
        f"dVfwd={d_vfwd:+.4f} dX={d_cross:+.3f} | "
        f"dA0mean={row['total_collective_residual_mean']:+.4f} "
        f"COLLmean={row['physical_collective_mean']:+.6f} | "
        f"MAXerr={max_abs_pos:.2f} XMAX={max_abs_cross:.2f}"
    )

    close_handoff(run)

safe_pulses = [r for r in pulse_rows if r["safe"]]

fit_slope = float("nan")
fit_intercept = float("nan")
fit_r2 = float("nan")

if len(safe_pulses) >= 3:
    x = np.asarray([
        r["extra_collective_residual"]
        for r in safe_pulses
    ], dtype=float)
    y = np.asarray([
        r["delta_vertical_speed_fps"]
        for r in safe_pulses
    ], dtype=float)

    fit_slope, fit_intercept = np.polyfit(x, y, 1)
    pred = fit_slope * x + fit_intercept
    ...
    print()
    print(
        f"FIT dVS(2s) ~= {fit_slope:+.6f} * extra_collective_residual "
        f"{fit_intercept:+.6f}   (R^2={fit_r2:.4f})"
    )


rule("D — SUSTAINED LOW-ALTITUDE COLLECTIVE CHECKS")
sustained_rows = []
sustained_trace_rows = []

for case_idx, extra in enumerate(
    COLLECTIVE_EXTRA_RESIDUAL_SUSTAINED,
    start=1,
):
    run = build_low_alt_trigger()
    ...
    for step in range(int(SUSTAINED_SECONDS / dt)):
        ...
        t = (step + 1) * dt
        residual_values.append(total_residual)
        collective_values.append(
            state["physical_collective_cmd"]
        )

        if t >= SUSTAINED_SECONDS - 2.0:
            vs_tail.append(state["vertical_speed_fps"])

        sustained_trace_rows.append({
            "case": int(case_idx),
            "extra_collective_residual": float(extra),
            "time_s": float(t),
            "base_collective_residual": float(
        ...
        reason = diagnostic_safety_reason(state)
        if reason and "corridor_exit" not in reason:
            safe = False
            term = reason
            break
    ...
    row = {
        ...
        "extra_collective_residual": float(extra),
        "safe": bool(safe),
        "corridor": bool(corridor),
        "termination": str(term),
        "final_altitude_ft": float(final["altitude_ft"]),
        "final_vertical_speed_fps": float(
            final["vertical_speed_fps"]
        ),
        "vs_mean_last2s": float(vs_mean2),
        ...
        ) if residual_values else float("nan"),
        "physical_collective_mean": float(
            np.mean(collective_values)
        ) if collective_values else float("nan"),
    }
    sustained_rows.append(row)

    print(
        f"Case {case_idx:02d} extra_dA0={extra:+.3f} | "
        f"SAFE={safe} CORRIDOR={corridor} TERM={term} | "
        f"ALT={final['altitude_ft']:.2f} "
        f"VS={final['vertical_speed_fps']:+.3f} "
        f"VSmean2={vs_mean2:+.3f} | "
        f"FWD={final['forward_ft']:.2f} "
        f"err={final['position_error_ft']:+.2f} "
        f"V={final['forward_speed_fps']:+.3f} | "
        f"X={final['cross_track_ft']:+.2f} "
        f"LAT={final['lateral_speed_fps']:+.3f} | "
        f"dA0mean={row['total_collective_residual_mean']:+.3f}"
    )

    close_handoff(run)


rule("E — LOW-ALTITUDE COLLECTIVE AUTHORITY CONCLUSION")
useful_negative_authority = bool(
    any(
        r["safe"]
        and r["extra_collective_residual"] < 0.0
        and r["delta_vertical_speed_fps"] <= -0.05
        for r in pulse_rows
    )
)

safe_sustained = [
    r for r in sustained_rows
    if r["safe"]
    and r["corridor"]
    and r["final_altitude_ft"] >= DIAG_MIN_AGL_FT
    and r["vs_mean_last2s"] < -0.05
]
...
if safe_sustained:
    preferred = min(
        safe_sustained,
        key=lambda r: (
            abs(r["vs_mean_last2s"] + 0.25),
            abs(r["final_position_error_ft"]),
            abs(r["final_cross_track_ft"]),
            abs(r["extra_collective_residual"]),
        ),
    )

ready = bool(
    useful_negative_authority
...
print(
    f"additional negative collective authority useful: "
    f"{useful_negative_authority}"
)
print(f"pulse-response R^2: {fit_r2:.4f}")
print(
    f"safe sustained candidate inside corridor: "
    f"{preferred is not None}"
)

if preferred is not None:
    print(
        "preferred sustained evidence case: "
        f"extra_dA0={preferred['extra_collective_residual']:+.3f}, "
        f"VSmean(last2s)={preferred['vs_mean_last2s']:+.3f}, "
        f"ALT={preferred['final_altitude_ft']:.2f}, "
        f"FWDerr={preferred['final_position_error_ft']:+.2f}, "
        f"X={preferred['final_cross_track_ft']:+.2f}, "
        f"dA0mean={preferred['total_collective_residual_mean']:+.3f}"
    )

print()
print(
    "READY FOR LOW-ALTITUDE VERTICAL BIAS / FEEDFORWARD "
    f"CALIBRATION: {ready}"
)

save_rows_csv(
    RESULT_DIR / "collective_authority_pulse_sweep.csv",
    pulse_rows,
)
save_rows_csv(
    RESULT_DIR / "collective_authority_pulse_traces.csv",
    pulse_trace_rows,
)
save_rows_csv(
    RESULT_DIR / "collective_sustained_sweep.csv",
    sustained_rows,
)
save_rows_csv(
    RESULT_DIR / "collective_sustained_traces.csv",
    sustained_trace_rows,
)
...
summary = {
    ...
    "operating_point_long_kpos": DIAG_LONG_KPOS,
    "operating_point_long_kv": DIAG_LONG_KV,
    "trigger_altitude_ft": TRIGGER_ALT_FT,
    "pulse_seconds": PULSE_SECONDS,
    "extra_residual_pulses": COLLECTIVE_EXTRA_RESIDUAL_PULSES,
    "sustained_seconds": SUSTAINED_SECONDS,
    "extra_residual_sustained": (
        COLLECTIVE_EXTRA_RESIDUAL_SUSTAINED
    ),
    "qualified_collective_residual_min": (
        COLLECTIVE_RESIDUAL_MIN
    ),
    "qualified_collective_residual_max": (
        COLLECTIVE_RESIDUAL_MAX
    ),
    "fit_slope": float(fit_slope),
    "fit_intercept": float(fit_intercept),
    "fit_r2": float(fit_r2),
    "additional_negative_collective_authority_useful": bool(
        useful_negative_authority
    ),
    "preferred_sustained_case": preferred,
    "ready_for_low_altitude_vertical_bias_feedforward_calibration": bool(
        ready
    ),
}

with open(
    RESULT_DIR / "diagnostic_summary.json",
    "w",
) as f:
...
print("Saved:")
print(
    f"  {RESULT_DIR / 'collective_authority_pulse_sweep.csv'}"
)
print(
    f"  {RESULT_DIR / 'collective_authority_pulse_traces.csv'}"
)
print(
    f"  {RESULT_DIR / 'collective_sustained_sweep.csv'}"
)
print(
    f"  {RESULT_DIR / 'collective_sustained_traces.csv'}"
)
print(
    f"  {RESULT_DIR / 'diagnostic_summary.json'}"
)
print()
print(
    "Do NOT train Stage 4 here. This script only measures the remaining "
    "low-altitude collective residual authority inside the already-qualified "
    "[-0.50,+0.15] envelope."
)
