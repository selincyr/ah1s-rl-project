...
"""
AH-1S / JSBSim
STAGE 4 VERTICAL-DESCENT TEACHER CALIBRATION V2
================================================

Purpose
-------
Retest the Stage-4 300 ft -> 30 ft descent using the longitudinal transition
that was actually qualified by the V3 long-horizon diagnostic.  This script
is still TEACHER CALIBRATION only: no Stage-4 PPO training, no distillation,
and no modification of the locked Stage-1/2/3 models.
...
E) During descent, longitudinal action[1] remains the qualified -1.0 maximum
   braking command.  Lateral station keeping uses the already-validated
   Stage-3 teacher gains; rudder remains at the mapped trim action.
F) Sweep only descent-rate / VS feedback parameters.  A candidate passes only
   if it reaches a stable 30 ft hover for >=5 s while the COMPLETE descent
   stays inside the 5 ft forward and cross-track presentation corridors.
...
RESULT_DIR = Path(
    "results_stage4_descent_teacher_v2"
)
...
DESCENT_VMAX_CANDIDATES = [1.00, 1.20]
VS_KP_CANDIDATES = [0.35, 0.45]
...
# an early 294 ft transfer.  Lateral gains are the locked Stage-3 teacher values.
...
# =====================================================================
# STAGE-4 LATERAL-IN-DESCENT DIAGNOSTIC V1
# =====================================================================
# This file intentionally reuses the already-qualified Stage1->2->3 handoff,
# early Stage-4 braking transfer, collective ID envelope, and the best
# non-passing vertical candidate from descent-teacher V2.  No Stage-4 PPO
# training is performed here.  The only purpose is to identify whether the
# lateral loss during descent is caused by insufficient action[2] authority / 
# clipping in the descending regime before changing lateral gains.

RESULT_DIR = Path("results_stage4_lateral_descent_diagnostic_v1")
RESULT_DIR.mkdir(parents=True, exist_ok=True)

NOMINAL_DESCENT_VMAX = 1.20
NOMINAL_VS_KP = 0.35
TRIGGER_CROSS_FT = -3.0
PULSE_SECONDS = 2.0
RECOVERY_SECONDS = 2.0
# V2 diagnostic had only qualified absolute Stage-3 action[2] up to about +0.17.
# We expand cautiously in small steps while preserving the generic [-1, +1]
# action-space bound and logging the *physical* aileron command.
A2_INCREMENT_CANDIDATES = [0.00, 0.08, 0.16, 0.24, 0.32]
A2_DIAGNOSTIC_MAX = 0.55


def _endpoint_prehold_live(start):
    """Reproduce the qualified early Stage-4 transfer and >=5 s endpoint hold."""
    env2 = start["env2"]
    fdm = start["fdm"]
    lat0 = start["lat0"]
    lon0 = start["lon0"]
    ...
    for step in range(int(40.0 / dt)):
        ...
        obs3 = stage3_observation(s0)
        base, _ = stage3_model.predict(obs3, deterministic=True)
        a = np.asarray(base, dtype=np.float32).reshape(-1).copy()
        a[1] = FIXED_BRAKE_A1
        s1, used = raw_policy_cycle(env2, fdm, a, lat0, lon0, mission_heading)

        now = endpoint_hover_now(s1)
        entered = entered or now
        pre_hold = pre_hold + dt if now else 0.0

        reason = stage3_safety_reason(s1)
        if reason:
            raise RuntimeError(f"Pre-descent hold became unsafe: {reason}")
        if entered:
            if abs(s1["position_error_ft"]) > PRESENTATION_MAX_POSITION_ERROR_FT:
                raise RuntimeError("Pre-descent hold left forward corridor.")
            if abs(s1["cross_track_ft"]) > PRESENTATION_MAX_CROSS_FT:
                raise RuntimeError("Pre-descent hold left cross-track corridor.")
        if pre_hold >= PRE_DESCENT_HOLD_SECONDS:
            break

    if pre_hold < PRE_DESCENT_HOLD_SECONDS:
        raise RuntimeError("Could not reproduce the qualified >=5 s Stage-4 pre-descent hold.")

    state = snapshot(fdm, lat0, lon0, mission_heading)
    obs3 = stage3_observation(state)
    hover_action, _ = stage3_model.predict(obs3, deterministic=True)
    hover_action = np.asarray(hover_action, dtype=np.float32).reshape(-1)
    return float(hover_action[0]), float(pre_hold)


def build_live_descent_trigger():
    """Build the full chain and descend nominally until cross-track reaches -3 ft."""
    start = build_stage4_handoff(
        detailed=False,
        require_full_hold=False,
        custom_entry_forward_ft=STAGE4_ENTRY_FORWARD_FT,
    ...
    hover_action0, pre_hold = _endpoint_prehold_live(start)
    trace = []
    max_steps = int(220.0 / dt)
    for step in range(max_steps):
        s0 = snapshot(fdm, lat0, lon0, mission_heading)
        action, ctrl = build_stage4_teacher_action(
            s0,
            hover_action0=hover_action0,
            descent_vmax=NOMINAL_DESCENT_VMAX,
            vs_kp=NOMINAL_VS_KP,
        )
        s1, used = raw_policy_cycle(env2, fdm, action, lat0, lon0, mission_heading)
        t = (step + 1) * dt
        trace.append({
            "phase": "nominal_descent_to_trigger",
            "time_s": float(t),
            "action0": float(used[0]),
            "action1": float(used[1]),
            "action2": float(used[2]),
            "action3": float(used[3]),
            "vs_des_fps": float(ctrl["vs_des_fps"]),
            "collective_residual": float(ctrl["collective_residual"]),
            "raw_action2": float(ctrl["raw_action2"]),
            **{k: float(v) for k, v in s1.items()},
        })

        reason = stage4_teacher_safety_reason(s1)
        if reason:
            close_handoff(start)
            raise RuntimeError(f"Nominal descent became unsafe before lateral trigger: {reason}")
        if abs(s1["position_error_ft"]) > PRESENTATION_MAX_POSITION_ERROR_FT:
            close_handoff(start)
            raise RuntimeError("Nominal descent left forward corridor before lateral trigger.")
        if s1["cross_track_ft"] <= TRIGGER_CROSS_FT:
            # Recompute the nominal command at the exact trigger state for the pulse baseline.
            nominal_action, nominal_ctrl = build_stage4_teacher_action(
                s1,
                hover_action0=hover_action0,
                descent_vmax=NOMINAL_DESCENT_VMAX,
                vs_kp=NOMINAL_VS_KP,
    ...
    close_handoff(start)
    raise RuntimeError("Did not reach the lateral diagnostic trigger within the nominal descent horizon.")


def pulse_safety_reason(state):
    if state["altitude_ft"] < 150.0 or state["altitude_ft"] > 310.0:
        return "altitude_diagnostic_limit"
    if abs(state["position_error_ft"]) > 6.0:
        return "forward_diagnostic_limit"
    if abs(state["cross_track_ft"]) > 7.5:
        return "cross_diagnostic_limit"
    if abs(math.degrees(state["roll_rad"])) > 10.0:
        return "roll_diagnostic_limit"
    if abs(math.degrees(state["pitch_rad"])) > 10.0:
        return "pitch_diagnostic_limit"
    if abs(state["heading_error_deg"]) > 3.0:
        return "heading_diagnostic_limit"
    return ""


def run_a2_pulse_case(case_index: int, increment: float):
    live = build_live_descent_trigger()
    start = live["start"]
    env2 = start["env2"]
    fdm = start["fdm"]
    lat0 = start["lat0"]
    ...
    mission_heading = start["mission_heading"]
    dt = env_control_dt(env2)
    hover_action0 = live["hover_action0"]
    trigger = live["state"].copy()
    nominal_a2 = float(live["nominal_action"][2])
    pulse_a2 = float(np.clip(nominal_a2 + increment, -1.0, A2_DIAGNOSTIC_MAX))

    initial_lat = float(trigger["lateral_speed_fps"])
    initial_gcross = float(trigger["mission_ground_cross_speed_fps"])
    initial_cross = float(trigger["cross_track_ft"])
    initial_phys_ail = float(trigger["physical_aileron_cmd"])

    trace = []
    safe = True
    reason = "pulse_complete"
    pulse_end = trigger.copy()
    max_abs_roll = abs(math.degrees(trigger["roll_rad"]))
    max_abs_heading = abs(trigger["heading_error_deg"])
    max_abs_cross = abs(trigger["cross_track_ft"])
    phys_ail_values = []
    ...
    for step in range(int(PULSE_SECONDS / dt)):
        ...
        s1, used = raw_policy_cycle(env2, fdm, action, lat0, lon0, mission_heading)
        pulse_end = s1
        phys_ail_values.append(s1["physical_aileron_cmd"])
        max_abs_roll = max(max_abs_roll, abs(math.degrees(s1["roll_rad"])))
        max_abs_heading = max(max_abs_heading, abs(s1["heading_error_deg"]))
        max_abs_cross = max(max_abs_cross, abs(s1["cross_track_ft"]))
        trace.append({
            "case_index": int(case_index),
            "phase": "pulse",
            "time_s": float((step + 1) * dt),
            "increment": float(increment),
            "nominal_a2_at_trigger": float(nominal_a2),
            "pulse_a2": float(pulse_a2),
            "action0": float(used[0]),
            "action1": float(used[1]),
            "action2": float(used[2]),
            "action3": float(used[3]),
        ...
        r = pulse_safety_reason(s1)
        if r:
            safe = False
            reason = r
            break

    # Short nominal recovery is diagnostic only; it verifies that the pulse did
    # not place the aircraft into an obviously unrecoverable local state.
    if safe:
    ...
    final_state = snapshot(fdm, lat0, lon0, mission_heading)
    mean_phys_ail = float(np.mean(phys_ail_values)) if phys_ail_values else float("nan")
    result = {
        "case_index": int(case_index),
        "increment": float(increment),
        "safe": bool(safe),
        "termination_reason": str(reason),
        "trigger_altitude_ft": float(trigger["altitude_ft"]),
        "trigger_forward_ft": float(trigger["forward_ft"]),
        "trigger_cross_ft": float(trigger["cross_track_ft"]),
        "trigger_lateral_speed_fps": float(initial_lat),
        "trigger_ground_cross_speed_fps": float(initial_gcross),
        "nominal_a2_at_trigger": float(nominal_a2),
        "pulse_a2": float(pulse_a2),
        "initial_physical_aileron_cmd": float(initial_phys_ail),
        "mean_pulse_physical_aileron_cmd": float(mean_phys_ail),
        "physical_aileron_delta_mean": float(mean_phys_ail - initial_phys_ail) if np.isfinite(mean_phys_ail) else float("nan"),
        "pulse_end_cross_ft": float(pulse_end["cross_track_ft"]),
        "pulse_end_lateral_speed_fps": float(pulse_end["lateral_speed_fps"]),
        "pulse_end_ground_cross_speed_fps": float(pulse_end["mission_ground_cross_speed_fps"]),
        "delta_lateral_speed_fps": float(pulse_end["lateral_speed_fps"] - initial_lat),
        "delta_ground_cross_speed_fps": float(pulse_end["mission_ground_cross_speed_fps"] - initial_gcross),
        "delta_cross_ft": float(pulse_end["cross_track_ft"] - initial_cross),
        "max_abs_cross_ft": float(max_abs_cross),
        "max_abs_roll_deg": float(max_abs_roll),
        "max_abs_heading_error_deg": float(max_abs_heading),
        "recovery_final_cross_ft": float(final_state["cross_track_ft"]),
        "recovery_final_lateral_speed_fps": float(final_state["lateral_speed_fps"]),
        "recovery_final_altitude_ft": float(final_state["altitude_ft"]),
    }
    close_handoff(start)
    return result, trace
...
if not qualification["handoff_pass"]:
    close_handoff(qualification)
    raise RuntimeError("Locked Stage-3 qualification failed; do not diagnose Stage 4.")
close_handoff(qualification)

rule("B — REPLAY BEST V2 DESCENT AND AUDIT LATERAL COMMAND")
print(
    f"Nominal diagnostic descent: Vmax={NOMINAL_DESCENT_VMAX:.2f}, "
    f"VS_Kp={NOMINAL_VS_KP:.2f}. Existing lateral law/bounds are unchanged."
)
baseline_result, baseline_trace = run_teacher_case(
    descent_vmax=NOMINAL_DESCENT_VMAX,
    vs_kp=NOMINAL_VS_KP,
    candidate_index=1,
    detailed=False,
)
write_csv(RESULT_DIR / "baseline_descent_trace.csv", baseline_trace)
descent_rows = [r for r in baseline_trace if r.get("phase") == "descent"]
a2_vals = [float(r["action2"]) for r in descent_rows]
upper_fraction = float(np.mean(np.asarray(a2_vals) >= IDENTIFIED_A2_MAX - 1e-4)) if a2_vals else 0.0
first_x3 = next((r for r in descent_rows if float(r["cross_track_ft"]) <= TRIGGER_CROSS_FT), None)
print(
    f"BASELINE | PASS={baseline_result['teacher_pass']} SAFE={baseline_result['safe']} "
    f"TERM={baseline_result['termination_reason']} | t={baseline_result['duration_s']:.1f}s | "
    f"ALT={baseline_result['final_altitude_ft']:.2f} | X={baseline_result['final_cross_track_ft']:+.2f} "
    f"LAT={baseline_result['final_lateral_speed_fps']:+.3f} | "
    f"A2range=[{baseline_result['action2_min']:+.4f},{baseline_result['action2_max']:+.4f}] | "
    f"A2@old_upper_fraction={100.0*upper_fraction:.1f}%"
)
if first_x3 is not None:
    print(
        "FIRST X<=-3.0 | "
        f"t={first_x3['time_s']:.2f}s ALT={first_x3['altitude_ft']:.2f} "
        f"FWD={first_x3['forward_ft']:.2f} V={first_x3['forward_speed_fps']:+.3f} "
        f"X={first_x3['cross_track_ft']:+.2f} LAT={first_x3['lateral_speed_fps']:+.3f} "
        f"GCROSS={first_x3['mission_ground_cross_speed_fps']:+.3f} "
        f"A2={first_x3['action2']:+.4f} AIL={first_x3['physical_aileron_cmd']:+.5f} "
        f"ROLL={math.degrees(first_x3['roll_rad']):+.3f}deg HDG={first_x3['heading_error_deg']:+.3f}deg"
    )

rule("C — DESCENDING-REGIME AILERON AUTHORITY PULSES")
print(
    "Every case rebuilds the full locked Stage1->2->3 chain, performs the qualified "
    "294-ft Stage-4 transfer and >=5 s endpoint hold, then follows the same nominal "
    "descent until X<=-3 ft. Only action[2] is changed for a 2 s pulse."
)
rows = []
all_traces = []
for i, inc in enumerate(A2_INCREMENT_CANDIDATES, start=1):
    result, trace = run_a2_pulse_case(i, inc)
    rows.append(result)
    all_traces.extend(trace)
    print(
        f"Case {i:02d} dA2={inc:+.2f} | A2={result['pulse_a2']:+.4f} | "
        f"SAFE={result['safe']} TERM={result['termination_reason']} | "
        f"trigger ALT={result['trigger_altitude_ft']:.1f} X={result['trigger_cross_ft']:+.2f} "
        f"LAT={result['trigger_lateral_speed_fps']:+.3f} | "
        f"dLAT={result['delta_lateral_speed_fps']:+.3f} "
        f"dGCROSS={result['delta_ground_cross_speed_fps']:+.3f} "
        f"dX={result['delta_cross_ft']:+.3f} | "
        f"AILd={result['physical_aileron_delta_mean']:+.5f} | "
        f"rollMax={result['max_abs_roll_deg']:.2f}deg hdgMax={result['max_abs_heading_error_deg']:.2f}deg"
    )

write_csv(RESULT_DIR / "aileron_authority_sweep.csv", rows)
write_csv(RESULT_DIR / "aileron_authority_traces.csv", all_traces)

safe_rows = [r for r in rows if r["safe"]]
fit = {}
if len(safe_rows) >= 3:
    xs = np.asarray([r["pulse_a2"] for r in safe_rows], dtype=float)
    ys = np.asarray([r["delta_lateral_speed_fps"] for r in safe_rows], dtype=float)
    coef = np.polyfit(xs, ys, 1)
    pred = np.polyval(coef, xs)
    ss_res = float(np.sum((ys - pred) ** 2))
    ss_tot = float(np.sum((ys - np.mean(ys)) ** 2))
    r2 = 1.0 - ss_res / ss_tot if ss_tot > 1e-12 else 1.0
    fit = {"slope": float(coef[0]), "intercept": float(coef[1]), "r2": float(r2)}
    print(
        f"\nFIT dLAT(2s) ~= {coef[0]:+.6f} * A2 {coef[1]:+.6f}   (R^2={r2:.4f})"
    )

# Evidence gate: do not claim a new lateral bound merely because the largest
# pulse is safe.  Require a safe pulse above the old +0.17 bound that materially
# improves lateral velocity in the needed positive direction.
useful_expanded = [
    r for r in safe_rows
    if r["pulse_a2"] > IDENTIFIED_A2_MAX + 1e-3
    and r["delta_lateral_speed_fps"] > 0.08
]
ready = bool(useful_expanded)
selected = min(useful_expanded, key=lambda r: r["pulse_a2"]) if useful_expanded else None

rule("D — STAGE-4 LATERAL-DESCENT DIAGNOSTIC CONCLUSION")
print(f"Old qualified A2 upper bound: {IDENTIFIED_A2_MAX:+.3f}")
print(f"Existing descent baseline A2 max: {baseline_result['action2_max']:+.4f}")
print(f"Expanded authority useful above old bound: {ready}")
if selected is not None:
    print(
        "Mildest useful expanded pulse: "
        f"A2={selected['pulse_a2']:+.4f}, dLAT={selected['delta_lateral_speed_fps']:+.3f}, "
        f"dGCROSS={selected['delta_ground_cross_speed_fps']:+.3f}, "
        f"rollMax={selected['max_abs_roll_deg']:.2f}deg, "
        f"hdgMax={selected['max_abs_heading_error_deg']:.2f}deg"
    )

summary = {
    "ready_for_lateral_teacher_recalibration": bool(ready),
    "training_performed": False,
    "vertical_retuned": False,
    "nominal_vertical_candidate": {
        "descent_vmax_fps": NOMINAL_DESCENT_VMAX,
        "vs_kp": NOMINAL_VS_KP,
    },
    "baseline": baseline_result,
    "old_identified_action2_max": IDENTIFIED_A2_MAX,
    "old_upper_bound_fraction": upper_fraction,
    "trigger_cross_ft": TRIGGER_CROSS_FT,
    "pulse_seconds": PULSE_SECONDS,
    "pulse_results": rows,
    "fit": fit,
    "selected_mildest_useful_expanded_pulse": selected,
    "git_head": git_head(),
    "sha256": {
...
print("\nREADY FOR LATERAL TEACHER RECALIBRATION:", ready)
print("Saved:")
print(" ", RESULT_DIR / "baseline_descent_trace.csv")
print(" ", RESULT_DIR / "aileron_authority_sweep.csv")
print(" ", RESULT_DIR / "aileron_authority_traces.csv")
print(" ", RESULT_DIR / "diagnostic_summary.json")
print("\nDo NOT train Stage 4 here. Do NOT change vertical gains from this script.")
print("Next step depends on whether expanded positive A2 authority is both safe and effective during descent.")
