...
RESULT_DIR = Path(
    "results_stage4_ground_effect_collective_curve_v1"
)
...
rule("B — REPRODUCE NATIVE A0=-1 LOW-ALT EQUILIBRIUM")
...
state0 = run["trigger_state"]
print(
    "START | "
    f"ALT={state0['altitude_ft']:.3f} "
    f"VS={state0['vertical_speed_fps']:+.4f} | "
    f"FWD={state0['forward_ft']:.3f} "
    f"err={state0['position_error_ft']:+.3f} "
    f"V={state0['forward_speed_fps']:+.4f} | "
    f"X={state0['cross_track_ft']:+.3f} "
    f"LAT={state0['lateral_speed_fps']:+.4f} | "
    f"COLL={run['baseline_physical_collective']:+.6f}"
)


rule("C — STEPWISE GROUND-EFFECT EQUILIBRIUM CONTINUATION")
rows = []
trace_rows = []

overall_safe = True
termination = "curve_complete"

for level_idx, physical_collective in enumerate(
    CURVE_COLLECTIVE_LEVELS,
    start=1,
):
    settle_hold = 0.0
    ...
    print(
        f"Level {level_idx:02d}/{len(CURVE_COLLECTIVE_LEVELS)} | "
        f"COLL={physical_collective:+.6f}"
    )

    for step in range(int(CURVE_LEVEL_MAX_TIME_S / dt)):
        state_before = snapshot(fdm, lat0, lon0, mission_heading)

        # Horizontal/lateral controllers are unchanged.  Vertical controller is
        # evaluated only to preserve the same action vector; physical collective
        # is overwritten to the diagnostic continuation level.
        action, ctrl, vertical_bias = build_near_ground_action_low_alt_bias(
            state_before,
            target_alt_ft=LOW_ALT_TARGET_FT,
            hover_action0=hover_action0,
        ...
        if t >= max(0.0, CURVE_LEVEL_MAX_TIME_S - 5.0):
            vs_tail.append(state["vertical_speed_fps"])
            alt_tail.append(state["altitude_ft"])

        if abs(state["vertical_speed_fps"]) <= CURVE_SETTLE_VS_ABS_FPS:
            settle_hold += dt
        else:
            settle_hold = 0.0
        ...
        reason = curve_safety_reason(state)
        if reason:
            overall_safe = False
            level_term = reason
            termination = reason
        ...
        if any_wow(contact_snapshot(fdm)):
            overall_safe = False
            level_term = "unexpected_wow"
            termination = level_term
            break

        if settle_hold >= CURVE_SETTLE_SECONDS:
            level_term = "equilibrium_settled_5s"
            break
    ...
    tail_vs_mean = (
        ...
        if vs_tail else float("nan")
    )
    tail_alt_mean = (
        float(np.mean(alt_tail))
        if alt_tail else float("nan")
    )

    settled = bool(
        overall_safe
        and level_term == "equilibrium_settled_5s"
    )

    row = {
        "level_index": int(level_idx),
        "physical_collective": float(physical_collective),
        "safe": bool(overall_safe),
        ...
        "max_abs_cross_track_ft": float(max_abs_cross),
        "tail_vs_mean_fps": float(tail_vs_mean),
        "tail_altitude_mean_ft": float(tail_alt_mean),
    }
    rows.append(row)

    print(
        f"  SETTLED={settled} SAFE={overall_safe} "
        f"TERM={level_term} | "
        ...
        f"minALT={min_alt:.2f} minVS={min_vs:+.3f}"
    )
...
rule("D — GROUND-EFFECT COLLECTIVE CURVE CONCLUSION")

settled_rows = [
    r for r in rows
    if r["safe"] and r["settled"]
]

monotonic = False
if len(settled_rows) >= 3:
    # As physical collective decreases, equilibrium altitude should also
    # decrease. We allow tiny numerical tolerance.
    alts = [r["final_altitude_ft"] for r in settled_rows]
    colls = [r["physical_collective"] for r in settled_rows]
    monotonic = all(
        alts[i + 1] <= alts[i] + 0.10
...
if len(settled_rows) >= 2:
    altitude_span = (
        settled_rows[0]["final_altitude_ft"]
        - settled_rows[-1]["final_altitude_ft"]
    )
...
ready_schedule = bool(
    overall_safe
    and len(settled_rows) >= 4
    and monotonic
    and altitude_span >= 2.5
    and np.isfinite(lowest_settled_alt)
    and lowest_settled_alt <= 13.0
)

needs_more_curve = bool(
    overall_safe
    and len(settled_rows) >= 3
    and monotonic
    and not ready_schedule
)

print(f"settled curve points: {len(settled_rows)}")
print(f"monotonic lower-collective/lower-altitude relation: {monotonic}")
print(f"measured equilibrium altitude span: {altitude_span:.3f} ft")
print(f"lowest settled equilibrium altitude: {lowest_settled_alt:.3f} ft")
print()
print(
    "READY FOR ALTITUDE-SCHEDULED LOW-ALT COLLECTIVE FEEDFORWARD "
    f"CALIBRATION: {ready_schedule}"
)
print(
    "NEEDS FURTHER SAFE EQUILIBRIUM-CURVE EXTENSION: "
    f"{needs_more_curve}"
)

save_rows_csv(
    RESULT_DIR / "ground_effect_collective_curve.csv",
    rows,
)
save_rows_csv(
    RESULT_DIR / "ground_effect_collective_curve_traces.csv",
    trace_rows,
)
...
summary = {
    ...
    "curve_collective_levels": CURVE_COLLECTIVE_LEVELS,
    "curve_level_max_time_s": CURVE_LEVEL_MAX_TIME_S,
    "curve_settle_vs_abs_fps": CURVE_SETTLE_VS_ABS_FPS,
    "curve_settle_seconds": CURVE_SETTLE_SECONDS,
    "overall_safe": bool(overall_safe),
    "termination": str(termination),
    "settled_points": settled_rows,
    "monotonic": bool(monotonic),
    "altitude_span_ft": float(altitude_span),
    "lowest_settled_altitude_ft": float(lowest_settled_alt),
    "ready_for_altitude_scheduled_low_alt_collective_feedforward_calibration": bool(
        ready_schedule
    ),
    "needs_further_safe_equilibrium_curve_extension": bool(
        needs_more_curve
    ),
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'ground_effect_collective_curve.csv'}")
print(f"  {RESULT_DIR / 'ground_effect_collective_curve_traces.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "This continuation test maps the near-ground equilibrium curve so the "
    "next controller can use altitude-scheduled feedforward instead of a "
    "single fixed collective offset."
)
