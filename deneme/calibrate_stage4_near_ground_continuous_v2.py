...
RESULT_DIR = Path(
    "results_stage4_near_ground_continuous_v2"
)
...
if not audit["handoff_pass"]:
    close_handoff(audit)
    raise RuntimeError(
        "Locked Stage1->2->3 qualification failed."
    )
close_handoff(audit)


rule("B — CONTINUOUS NEAR-GROUND DESCENT SWEEP (30 FT -> 9 FT)")
print(
    "Intermediate 25/20/15/12/10-ft values are diagnostic crossings only; "
    "there are NO artificial hover requirements at those altitudes."
)
print(
    "Near-ground action1 mapping: "
    f"-1 -> {NEAR_ELEVATOR_PHYSICAL_MIN:+.6f}, "
    f"+1 -> {NEAR_ELEVATOR_PHYSICAL_MAX:+.6f}"
)
print(
    f"Vertical target={CONTINUOUS_TARGET_ALT_FT:.1f} ft | "
    f"max descent={NEAR_GROUND_VMAX_FPS:.2f} ft/s | "
    f"final hold={CONTINUOUS_SETTLE_SECONDS:.1f}s"
)
...
case_idx = 0
total_candidates = (
    len(NEAR_LONG_KPOS_GRID)
    * len(NEAR_LONG_KV_GRID)
)

for kpos in NEAR_LONG_KPOS_GRID:
    for kv in NEAR_LONG_KV_GRID:
        case_idx += 1
        print(
            f"Candidate {case_idx:2d}/{total_candidates} | "
            f"Kpos={kpos:.4f} | Kv={kv:.3f}"
        )

        result, trace = run_continuous_near_ground_candidate(
            long_kpos_phys=kpos,
            long_kv_phys=kv,
            detailed=False,
        )
        result["case"] = int(case_idx)
        rows.append(result)

        for tr in trace:
            tr["case"] = int(case_idx)
            all_traces.append(tr)

        crossed = ",".join(sorted(
            result["crossings"].keys(),
        ...
        print(
            ...
            f"ALT={result['final_altitude_ft']:.2f} "
            ...
            f"VB={result['vertical_bias_final']:+.3f} "
            f"dA0=[{result['collective_residual_min']:+.3f},"
            f"{result['collective_residual_max']:+.3f}] | "
            f"satC={100*result['collective_low_saturation_fraction']:.1f}% "
            f"satB={100*result['brake_saturation_fraction']:.1f}% "
            f"satR={100*result['release_saturation_fraction']:.1f}% | "
            f"crossings={crossed or 'none'}"
        )


rule("C — CONTINUOUS NEAR-GROUND SELECTION")
ranked = sorted(rows, key=continuous_sort_key)

for rank, row in enumerate(ranked, start=1):
    print(
        f"#{rank} C{row['case']:02d} | "
        f"PASS={row['pass']} SAFE={row['safe']} | "
        f"Kpos={row['long_kpos_phys']:.4f} "
        f"Kv={row['long_kv_phys']:.3f} | "
        f"ALT={row['final_altitude_ft']:.2f} "
        f"VS={row['final_vertical_speed_fps']:+.3f} | "
        f"MAXerr={row['max_abs_position_error_ft']:.2f} "
        f"XMAX={row['max_abs_cross_track_ft']:.2f} | "
        f"finalErr={row['final_position_error_ft']:+.2f} "
        f"V={row['final_forward_speed_fps']:+.3f} | "
        f"VB={row['vertical_bias_final']:+.3f} "
        f"satC={100*row['collective_low_saturation_fraction']:.1f}%"
    )
...
if passing:
    ...
    print()
    print(
        f"SELECTED C{selected['case']:02d} | "
        f"Kpos={selected['long_kpos_phys']:.4f} "
        f"Kv={selected['long_kv_phys']:.3f}"
    )

    rule("D — FRESH DETAILED REPEAT")
    fresh_result, fresh_trace = (
        run_continuous_near_ground_candidate(
            long_kpos_phys=selected["long_kpos_phys"],
            long_kv_phys=selected["long_kv_phys"],
            detailed=True,
        )
    )
    ...
    print(
        "FRESH RESULT | "
        f"PASS={fresh_result['pass']} "
        f"SAFE={fresh_result['safe']} "
        f"TERM={fresh_result['termination']} | "
        f"ALT={fresh_result['final_altitude_ft']:.3f} "
        f"VS={fresh_result['final_vertical_speed_fps']:+.3f} | "
        f"FWD={fresh_result['final_forward_ft']:.3f} "
        f"err={fresh_result['final_position_error_ft']:+.3f} "
        f"V={fresh_result['final_forward_speed_fps']:+.3f} | "
        f"X={fresh_result['final_cross_track_ft']:+.3f} "
        f"LAT={fresh_result['final_lateral_speed_fps']:+.3f} | "
        f"MAXerr={fresh_result['max_abs_position_error_ft']:.3f} "
        f"XMAX={fresh_result['max_abs_cross_track_ft']:.3f} | "
        f"HOLD={fresh_result['settle_hold_s']:.2f}s"
    )
else:
    print()
    print("NO PASSING CONTINUOUS 30->9 FT CANDIDATE.")


rule("E — CONTINUOUS NEAR-GROUND V2 RESULT")
ready_touchdown = bool(
    fresh_result is not None
    and fresh_result["pass"]
    and fresh_result["safe"]
)

# If no 9-ft pass, identify whether the remaining blocker is clearly vertical
# rather than horizontal. This helps choose the next diagnostic without
# changing any controller blindly.
best = ranked[0]
horizontal_ok_best = bool(
    best["safe"]
    and best["max_abs_position_error_ft"]
        <= PRESENTATION_MAX_POSITION_ERROR_FT
    and best["max_abs_cross_track_ft"]
        <= PRESENTATION_MAX_CROSS_FT
)
ready_vertical_id = bool(
    not ready_touchdown
    and horizontal_ok_best
    and best["final_altitude_ft"] > (
        CONTINUOUS_TARGET_ALT_FT
        + CONTINUOUS_SETTLE_ALT_TOL_FT
    )
)

print(
    f"READY FOR TOUCHDOWN IDENTIFICATION: {ready_touchdown}"
)
print(
    f"READY FOR LOW-ALT COLLECTIVE AUTHORITY DIAGNOSTIC: "
    f"{ready_vertical_id}"
)
print(
    "Best state | "
    f"ALT={best['final_altitude_ft']:.3f} "
    f"VS={best['final_vertical_speed_fps']:+.3f} | "
    f"FWDerr={best['final_position_error_ft']:+.3f} "
    f"V={best['final_forward_speed_fps']:+.3f} | "
    f"X={best['final_cross_track_ft']:+.3f} "
    f"LAT={best['final_lateral_speed_fps']:+.3f} | "
    f"VB={best['vertical_bias_final']:+.3f} "
    f"dA0min={best['collective_residual_min']:+.3f} "
    f"satC={100*best['collective_low_saturation_fraction']:.1f}%"
)

def save_rows_csv(path, data_rows):
    if not data_rows:
        return
    keys = []
    seen = set()
...
save_rows_csv(
    RESULT_DIR / "continuous_near_ground_sweep.csv",
    [
        {k: v for k, v in r.items() if k != "crossings"}
        for r in rows
    ],
)
save_rows_csv(
    RESULT_DIR / "continuous_near_ground_traces.csv",
    all_traces,
)

if fresh_result is not None:
    save_rows_csv(
        RESULT_DIR / "best_continuous_near_ground_trace.csv",
        fresh_trace,
    )

summary = {
    "stage4_training": "NONE",
    "intermediate_hover_requirements": False,
    "target_altitude_ft": CONTINUOUS_TARGET_ALT_FT,
    "settle_altitude_tolerance_ft": (
        CONTINUOUS_SETTLE_ALT_TOL_FT
    ),
    "settle_vertical_speed_tolerance_fps": (
        CONTINUOUS_SETTLE_VS_TOL_FPS
    ),
    "settle_seconds": CONTINUOUS_SETTLE_SECONDS,
    "near_ground_mapping_physical_min": (
        NEAR_ELEVATOR_PHYSICAL_MIN
    ),
    "near_ground_mapping_physical_max": (
        NEAR_ELEVATOR_PHYSICAL_MAX
    ),
    "rows": rows,
    "fresh_result": fresh_result,
    "ready_for_touchdown_identification": (
        ready_touchdown
    ),
    "ready_for_low_alt_collective_authority_diagnostic": (
        ready_vertical_id
    ),
}
...
print("Saved:")
print(
    f"  {RESULT_DIR / 'continuous_near_ground_sweep.csv'}"
)
print(
    f"  {RESULT_DIR / 'continuous_near_ground_traces.csv'}"
)
if fresh_result is not None:
    print(
        f"  {RESULT_DIR / 'best_continuous_near_ground_trace.csv'}"
    )
print(
...
print(
    ...
    "This test removes the artificial intermediate-hover constraint and "
    "evaluates the actual continuous 30->9 ft mission segment."
)
