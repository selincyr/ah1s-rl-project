...
RESULT_DIR = Path(
    "results_stage4_touchdown_identification_v1"
)
...
curve_points = load_confirmed_curve_points()
local_slope, local_intercept, local_r2 = (
    fit_local_lower_curve_slope(curve_points, n=5)
)
...
rule("B — REPRODUCE SELECTED 9-FT CAPTURE")
capture_check = build_selected_9ft_capture_open(
    curve_points,
    local_slope,
    detailed=False,
)
cap = capture_check["state"]

print(
    "9FT START | "
    f"ALT={cap['altitude_ft']:.3f} "
    f"VS={cap['vertical_speed_fps']:+.3f} | "
    f"FWD={cap['forward_ft']:.3f} "
    f"err={cap['position_error_ft']:+.3f} "
    f"V={cap['forward_speed_fps']:+.3f} | "
    f"X={cap['cross_track_ft']:+.3f} "
    f"LAT={cap['lateral_speed_fps']:+.3f} | "
    f"HOLD={capture_check['capture_hold_s']:.2f}s | "
    f"WOW={int(any_wow(contact_snapshot(capture_check['fdm'])))}"
)

close_handoff(capture_check)


rule("C — VERY-SLOW TOUCHDOWN / WOW IDENTIFICATION SWEEP")
rows = []
all_traces = []

for case_idx, vs_des in enumerate(
    TD_VS_DES_CANDIDATES,
    start=1,
):
    print(
        f"Candidate {case_idx}/{len(TD_VS_DES_CANDIDATES)} | "
        f"VSdes={vs_des:+.2f} ft/s"
    )

    result, trace = run_touchdown_candidate(
        vs_des_touchdown=vs_des,
        points=curve_points,
    ...
    if fc is None:
        ...
        contact_text = (
            f"CONTACT@AGL={fc['altitude_ft']:.3f} "
            f"VS={fc['vertical_speed_fps']:+.3f} "
            f"FWDerr={fc['position_error_ft']:+.2f} "
            f"X={fc['cross_track_ft']:+.2f} "
            f"COLL={fc['physical_collective_cmd']:.6f} "
            f"COMP={fc['max_compression_ft']:.4f}"
        )
    ...
    print(
        ...
        f"{contact_text} | "
        f"stableLanded={result['stable_landed']} "
        f"hold={result['landed_hold_s']:.2f}s | "
        f"finalALT={result['final_altitude_ft']:.3f} "
        f"finalVS={result['final_vertical_speed_fps']:+.3f} "
        f"finalWOW={result['final_wow']} | "
        f"MAXerr={result['max_abs_position_error_ft']:.2f} "
        f"XMAX={result['max_abs_cross_track_ft']:.2f}"
    )


rule("D — TOUCHDOWN IDENTIFICATION SELECTION")
ranked = sorted(rows, key=touchdown_rank_key)

for rank, row in enumerate(ranked, start=1):
    fc = row.get("first_contact") or {}
    print(
        f"#{rank} C{row['case']:02d} | "
        f"VSdes={row['commanded_touchdown_vs_fps']:+.2f} | "
        f"SAFE={row['safe']} "
        f"CONTACT={row['contact_detected']} "
        f"LANDED={row['stable_landed']} | "
        f"contactAGL={float(fc.get('altitude_ft', float('nan'))):.3f} "
        f"contactVS={float(fc.get('vertical_speed_fps', float('nan'))):+.3f} | "
...
usable = [
    r for r in ranked
    if r["safe"]
    and r["contact_detected"]
    and r["first_contact_vs_ok"]
]
...
if preferred is not None:
    print()
    print(
        f"SELECTED C{preferred['case']:02d} | "
        f"VSdes={preferred['commanded_touchdown_vs_fps']:+.2f}"
    )

    rule("E — FRESH DETAILED TOUCHDOWN REPEAT")
    fresh_result, fresh_trace = run_touchdown_candidate(
        vs_des_touchdown=preferred[
            "commanded_touchdown_vs_fps"
        ],
        points=curve_points,
        local_slope=local_slope,
        detailed=True,
    ...
    fc = fresh_result.get("first_contact")
    print()
    if fc is not None:
        print(
            "FIRST CONTACT | "
            ...
            f"VS={fc['vertical_speed_fps']:+.3f} | "
            f"FWDerr={fc['position_error_ft']:+.3f} "
            f"V={fc['forward_speed_fps']:+.3f} | "
            f"X={fc['cross_track_ft']:+.3f} "
            f"LAT={fc['lateral_speed_fps']:+.3f} | "
            f"PITCH={fc['pitch_deg']:+.2f}deg "
            f"ROLL={fc['roll_deg']:+.2f}deg | "
            f"COLL={fc['physical_collective_cmd']:.6f} "
            f"COMP={fc['max_compression_ft']:.4f}"
        )
    ...
    print(
        ...
        f"CONTACT={fresh_result['contact_detected']} "
        f"STABLE_LANDED={fresh_result['stable_landed']} | "
        f"finalALT={fresh_result['final_altitude_ft']:.3f} "
        f"finalVS={fresh_result['final_vertical_speed_fps']:+.3f} | "
        f"FWDerr={fresh_result['final_position_error_ft']:+.3f} "
        f"X={fresh_result['final_cross_track_ft']:+.3f} | "
        f"WOW={fresh_result['final_wow']} "
        f"COMP={fresh_result['final_max_compression_ft']:.4f} "
        f"HOLD={fresh_result['landed_hold_s']:.2f}s"
    )
else:
    print()
    print("NO SAFE TOUCHDOWN CONTACT CANDIDATE FOUND.")


rule("F — TOUCHDOWN / WOW IDENTIFICATION V1 CONCLUSION")
ready_teacher = bool(
    fresh_result is not None
    and fresh_result["safe"]
    and fresh_result["contact_detected"]
    and fresh_result["first_contact_vs_ok"]
)

stable_landed_ready = bool(
    ready_teacher
    and fresh_result["stable_landed"]
)

print(
    "READY FOR FINAL TOUCHDOWN CONTROLLER / TEACHER CALIBRATION: "
    f"{ready_teacher}"
)
print(
    "STABLE LANDED HOLD ALREADY ACHIEVED IN IDENTIFICATION: "
    f"{stable_landed_ready}"
)

save_rows_csv(
    RESULT_DIR / "touchdown_sweep.csv",
    [
        {
            k: v
            for k, v in r.items()
...
save_rows_csv(
    RESULT_DIR / "touchdown_traces.csv",
    all_traces,
)

if fresh_result is not None:
    save_rows_csv(
        RESULT_DIR / "best_touchdown_trace.csv",
        fresh_trace,
    )
...
summary = {
    ...
    "selected_9ft_capture_slope_multiplier": (
        TD_SLOPE_MULTIPLIER
    ),
    "selected_9ft_capture_kvs": TD_CAPTURE_KVS,
    "touchdown_vs_candidates_fps": (
        TD_VS_DES_CANDIDATES
    ),
    "curve_local_lower_slope": float(local_slope),
    "curve_local_lower_r2": float(local_r2),
    "rows": rows,
    "preferred_case": preferred,
    "fresh_result": fresh_result,
    "ready_for_final_touchdown_controller_teacher_calibration": bool(
        ready_teacher
    ),
    "stable_landed_hold_already_achieved": bool(
        stable_landed_ready
    ),
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'touchdown_sweep.csv'}")
print(f"  {RESULT_DIR / 'touchdown_traces.csv'}")
if fresh_result is not None:
    print(f"  {RESULT_DIR / 'best_touchdown_trace.csv'}")
print(f"  {RESULT_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "This script identifies actual skid contact using JSBSim WOW/compression "
    "and records the first-contact AGL, vertical speed, geometry, actuator "
    "state, bounce behavior, and 5-s landed-hold evidence."
)
