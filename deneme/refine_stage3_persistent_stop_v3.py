...
OUT_DIR = Path("results_stage3_persistent_stop_v3")
OUT_DIR.mkdir(parents=True, exist_ok=True)

SUMMARY_CSV = OUT_DIR / "persistent_stop_sweep.csv"
BEST_TRACE_CSV = OUT_DIR / "best_persistent_stop_trace.csv"
...
KV = 1.00
K_POS_VALUES = [0.055, 0.060, 0.065]

# Critical V3 refinement:
# V2 allowed v_des=12 ft/s immediately after takeover even though
# actual cruise speed was only ~10.2 ft/s, so the teacher first
# accelerated the helicopter before asking it to brake.
# Sweep a lower approach speed cap instead.
V_FWD_MAX_VALUES = [9.0, 9.5, 10.0, 10.5]
...
def run_case(k_pos, v_fwd_max, detailed=False):
    (
        env1,
        env2,
        fdm,
...
rule("STAGE 3 — PERSISTENT ENDPOINT STOP V3")
...
print(
    "The previous STOP=True was transient; the aircraft later drifted "
    "to ~310.7 ft. V3 therefore requires a stricter 5-second persistent stop."
)
print(
    "V3 changes only the approach speed cap and a narrow K_POS band."
)
print()
...
for k_pos in K_POS_VALUES:
    for v_fwd_max in V_FWD_MAX_VALUES:
        case_no += 1
...
rule("TOP PERSISTENT-STOP V3 CANDIDATES")
...
rule("BEST PERSISTENT-STOP V3 — FULL DETAILED FLIGHT")
...
rule("STAGE 3 PERSISTENT STOP V3 FINAL RESULT")
...
if best_detailed["persistent_stop"]:
    ...
    print(
        "Use this trace for one further narrow approach-speed refinement."
    )
