...
RESULT_DIR = Path(
    "results_stage4_continuous_landed_hold_v2"
)
...
FULL_RESULT_DIR = Path("results_stage4_continuous_landed_hold_v2")
...
rule("A — CONTINUOUS-MISSION LANDED-HOLD TARGET SWEEP")

rows = []
all_traces = []

for case_idx, landed_target in enumerate(
    LANDED_TARGET_COLLECTIVES,
    start=1,
):
    print(
        f"Candidate {case_idx}/{len(LANDED_TARGET_COLLECTIVES)} | "
        f"landed target COLL={landed_target:.6f}"
    )

    result, trace = run_full_stage4_teacher(
        landed_target_collective=landed_target,
        detailed=False,
    )
...
def rank_key(row):
    return (
        ...
        0 if not row["bounce"] else 1,
        -row["landed_hold_s"],
        abs(row["landed_target_collective"] - 0.541194),
        row["max_abs_position_error_ft"],
        row["max_abs_cross_track_ft"],
    )


rule("B — CONTINUOUS-MISSION LANDED-HOLD SELECTION")
...
for rank, row in enumerate(ranked, start=1):
    print(
        ...
        f"PASS={row['pass']} | "
        f"TARGET={row['landed_target_collective']:.6f} | "
        f"HOLD={row['landed_hold_s']:.2f}s "
        f"WOW={row['final_wow']} "
        f"bounce={row['bounce']} | "
        f"presentation={row['presentation_pass']}"
    )
...
if passing:
    ...
    print(
        ...
        f"landed target COLL="
        f"{selected['landed_target_collective']:.6f}"
    )

    rule("C — FRESH DETAILED FULL-TEACHER REPEAT")

    fresh_result, fresh_trace = run_full_stage4_teacher(
        landed_target_collective=
            selected["landed_target_collective"],
        detailed=True,
    )

    fc = fresh_result["touchdown_first_contact"]

    print()
    print(
        "FRESH RESULT | "
        f"PASS={fresh_result['pass']} | "
        f"same_fdm={fresh_result['same_fdm']} "
        f"clock_reset={fresh_result['clock_reset_on_attach']} | "
        f"presentation={fresh_result['presentation_pass']} | "
        f"CONTACT_ALT={fc['altitude_ft']:.3f} "
        f"CONTACT_VS={fc['vertical_speed_fps']:+.3f} "
        f"CONTACT_COLL={fc['physical_collective_cmd']:.6f} | "
        f"LANDED_TARGET="
        f"{fresh_result['landed_target_collective']:.6f} | "
        f"HOLD={fresh_result['landed_hold_s']:.2f}s "
        f"WOW={fresh_result['final_wow']} "
        f"BOUNCE={fresh_result['bounce']} | "
        f"FWDerr={fresh_result['final_position_error_ft']:+.3f} "
        f"X={fresh_result['final_cross_track_ft']:+.3f}"
    )
else:
    print()
    print("NO PASSING CONTINUOUS-MISSION LANDED-HOLD TARGET.")


rule("D — CONTINUOUS LANDED-HOLD V2 CONCLUSION")

ready_full_teacher_lock = bool(
    fresh_result is not None
    and fresh_result["pass"]
)

print(
    "READY TO LOCK FULL STAGE-4 TEACHER: "
    f"{ready_full_teacher_lock}"
)

save_rows_csv(
    FULL_RESULT_DIR / "landed_target_sweep.csv",
    rows,
)
save_rows_csv(
    FULL_RESULT_DIR / "landed_target_sweep_traces.csv",
    all_traces,
)

if fresh_result is not None:
    save_rows_csv(
        FULL_RESULT_DIR / "best_full_teacher_trace.csv",
        fresh_trace,
    )
...
summary = {
    ...
    "landed_target_collectives": LANDED_TARGET_COLLECTIVES,
    "rows": rows,
    "fresh_result": fresh_result,
    "ready_to_lock_full_stage4_teacher": bool(
        ready_full_teacher_lock
    ),
}

with open(
    FULL_RESULT_DIR / "final_summary.json",
    "w",
) as f:
    json.dump(summary, f, indent=2)

print("Saved:")
print(f"  {FULL_RESULT_DIR / 'landed_target_sweep.csv'}")
print(
    f"  {FULL_RESULT_DIR / 'landed_target_sweep_traces.csv'}"
)
if fresh_result is not None:
    print(f"  {FULL_RESULT_DIR / 'best_full_teacher_trace.csv'}")
print(f"  {FULL_RESULT_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "This script changes only the post-contact collective target while "
    "reproducing the same continuous same-FDM teacher path to first WOW."
)
