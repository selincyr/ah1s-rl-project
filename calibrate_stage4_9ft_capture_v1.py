...
rule("B — LOAD CONFIRMED V4 GROUND-EFFECT CURVE")
print(f"confirmed curve points loaded: {len(curve_points)}")
print(
    f"measured altitude domain: "
    f"{curve_points[0][0]:.3f} .. {curve_points[-1][0]:.3f} ft"
)
print(
    f"measured collective domain: "
    f"{curve_points[0][1]:.6f} .. {curve_points[-1][1]:.6f}"
)
print(
    "lowest confirmed point | "
    f"ALT={curve_points[0][0]:.3f} "
    f"COLL={curve_points[0][1]:.6f}"
)
print(
    "local lower-curve fit | "
    f"dCOLL/dALT={local_slope:+.6f} per ft | "
    f"R^2={local_r2:.4f}"
)
print(
    "NOTE: 9 ft lies below the confirmed curve domain; "
    "V1 uses bounded local extrapolation plus VS feedback."
)


rule("C — 9-FT CAPTURE CALIBRATION SWEEP")
rows = []
all_traces = []
case_idx = 0

total_cases = (
    len(EXTRAP_SLOPE_MULTIPLIERS)
    * len(CAPTURE_KVS_GRID)
)

for mult in EXTRAP_SLOPE_MULTIPLIERS:
    for kvs in CAPTURE_KVS_GRID:
        case_idx += 1

        print(
            f"Candidate {case_idx:02d}/{total_cases} | "
            f"slopeMult={mult:.2f} | "
            f"Kvs={kvs:.3f}"
        )

        result, trace = run_capture_candidate(
            slope_multiplier=mult,
            capture_kvs=kvs,
            points=curve_points,
        ...
        print(
            ...
            f"COLL=[{result['physical_collective_min']:.6f},"
            f"{result['physical_collective_max']:.6f}] | "
            f"extrap={100*result['extrapolation_fraction']:.1f}% "
            f"HOLD={result['hold_s']:.2f}s"
        )


rule("D — 9-FT CAPTURE SELECTION")
...
for rank, row in enumerate(ranked, start=1):
    print(
        ...
        f"mult={row['slope_multiplier']:.2f} "
        f"Kvs={row['capture_kvs']:.3f} | "
        f"ALT={row['final_altitude_ft']:.3f} "
...
if passing:
    ...
    print(
        ...
        f"slopeMult={selected['slope_multiplier']:.2f} | "
        f"Kvs={selected['capture_kvs']:.3f}"
    )

    rule("E — FRESH DETAILED REPEAT")
    fresh_result, fresh_trace = run_capture_candidate(
        slope_multiplier=selected["slope_multiplier"],
        capture_kvs=selected["capture_kvs"],
        points=curve_points,
        local_slope=local_slope,
        detailed=True,
    )
    ...
    print(
        ...
        f"HOLD={fresh_result['hold_s']:.2f}s"
    )
else:
    print()
    print("NO PASSING 9-FT CAPTURE CANDIDATE.")


rule("F — 9-FT CAPTURE V1 CONCLUSION")
...
print(
    f"READY FOR TOUCHDOWN / WOW IDENTIFICATION: "
    f"{ready_touchdown}"
)

if fresh_result is not None:
    print(
        "selected capture controller | "
        f"slopeMult={fresh_result['slope_multiplier']:.2f}, "
        f"Kvs={fresh_result['capture_kvs']:.3f}, "
        f"COLLmin={fresh_result['physical_collective_min']:.6f}, "
        f"COLLmax={fresh_result['physical_collective_max']:.6f}"
    )

save_rows_csv(
    RESULT_DIR / "capture_sweep.csv",
    rows,
)
save_rows_csv(
    RESULT_DIR / "capture_traces.csv",
    all_traces,
)

if fresh_result is not None:
    save_rows_csv(
        RESULT_DIR / "best_capture_trace.csv",
        fresh_trace,
    )
...
summary = {
    ...
    "target_altitude_ft": CAPTURE_TARGET_ALT_FT,
    "target_tolerance_ft": CAPTURE_ALT_TOL_FT,
    "hold_seconds": CAPTURE_HOLD_SECONDS,
    "confirmed_curve_points": [
        {
            "altitude_ft": float(a),
            "physical_collective": float(c),
        }
        for a, c in curve_points
    ],
    "local_lower_curve_slope": float(local_slope),
    "local_lower_curve_intercept": float(local_intercept),
    "local_lower_curve_r2": float(local_r2),
    "slope_multipliers": EXTRAP_SLOPE_MULTIPLIERS,
    "capture_kvs_grid": CAPTURE_KVS_GRID,
    "rows": rows,
    "fresh_result": fresh_result,
    "ready_for_touchdown_wow_identification": bool(
        ready_touchdown
    ),
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'capture_sweep.csv'}")
print(f"  {RESULT_DIR / 'capture_traces.csv'}")
if fresh_result is not None:
    print(f"  {RESULT_DIR / 'best_capture_trace.csv'}")
print(f"  {RESULT_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "If READY=True, the next step is a separate very-slow touchdown "
    "identification using actual WOW/compression properties."
)
