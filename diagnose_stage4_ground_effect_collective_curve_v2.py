...
RESULT_DIR = Path(
    "results_stage4_ground_effect_collective_curve_v2"
)
...
rule("C — ROBUST STEPWISE GROUND-EFFECT EQUILIBRIUM CONTINUATION")
rows = []
trace_rows = []
overall_safe = True
curve_valid = True
...
window_steps = max(
    2,
    int(round(CURVE_STABILITY_WINDOW_S / dt)),
)
...
for level_idx, physical_collective in enumerate(
    ...
    for step in range(int(CURVE_LEVEL_MAX_TIME_S / dt)):
        ...
        if len(level_window) > window_steps:
            level_window.pop(0)
        ...
        if (
            t >= CURVE_MIN_DWELL_S
            and len(level_window) >= window_steps
        ):
            stable_now, stats = rolling_equilibrium(level_window)
    ...
    settled = bool(
        overall_safe
        and level_term == "equilibrium_confirmed"
    )

    if not settled and overall_safe:
        curve_valid = False
        termination = "unsettled_level"
        level_term = "unsettled_level"
    ...
    row = {
        ...
        "equilibrium_vs_abs_peak_fps": float(
            equilibrium_stats.get(
                "vs_abs_peak_fps",
                float("nan"),
            )
        ),
        "equilibrium_altitude_range_ft": float(
            equilibrium_stats.get(
                "altitude_range_ft",
                float("nan"),
            )
    ...
    print(
        ...
        f"TERM={level_term} | "
        f"EQALT={row['equilibrium_altitude_mean_ft']:.2f} "
        f"EQVS={row['equilibrium_vs_mean_fps']:+.3f} "
        f"ALTrange={row['equilibrium_altitude_range_ft']:.3f} | "
        f"finalALT={final['altitude_ft']:.2f} "
        f"finalVS={final['vertical_speed_fps']:+.3f} | "
        f"FWDerr={final['position_error_ft']:+.2f} "
        f"X={final['cross_track_ft']:+.2f} | "
        f"minALT={min_alt:.2f} minVS={min_vs:+.3f}"
    )

    if not overall_safe or not settled:
        break


rule("D — ROBUST GROUND-EFFECT CURVE V2 CONCLUSION")

settled_rows = [
    r for r in rows
    if r["safe"]
    and r["settled"]
    and np.isfinite(r["equilibrium_altitude_mean_ft"])
]

monotonic = False
if len(settled_rows) >= 3:
    alts = [
        r["equilibrium_altitude_mean_ft"]
        for r in settled_rows
    ]
    colls = [
        r["physical_collective"]
        for r in settled_rows
    ]
    monotonic = all(
        alts[i + 1] <= alts[i] + 0.15
...
if len(settled_rows) >= 2:
    altitude_span = (
        settled_rows[0]["equilibrium_altitude_mean_ft"]
        - settled_rows[-1]["equilibrium_altitude_mean_ft"]
    )
...
ready_schedule = bool(
    curve_valid
    and overall_safe
    and len(settled_rows) >= 5
    and monotonic
    and altitude_span >= 4.0
    and np.isfinite(lowest_eq_alt)
    and lowest_eq_alt <= 11.0
)

ready_capture_range = bool(
    curve_valid
    and overall_safe
    and len(settled_rows) >= 4
    and monotonic
    and np.isfinite(lowest_eq_alt)
    and lowest_eq_alt <= 12.5
)

print(f"curve valid: {curve_valid}")
...
print(f"lowest confirmed equilibrium altitude: {lowest_eq_alt:.3f} ft")
print()
print(
...
print(
    "READY FOR 9-FT CAPTURE-RANGE CONTROLLER DESIGN: "
    f"{ready_capture_range}"
)

save_rows_csv(
    RESULT_DIR / "ground_effect_collective_curve_v2.csv",
    rows,
)
save_rows_csv(
    RESULT_DIR / "ground_effect_collective_curve_v2_traces.csv",
    trace_rows,
)
...
summary = {
    ...
    "minimum_dwell_s": CURVE_MIN_DWELL_S,
    "stability_window_s": CURVE_STABILITY_WINDOW_S,
    "curve_level_max_time_s": CURVE_LEVEL_MAX_TIME_S,
    "vs_mean_abs_max_fps": CURVE_VS_MEAN_ABS_MAX_FPS,
    "vs_peak_abs_max_fps": CURVE_VS_PEAK_ABS_MAX_FPS,
    "altitude_range_max_ft": CURVE_ALT_RANGE_MAX_FT,
    ...
    "lowest_confirmed_equilibrium_altitude_ft": float(
        lowest_eq_alt
    ),
    "ready_for_altitude_scheduled_low_alt_collective_feedforward_calibration": bool(
        ready_schedule
    ),
    "ready_for_9ft_capture_range_controller_design": bool(
        ready_capture_range
    ),
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'ground_effect_collective_curve_v2.csv'}")
print(f"  {RESULT_DIR / 'ground_effect_collective_curve_v2_traces.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "V2 is a robust continuation measurement only; a final feedforward "
    "schedule is created only from confirmed equilibria."
)
