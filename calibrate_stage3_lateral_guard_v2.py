...
OUT_DIR = Path("results_stage3_lateral_guard_v2")
OUT_DIR.mkdir(parents=True, exist_ok=True)

SUMMARY_CSV = OUT_DIR / "stage3_lateral_guard_v2_summary.csv"
TRACE_CSV = OUT_DIR / "stage3_lateral_guard_v2_trace.csv"
...
# ------------------------------------------------------------
# PRE-ENDPOINT LATERAL GUARD
# ------------------------------------------------------------
# The Hybrid Final Stage-2 handoff drifts laterally during braking and
# reaches the +/-15 ft safety wall before the old endpoint-only teacher
# can activate.  This guard does NOT change the locked longitudinal
# braking law; it only reduces lateral drift while braking.
LATERAL_GUARD_CROSS_ON_FT = 3.5
LATERAL_GUARD_KP = 0.020
LATERAL_GUARD_KD = 0.180
MAX_GUARD_CORR = 0.30

# Single V2 diagnostic endpoint settings.  First verify the guard sign
# and full 75 s geometry.  Only after this passes do another trim/Kp sweep.
...
def teacher_action(
    ...
    # Early guard: only prevents the braking segment from walking into
    # the +/-15 ft cross-track safety wall before endpoint control starts.
    guard_active = bool(
        not lateral_active
        and abs(state["cross_track_ft"])
        >= LATERAL_GUARD_CROSS_ON_FT
    )
    ...
    if lateral_active:
        ...
        # Correction is residual around the locked Stage-2 cruise aileron.
        action[2] = float(
            np.clip(
...
def run_case(
    ...
    for step in range(
        ...
        guard_ever_active = (
            guard_ever_active or bool(guard_active)
        )
    ...
    result = {
        ...
        "guard_ever_active": bool(guard_ever_active),
...
# ============================================================
# SINGLE V2 DIAGNOSTIC
# ============================================================

rule("STAGE 3 — PRE-ENDPOINT LATERAL GUARD V2")
...
print("Goal 1: prevent cross-track safety break during braking.")
print("Goal 2: reach endpoint teacher activation and evaluate full hover.")
print()
print(f"Guard threshold  : {LATERAL_GUARD_CROSS_ON_FT:.2f} ft")
print(f"Guard Kp / Kd    : {LATERAL_GUARD_KP:.3f} / {LATERAL_GUARD_KD:.3f}")
print(f"Guard max corr   : {MAX_GUARD_CORR:.2f}")
...
rule("STAGE 3 LATERAL GUARD V2 — FINAL RESULT")
...
if result["endpoint_hover"]:
    print("ENDPOINT HOVER: TRUE")
    print("V2 diagnostic passed the complete Stage-3 hover criteria.")
elif result["safe"]:
    print("ENDPOINT HOVER: FALSE")
    print("Guard kept the run safe, but Stage-3 still needs endpoint tuning/re-validation.")
else:
    print("ENDPOINT HOVER: FALSE")
    print("Safety was violated. Inspect the detailed trace before changing any locked longitudinal parameter.")

print()
print("IMPORTANT: Do not declare Stage-3 locked from a flag alone; inspect full geometry.")
