...
RESULT_DIR = Path(
    "results_stage4_full_teacher_v3"
)
...
FULL_RESULT_DIR = Path("results_stage4_full_teacher_v3")
FULL_RESULT_DIR.mkdir(parents=True, exist_ok=True)

TOUCHDOWN_VS_DES_HIGH = -0.10
TOUCHDOWN_VS_DES_LOW = -0.04
...
def run_full_stage4_teacher(detailed=True):
    ...
    for step in range(int(TOUCHDOWN_MAX_TIME_S / dt)):
        ...
        # range. Unlike V2, this cannot get trapped merely because the current
        # altitude maps to a local equilibrium collective.
        ramp_baseline_coll = float(max(
            contact_coll_ref,
            touchdown_start_coll
            - TOUCHDOWN_BASELINE_RAMP_PER_S
            * touchdown_elapsed,
        ))
