...
RESULT_DIR = Path(
    "results_stage4_ground_effect_curve_extension_v4"
)
...
rule("B — REPRODUCE CONFIRMED CURVE TO COLL=0.569500")
...
for idx, coll in enumerate(ANCHOR_LEVELS, start=1):
    ...
    if not result["safe"] or not result["settled"]:
        close_handoff(run)
        raise RuntimeError(
            "Could not reproduce previously confirmed curve anchor."
        )


rule("C — FINAL TARGETED CURVE EXTENSION")
overall_safe = True
curve_valid = True
...
for idx, coll in enumerate(EXTENSION_LEVELS, start=1):
    ...
    # Enough direct lower-curve support exists once the confirmed equilibrium
    # is at or below 10.25 ft. Stop before moving closer to touchdown.
    if (
        np.isfinite(row["equilibrium_altitude_mean_ft"])
        and row["equilibrium_altitude_mean_ft"] <= 10.25
    ):
        termination = "direct_schedule_support_reached"
        break


rule("D — GROUND-EFFECT CURVE EXTENSION V4 CONCLUSION")
...
print(
    f"confirmed new extension points: "
    f"{len(confirmed_extension)}"
)
print(f"combined monotonic relation: {monotonic}")
print(
...
save_rows_csv(
    RESULT_DIR / "final_curve_extension.csv",
    extension_rows,
)
save_rows_csv(
    RESULT_DIR / "curve_extension_traces.csv",
    trace_rows,
)
...
print(f"  {RESULT_DIR / 'final_curve_extension.csv'}")
print(f"  {RESULT_DIR / 'curve_extension_traces.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "This V4 closes only the final pre-touchdown equilibrium-curve gap."
)
