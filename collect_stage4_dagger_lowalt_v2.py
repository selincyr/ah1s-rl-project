...
FULL_RESULT_DIR = Path("results_stage4_dagger_lowalt_v2")
...
# =====================================================================
# STAGE-4 DISTILLED POLICY — TEACHER-OFF JSBSIM VALIDATION V1
# =====================================================================
#
...
# =====================================================================
# STAGE-4 LOW-ALT DAgger-STYLE SHADOW COLLECTION V2
# =====================================================================
#
# Student remains the ONLY live controller.
# The locked teacher is queried only as a shadow labeler on student-visited
# high-altitude descent states. No teacher action is applied to JSBSim.
# This is corrective imitation-data collection, NOT reinforcement learning.

DAGGER_DIR = Path("results_stage4_dagger_lowalt_v2")
DAGGER_DIR.mkdir(parents=True, exist_ok=True)

LOCKED_TEACHER_TRACE = Path(
    "results_stage4_teacher_final_v1/final_trace.csv"
)

DAGGER_ENTRY_CONFIGS = [
...
DAGGER_MAX_DESCENT_TIME_S = 420.0

# Corrective V2 fixed the high-altitude descent but overshot the 30-ft
# settle window. Query the shadow teacher throughout the descent so its
# internal vertical-bias state evolves correctly, but SAVE labels only
# in the low-altitude approach window.
LOWALT_SAVE_START_FT = 70.0
LOWALT_HARD_FLOOR_FT = 24.0
LOWALT_MIN_VS_FPS = -1.60

DAGGER_BASE_WEIGHT = 1.0
DAGGER_COLLECTIVE_DELTA_SCALE = 0.0010
DAGGER_MAX_PRIORITY_WEIGHT = 6.0


def load_teacher_physical_mapping():
    if not LOCKED_TEACHER_TRACE.exists():
        raise FileNotFoundError(
            f"Missing locked teacher trace: {LOCKED_TEACHER_TRACE}"
        )

    rows = []

    with open(LOCKED_TEACHER_TRACE, newline="") as f:
        reader = csv.DictReader(f)

        for r in reader:
            if r.get("phase") != "descent_300_to_30":
                continue

            try:
                rows.append({
                    "a0": float(r["teacher_action0"]),
                    "a2": float(r["teacher_action2"]),
                    "phys0": float(r["physical_collective_cmd"]),
                    "phys2": float(r["physical_aileron_cmd"]),
                    "phys3": float(r["physical_rudder_cmd"]),
                })
            except Exception:
                continue
    ...
    if len(rows) < 100:
        raise RuntimeError(
            "Not enough locked descent teacher rows to fit physical mapping."
        )

    a0 = np.asarray([r["a0"] for r in rows], dtype=float)
    a2 = np.asarray([r["a2"] for r in rows], dtype=float)
    p0 = np.asarray([r["phys0"] for r in rows], dtype=float)
    p2 = np.asarray([r["phys2"] for r in rows], dtype=float)
    p3 = np.asarray([r["phys3"] for r in rows], dtype=float)

    c0 = np.polyfit(a0, p0, 1)
    c2 = np.polyfit(a2, p2, 1)
    ...
    def r2(y, yhat):
        ...
        ss_tot = float(np.sum((y - np.mean(y)) ** 2))
        return 1.0 - ss_res / max(1e-20, ss_tot)

    info = {
        "collective_slope": float(c0[0]),
        "collective_intercept": float(c0[1]),
        "collective_r2": float(r2(p0, pred0)),
        "aileron_slope": float(c2[0]),
        "aileron_intercept": float(c2[1]),
        "aileron_r2": float(r2(p2, pred2)),
        "rudder_physical_median": float(np.median(p3)),
        "rows": int(len(rows)),
    }

    if info["collective_r2"] < 0.999:
        raise RuntimeError(
            f"Collective normalized->physical fit R2 too low: "
            f"{info['collective_r2']:.6f}"
        )

    if info["aileron_r2"] < 0.999:
        raise RuntimeError(
            f"Aileron normalized->physical fit R2 too low: "
            f"{info['aileron_r2']:.6f}"
        )

    return info


def shadow_teacher_physical_action(
    state,
    hover_action0,
    vertical_bias,
    dt,
    ...
    teacher_action, ctrl, new_vertical_bias = (
        build_stage4_teacher_action(
            state,
            hover_action0=hover_action0,
            descent_vmax=LOCKED_DESCENT_VMAX,
            vs_kp=LOCKED_VS_KP,
    ...
    physical = np.asarray(
        [
            mapping["collective_slope"] * float(teacher_action[0])
            + mapping["collective_intercept"],
            stage4_action1_to_physical(float(teacher_action[1])),
            mapping["aileron_slope"] * float(teacher_action[2])
            + mapping["aileron_intercept"],
            mapping["rudder_physical_median"],
        ],
        dtype=np.float32,
    )
    ...
    return (
        ...
        physical,
        ctrl,
        float(new_vertical_bias),
    )


def priority_weight(student_phys, teacher_phys):
    collective_delta = abs(
        float(student_phys[0]) - float(teacher_phys[0])
    )

    w = (
        DAGGER_BASE_WEIGHT
        + collective_delta / DAGGER_COLLECTIVE_DELTA_SCALE
    )
...
def run_shadow_rollout(
    ...
    obs_pack = np.load(OBS_NORM_PATH)
    obs_mean = obs_pack["mean"].astype(np.float32)
    obs_std = obs_pack["std"].astype(np.float32)

    action_pack = np.load(ACTION_MAP_PATH)
    physical_low = action_pack["physical_low"].astype(np.float32)
    physical_high = action_pack["physical_high"].astype(np.float32)
    ...
    for _ in range(int(PHASE_TIMEOUT_S["pre_descent_hold"] / dt)):
        contact = contact_snapshot(fdm)

        raw_obs, norm_obs = build_student_obs(
            state,
            contact,
            "pre_descent_hold",
            obs_mean,
        ...
        _, student_phys = student_predict_physical(
            model,
            norm_obs,
            device,
            physical_low,
        ...
        endpoint_now = endpoint_hover_now(state)
        endpoint_seen = endpoint_seen or endpoint_now
        ...
        if not endpoint_seen:
            if abs(state["position_error_ft"]) > PRE_HOLD_FORWARD_ERROR_LIMIT_FT:
                ...
                return {
                    "valid": False,
                    "termination": "pre_hold_forward_guard_exit",
                    "samples": [],
                }
        else:
            if abs(state["position_error_ft"]) > 5.0:
                ...
                return {
                    "valid": False,
                    "termination": "pre_hold_forward_corridor_exit",
                    "samples": [],
                }
        ...
        if abs(state["cross_track_ft"]) > 5.0:
            ...
            return {
                "valid": False,
                "termination": "pre_hold_cross_exit",
                "samples": [],
            }

        if endpoint_hold_s >= PRE_DESCENT_HOLD_SECONDS:
            break
    ...
    shadow_vertical_bias = 0.0

    samples = []
    min_vs = +999.0
    max_abs_coll_delta = 0.0
    termination = "descent_timeout"

    for step in range(int(DAGGER_MAX_DESCENT_TIME_S / dt)):
        state_before = snapshot(
            fdm,
            lat0,
            lon0,
        ...
        contact_before = contact_snapshot(fdm)
        ...
        student_norm, student_phys = student_predict_physical(
            model,
            norm_obs,
            device,
            physical_low,
        ...
        (
            ...
            teacher_phys,
            ctrl,
            shadow_vertical_bias,
        ) = shadow_teacher_physical_action(
            state_before,
            hover_action0,
            shadow_vertical_bias,
            dt,
        ...
        delta = (
            student_phys.astype(np.float64)
            - teacher_phys.astype(np.float64)
        )
        ...
        in_lowalt_window = bool(
            state_before["altitude_ft"] <= LOWALT_SAVE_START_FT
            and state_before["altitude_ft"] >= LOWALT_HARD_FLOOR_FT
        )

        if in_lowalt_window:
            samples.append({
                "rollout_id": int(rollout_id),
                "sample_index": int(step),
                "phase_id": 1,
                "phase": "descent_300_to_30",
                "time_in_descent_s": float(step * dt),
                "obs": raw_obs.astype(np.float32),
                "teacher_physical": teacher_phys.astype(np.float32),
                "student_physical": student_phys.astype(np.float32),
                "student_normalized": student_norm.astype(np.float32),
                "teacher_legacy_normalized": teacher_norm_legacy.astype(np.float32),
                "priority_weight": float(w),
                "teacher_vs_des_fps": float(ctrl["vs_des_fps"]),
                "teacher_vs_error_fps": float(ctrl["vs_error_fps"]),
                "teacher_collective_residual": float(
                    ctrl["collective_residual"]
                ),
                "student_minus_teacher_collective": float(delta[0]),
                "altitude_ft": float(state_before["altitude_ft"]),
                "vertical_speed_fps": float(
                    state_before["vertical_speed_fps"]
                ),
                "position_error_ft": float(
                    state_before["position_error_ft"]
                ),
                "cross_track_ft": float(
                    state_before["cross_track_ft"]
                ),
            })

            max_abs_coll_delta = max(
                max_abs_coll_delta,
                abs(float(delta[0])),
            )

        # Student action is the ONLY action applied to the live FDM.
        state = raw_student_physical_cycle(
            env2,
            fdm,
            student_phys,
        ...
        if detailed and (
            state_before["altitude_ft"] <= 80.0
            and (
                step == 0
                or step % max(1, int(round(5.0 / dt))) == 0
            )
        ):
            print(
                f"R{rollout_id:02d} t={step*dt:6.2f}s | "
                f"ALT={state_before['altitude_ft']:7.2f} "
                f"VS={state_before['vertical_speed_fps']:+6.3f} | "
                f"COLL student={student_phys[0]:.6f} "
                f"shadow={teacher_phys[0]:.6f} "
                f"Δ={delta[0]:+.7f} | "
                f"save={int(in_lowalt_window)} w={w:.2f}"
            )

        if stage4_settle_now(state):
            termination = "student_reached_30ft_settle"
            break

        if state["altitude_ft"] <= LOWALT_HARD_FLOOR_FT:
            termination = "lowalt_floor_guard"
            break

        if state["vertical_speed_fps"] <= LOWALT_MIN_VS_FPS:
            termination = "lowalt_vs_guard"
            break

        if abs(state["position_error_ft"]) > 5.0:
            termination = "forward_corridor_exit"
            break

        if abs(state["cross_track_ft"]) > 5.0:
            termination = "cross_corridor_exit"
            break

        if abs(state["heading_error_deg"]) > 2.0:
            termination = "heading_limit"
            break

        if abs(math.degrees(state["pitch_rad"])) > 8.0:
            termination = "pitch_limit"
            break

        if abs(math.degrees(state["roll_rad"])) > 10.0:
            termination = "roll_limit"
            break
    ...
    summary = {
        "valid": bool(len(samples) > 0),
        "termination": str(termination),
        "actual_handoff_forward_ft": float(
            start["state"]["forward_ft"]
        ),
        "actual_handoff_forward_speed_fps": float(
            start["state"]["forward_speed_fps"]
        ),
        "endpoint_hold_s": float(endpoint_hold_s),
        "num_samples": int(len(samples)),
        "min_vertical_speed_fps": float(min_vs),
        "max_abs_collective_delta": float(max_abs_coll_delta),
        "final_altitude_ft": float(final_state["altitude_ft"]),
        "final_vertical_speed_fps": float(
            final_state["vertical_speed_fps"]
        ),
...
def flatten_sample_for_csv(s):
    row = {
        "rollout_id": s["rollout_id"],
        "sample_index": s["sample_index"],
        "phase_id": s["phase_id"],
        "phase": s["phase"],
        "time_in_descent_s": s["time_in_descent_s"],
        "priority_weight": s["priority_weight"],
        "teacher_vs_des_fps": s["teacher_vs_des_fps"],
        "teacher_vs_error_fps": s["teacher_vs_error_fps"],
        "teacher_collective_residual": s[
            "teacher_collective_residual"
        ],
        "student_minus_teacher_collective": s[
            "student_minus_teacher_collective"
        ],
        "altitude_ft": s["altitude_ft"],
        "vertical_speed_fps": s["vertical_speed_fps"],
        "position_error_ft": s["position_error_ft"],
        "cross_track_ft": s["cross_track_ft"],
    }

    for i, v in enumerate(s["obs"]):
        row[f"obs_{i}"] = float(v)

    for i, v in enumerate(s["teacher_physical"]):
        row[f"teacher_physical_{i}"] = float(v)

    for i, v in enumerate(s["student_physical"]):
        row[f"student_physical_{i}"] = float(v)

    return row


rule("A — FIT LOCKED TEACHER NORMALIZED→PHYSICAL MAPPING")

mapping = load_teacher_physical_mapping()

print(
    f"teacher mapping rows={mapping['rows']} | "
    f"collective R2={mapping['collective_r2']:.8f} | "
    f"aileron R2={mapping['aileron_r2']:.8f}"
)
print(
    f"collective physical = "
    f"{mapping['collective_slope']:+.8f} * action0 "
    f"{mapping['collective_intercept']:+.8f}"
)
print(
    f"aileron physical = "
    f"{mapping['aileron_slope']:+.8f} * action2 "
    f"{mapping['aileron_intercept']:+.8f}"
)


rule("B — STUDENT-LIVE / TEACHER-SHADOW CORRECTIVE ROLLOUTS")

all_samples = []
rollout_rows = []

for rollout_id, cfg in enumerate(DAGGER_ENTRY_CONFIGS):
    print(
        f"Rollout {rollout_id + 1}/{len(DAGGER_ENTRY_CONFIGS)} | "
        f"entry={cfg['entry_forward_ft']:.1f} ft"
    )

    result = run_shadow_rollout(
        rollout_id=rollout_id,
        entry_forward_ft=cfg["entry_forward_ft"],
        entry_max_speed_fps=cfg["entry_max_speed_fps"],
        mapping=mapping,
        detailed=(rollout_id == 0),
    )
    ...
    print(
        ...
        f"minVS={result['min_vertical_speed_fps']:+.3f} | "
        f"max|Δcoll|={result['max_abs_collective_delta']:.6f} | "
        f"final ALT={result['final_altitude_ft']:.2f} "
        f"VS={result['final_vertical_speed_fps']:+.3f}"
    )

if not all_samples:
    raise RuntimeError(
        "No DAgger shadow samples were collected."
    )


rule("C — BUILD CORRECTIVE DATASET")

obs = np.stack(
    [s["obs"] for s in all_samples]
).astype(np.float32)

teacher_actions = np.stack(
    [s["teacher_physical"] for s in all_samples]
).astype(np.float32)

student_actions = np.stack(
    [s["student_physical"] for s in all_samples]
).astype(np.float32)
...
priority_weights = np.asarray(
    [s["priority_weight"] for s in all_samples],
    dtype=np.float32,
)
...
csv_rows = [
    flatten_sample_for_csv(s)
    for s in all_samples
]

save_rows_csv(
    DAGGER_DIR / "shadow_rollout_summary.csv",
    rollout_rows,
)

save_rows_csv(
    DAGGER_DIR / "dagger_shadow_dataset.csv",
    csv_rows,
)

np.save(
    DAGGER_DIR / "dagger_obs_raw.npy",
    obs,
)
np.save(
    DAGGER_DIR / "dagger_teacher_actions_physical.npy",
    teacher_actions,
)
np.save(
    DAGGER_DIR / "dagger_student_actions_physical.npy",
    student_actions,
)
np.save(
    DAGGER_DIR / "dagger_phase_ids.npy",
    phase_ids,
)
np.save(
    DAGGER_DIR / "dagger_rollout_ids.npy",
    rollout_ids,
)
np.save(
    DAGGER_DIR / "dagger_priority_weights.npy",
    priority_weights,
)
...
summary = {
    "training_type": "NONE",
    "collection_type": (
        "DAgger-style student-live / teacher-shadow corrective labeling"
    ),
    "teacher_applied_to_live_fdm": False,
    "student_applied_to_live_fdm": True,
    "num_rollouts": int(len(DAGGER_ENTRY_CONFIGS)),
    "num_samples": int(len(obs)),
    "obs_shape": list(obs.shape),
    "action_shape": list(teacher_actions.shape),
    "collective_delta_student_minus_teacher": {
        "mean": float(np.mean(collective_delta)),
        "mae": float(np.mean(np.abs(collective_delta))),
        "p95_abs": float(
            np.percentile(np.abs(collective_delta), 95.0)
        ),
        "max_abs": float(np.max(np.abs(collective_delta))),
    },
    "priority_weight": {
        "min": float(np.min(priority_weights)),
        "mean": float(np.mean(priority_weights)),
        "max": float(np.max(priority_weights)),
    },
    "teacher_mapping": mapping,
    "student_model": str(BC_MODEL),
    "targeted_phase": "descent_300_to_30 low-altitude settle approach",
    "lowalt_save_start_ft": float(LOWALT_SAVE_START_FT),
    "lowalt_hard_floor_ft": float(LOWALT_HARD_FLOOR_FT),
    "lowalt_min_vs_fps": float(LOWALT_MIN_VS_FPS),
    "ready_for_corrective_distillation": bool(
        len(obs) >= 1000
        and np.all(np.isfinite(obs))
        and np.all(np.isfinite(teacher_actions))
    ),
}

with open(
    DAGGER_DIR / "final_summary.json",
    "w",
) as f:
    json.dump(summary, f, indent=2)


rule("D — LOW-ALT DAgger SHADOW COLLECTION V2 CONCLUSION")

print(f"low-alt corrective samples: {len(obs)}")
print(
    "student-teacher collective delta | "
    f"mean={summary['collective_delta_student_minus_teacher']['mean']:+.7f} | "
    f"MAE={summary['collective_delta_student_minus_teacher']['mae']:.7f} | "
    f"P95={summary['collective_delta_student_minus_teacher']['p95_abs']:.7f} | "
    f"MAX={summary['collective_delta_student_minus_teacher']['max_abs']:.7f}"
)
print(
    "READY FOR CORRECTIVE DISTILLATION: "
    f"{summary['ready_for_corrective_distillation']}"
)
print("Saved:")
print(f"  {DAGGER_DIR / 'shadow_rollout_summary.csv'}")
print(f"  {DAGGER_DIR / 'dagger_shadow_dataset.csv'}")
print(f"  {DAGGER_DIR / 'dagger_obs_raw.npy'}")
print(
    f"  {DAGGER_DIR / 'dagger_teacher_actions_physical.npy'}"
)
print(
    f"  {DAGGER_DIR / 'dagger_student_actions_physical.npy'}"
)
print(f"  {DAGGER_DIR / 'dagger_priority_weights.npy'}")
print(f"  {DAGGER_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT call this RL. "
    "The live aircraft was controlled only by the distilled student; "
    "the locked teacher produced shadow labels only."
)
