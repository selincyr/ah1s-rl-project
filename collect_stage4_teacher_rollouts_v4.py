...
def run_teacher_case(
    ...
    start = build_stage4_handoff(
        ...
        custom_entry_max_speed_fps=float(entry_max_speed_fps),
    )
    env2 = start["env2"]
    fdm = start["fdm"]
    lat0 = start["lat0"]
...
FULL_RESULT_DIR = Path("results_stage4_teacher_rollouts_v4")
...
def run_full_stage4_teacher(landed_target_collective, detailed=True, entry_forward_ft=STAGE4_ENTRY_FORWARD_FT, entry_max_speed_fps=STAGE4_ENTRY_MAX_SPEED_FPS):
    ...
    actual_handoff_state = start["state"].copy()
    actual_handoff_stage3_elapsed_s = float(
        start["stage3_elapsed_s"]
    )
    ...
    result = {
        ...
        "requested_entry_forward_ft": float(entry_forward_ft),
        "requested_entry_max_speed_fps": float(entry_max_speed_fps),
        "actual_handoff_forward_ft": float(
            actual_handoff_state["forward_ft"]
        ),
        "actual_handoff_forward_speed_fps": float(
            actual_handoff_state["forward_speed_fps"]
        ),
        "actual_handoff_cross_track_ft": float(
            actual_handoff_state["cross_track_ft"]
        ),
        "actual_handoff_lateral_speed_fps": float(
            actual_handoff_state["lateral_speed_fps"]
        ),
        "actual_handoff_altitude_ft": float(
            actual_handoff_state["altitude_ft"]
        ),
        "actual_handoff_vertical_speed_fps": float(
            actual_handoff_state["vertical_speed_fps"]
        ),
        "actual_handoff_stage3_elapsed_s": float(
            actual_handoff_stage3_elapsed_s
        ),
        "mission_time_s": float(mission_time),
        "presentation_pass": bool(presentation_pass),
        "max_abs_position_error_ft": float(max_abs_pos),
...
# =====================================================================
# STAGE-4 TEACHER ROLLOUT COLLECTION / DATASET V1
# =====================================================================

# Locked teacher from final validation.
LOCKED_LANDED_TARGET_COLLECTIVE = 0.540944

# Previously qualified Stage-4 mission-manager entry window.
# This creates genuine same-FDM initial-state variation without injecting
# artificial action noise into the fragile touchdown/contact region.
ROLLOUT_CONFIGS = [
    {"entry_forward_ft": 294.0, "entry_max_speed_fps": 1.0},
    {"entry_forward_ft": 295.0, "entry_max_speed_fps": 1.0},
    {"entry_forward_ft": 296.0, "entry_max_speed_fps": 1.0},
    {"entry_forward_ft": 297.0, "entry_max_speed_fps": 1.0},
    {"entry_forward_ft": 298.0, "entry_max_speed_fps": 1.0},
    {"entry_forward_ft": 299.0, "entry_max_speed_fps": 1.0},
]
...
OBS_FEATURES = (
    OBS_BASE_FEATURES
    + [f"phase_{name}" for name in PHASES]
)

TARGET_FEATURES = [
    "target_physical_collective_cmd",
    "target_physical_elevator_cmd",
    "target_physical_aileron_cmd",
    "target_physical_rudder_cmd",
]

DATASET_DIR = Path("results_stage4_teacher_rollouts_v4")
...
def row_wow(row):
    # WOW is explicitly logged in touchdown/landed phases.
    return 1.0 if row_float(row, "wow_detected", 0.0) > 0.5 else 0.0


def row_compression(row):
    return max(
        0.0,
        row_float(row, "max_compression_ft", 0.0),
    )


def build_observation_from_trace_row(row, decision_phase):
    heading = row_float(row, "heading_error_rad", 0.0)

    base = [
        row_float(row, "altitude_ft"),
        row_float(row, "vertical_speed_fps"),
        row_float(row, "position_error_ft"),
        row_float(row, "forward_speed_fps"),
        row_float(row, "cross_track_ft"),
        row_float(row, "lateral_speed_fps"),
        math.sin(heading),
        math.cos(heading),
        row_float(row, "pitch_rad"),
        row_float(row, "roll_rad"),
        row_float(row, "roll_rate_rad_s"),
        row_float(row, "pitch_rate_rad_s"),
        row_float(row, "yaw_rate_rad_s"),
        row_float(row, "rotor_rpm"),
        row_float(row, "physical_collective_cmd"),
        row_float(row, "physical_elevator_cmd"),
        row_float(row, "physical_aileron_cmd"),
        row_float(row, "physical_rudder_cmd"),
        row_wow(row),
        row_compression(row),
    ]

    one_hot = [0.0] * len(PHASES)
    one_hot[PHASE_TO_ID[decision_phase]] = 1.0

    return np.asarray(base + one_hot, dtype=np.float32)


def build_physical_target_from_next_row(next_row):
    # Canonical Stage-4 student target is the actual physical actuator command
    # selected by the locked teacher for the NEXT control cycle.
    #
    # We intentionally do not use legacy normalized action0/action1 labels here:
    # Stage-4 uses calibrated physical collective/elevator mappings in different
    # altitude regimes. Physical actuator commands form one consistent target
    # representation across the entire Stage-4 mission.
    return np.asarray([
        row_float(next_row, "physical_collective_cmd"),
        row_float(next_row, "physical_elevator_cmd"),
        row_float(next_row, "physical_aileron_cmd"),
        row_float(next_row, "physical_rudder_cmd"),
    ], dtype=np.float32)


def aligned_samples_from_trace(trace, rollout_id):
    """
    Trace rows are logged AFTER a control cycle. Therefore:
      state at row i  -> command applied on row i+1
    is the correct one-step alignment for behavioral cloning.

    At phase boundaries, the mission manager has already selected the next
    decision phase, so the observation receives phase(row i+1).
    """
    samples = []
    ...
    for i in range(len(trace) - 1):
        ...
        decision_phase = str(nxt.get("phase", ""))
        if decision_phase not in PHASE_TO_ID:
            continue

        obs = build_observation_from_trace_row(
            row,
            decision_phase=decision_phase,
        )
        target = build_physical_target_from_next_row(nxt)
        ...
        samples.append({
            ...
            "sample_index": int(i),
            "state_phase": str(row.get("phase", "")),
            "decision_phase": decision_phase,
            "phase_id": int(PHASE_TO_ID[decision_phase]),
            "time_s": row_float(row, "time_s"),
            "next_time_s": row_float(nxt, "time_s"),
            "obs": obs,
            "target": target,
        })
...
def phase_balancing_weights(phase_ids):
    phase_ids = np.asarray(phase_ids, dtype=np.int64)
    counts = {
        int(pid): int(np.sum(phase_ids == pid))
        for pid in np.unique(phase_ids)
    }

    # sqrt inverse-frequency is intentionally milder than full inverse count.
    raw = np.asarray([
        1.0 / math.sqrt(max(1, counts[int(pid)]))
        for pid in phase_ids
    ], dtype=np.float32)

    raw /= max(1e-12, float(np.mean(raw)))
    return raw.astype(np.float32), counts


def dataset_rows_for_csv(samples):
    rows = []
    ...
    for s in samples:
        row = {
            ...
            "decision_phase": s["decision_phase"],
            "phase_id": s["phase_id"],
            "time_s": s["time_s"],
            "next_time_s": s["next_time_s"],
        }

        for name, value in zip(OBS_FEATURES, s["obs"]):
            row[f"obs_{name}"] = float(value)

        for name, value in zip(TARGET_FEATURES, s["target"]):
...
def target_stats(actions):
    ...
    for j, name in enumerate(TARGET_FEATURES):
        ...
        stats[name] = {
            ...
            "std": float(np.std(x)),
            "p01": float(np.percentile(x, 1.0)),
            "p99": float(np.percentile(x, 99.0)),
        }
    return stats


rule("A — LOCKED STAGE-4 TEACHER ROLLOUT COLLECTION")

all_samples = []
rollout_summaries = []
passing_rollouts = 0

for rollout_id, cfg in enumerate(ROLLOUT_CONFIGS):
    print(
        f"Rollout {rollout_id + 1}/{len(ROLLOUT_CONFIGS)} | "
        f"entry_forward={cfg['entry_forward_ft']:.1f} ft | "
        f"entry_vmax={cfg['entry_max_speed_fps']:.2f} ft/s"
    )

    result, trace = run_full_stage4_teacher(
        landed_target_collective=
            LOCKED_LANDED_TARGET_COLLECTIVE,
        detailed=False,
        entry_forward_ft=cfg["entry_forward_ft"],
        entry_max_speed_fps=cfg["entry_max_speed_fps"],
    )

    valid = bool(
        result["pass"]
        and result["same_fdm"]
        and not result["clock_reset_on_attach"]
        and result["presentation_pass"]
        and result["touchdown_vs_ok"]
        and result["landed_hold_s"] >= 5.0
        and result["final_wow"]
        and not result["bounce"]
    )

    samples = (
        aligned_samples_from_trace(trace, rollout_id)
        if valid
        else []
    )
    ...
    fc = result["touchdown_first_contact"]

    if "actual_handoff_forward_ft" not in result:
        raise RuntimeError(
            "Internal collector error: actual handoff telemetry missing "
            "from full-teacher result."
        )

    rollout_summary = {
        "rollout_id": int(rollout_id),
        "entry_forward_ft": float(cfg["entry_forward_ft"]),
        "entry_max_speed_fps": float(
            cfg["entry_max_speed_fps"]
        ),
        "actual_handoff_forward_ft": float(
            result["actual_handoff_forward_ft"]
        ),
        "actual_handoff_forward_speed_fps": float(
            result["actual_handoff_forward_speed_fps"]
        ),
        "actual_handoff_cross_track_ft": float(
            result["actual_handoff_cross_track_ft"]
        ),
        "actual_handoff_altitude_ft": float(
            result["actual_handoff_altitude_ft"]
        ),
        "actual_handoff_stage3_elapsed_s": float(
            result["actual_handoff_stage3_elapsed_s"]
        ),
        "valid_for_dataset": bool(valid),
        "pass": bool(result["pass"]),
        "same_fdm": bool(result["same_fdm"]),
        "clock_reset_on_attach": bool(
            result["clock_reset_on_attach"]
        ),
        "presentation_pass": bool(
            result["presentation_pass"]
        ),
        "samples": int(len(samples)),
        "contact_altitude_ft": float(fc["altitude_ft"]),
        "contact_vertical_speed_fps": float(
            fc["vertical_speed_fps"]
        ),
        "contact_collective": float(
            fc["physical_collective_cmd"]
        ),
        "landed_hold_s": float(result["landed_hold_s"]),
        "final_position_error_ft": float(
            result["final_position_error_ft"]
        ),
        "final_cross_track_ft": float(
            result["final_cross_track_ft"]
        ),
        "final_forward_speed_fps": float(
            result["final_forward_speed_fps"]
        ),
        "final_lateral_speed_fps": float(
            result["final_lateral_speed_fps"]
        ),
        "final_wow": bool(result["final_wow"]),
        "bounce": bool(result["bounce"]),
    }
    rollout_summaries.append(rollout_summary)

    print(
        f"  HANDOFF actual FWD={result['actual_handoff_forward_ft']:.3f} "
        f"V={result['actual_handoff_forward_speed_fps']:+.3f} "
        f"t3={result['actual_handoff_stage3_elapsed_s']:.2f}s | "
        f"VALID={valid} PASS={result['pass']} | "
        f"samples={len(samples)} | "
        f"contact ALT={fc['altitude_ft']:.3f} "
        f"VS={fc['vertical_speed_fps']:+.3f} | "
        f"HOLD={result['landed_hold_s']:.2f}s "
        f"WOW={result['final_wow']} bounce={result['bounce']} | "
        f"FWDerr={result['final_position_error_ft']:+.2f} "
        f"X={result['final_cross_track_ft']:+.2f}"
    )


rule("B — STAGE-4 DATASET ASSEMBLY")

if passing_rollouts < 4:
    raise RuntimeError(
        f"Only {passing_rollouts} valid teacher rollouts. "
        "Need at least 4 before building the Stage-4 BC dataset."
    )

if not all_samples:
    raise RuntimeError("No aligned Stage-4 teacher samples collected.")

obs = np.stack([s["obs"] for s in all_samples]).astype(np.float32)
actions = np.stack(
    [s["target"] for s in all_samples]
).astype(np.float32)
phase_ids = np.asarray(
    [s["phase_id"] for s in all_samples],
    dtype=np.int64,
)
rollout_ids = np.asarray(
    [s["rollout_id"] for s in all_samples],
    dtype=np.int64,
)

sample_weights, phase_counts = phase_balancing_weights(
    phase_ids
)

csv_rows = dataset_rows_for_csv(all_samples)

save_rows_csv(
    DATASET_DIR / "teacher_rollout_summary.csv",
    rollout_summaries,
)
save_rows_csv(
    DATASET_DIR / "stage4_bc_dataset.csv",
    csv_rows,
)

np.save(
    DATASET_DIR / "stage4_obs_raw.npy",
    obs,
)
np.save(
    DATASET_DIR / "stage4_actions_physical.npy",
    actions,
)
np.save(
    DATASET_DIR / "stage4_phase_ids.npy",
    phase_ids,
)
np.save(
    DATASET_DIR / "stage4_rollout_ids.npy",
    rollout_ids,
)
np.save(
    DATASET_DIR / "stage4_sample_weights.npy",
    sample_weights,
)


# Verify that the requested handoff variation actually produced distinct
# trajectories. This prevents duplicated deterministic rollouts from being
# mistaken for independent data.
import hashlib
...
rollout_obs_hashes = {}
rollout_action_hashes = {}

for rollout_id in sorted(int(x) for x in np.unique(rollout_ids)):
    mask = rollout_ids == rollout_id
    rollout_obs_hashes[str(rollout_id)] = _sha256_array(obs[mask])
    rollout_action_hashes[str(rollout_id)] = _sha256_array(actions[mask])

distinct_obs_rollouts = len(set(rollout_obs_hashes.values()))
distinct_action_rollouts = len(set(rollout_action_hashes.values()))

rollout_diversity_ok = bool(
    distinct_obs_rollouts >= 2
    and distinct_action_rollouts >= 2
)

metadata = {
    "stage4_training": "NONE",
    "collection_method": (
        "locked teacher, same FDM, qualified Stage-4 entry-state variation"
    ),
    "teacher_file": "build_stage4_teacher_final_v1.py",
    "locked_landed_target_collective": float(
        LOCKED_LANDED_TARGET_COLLECTIVE
    ),
    "rollout_configs": ROLLOUT_CONFIGS,
    "handoff_parameters_wired": True,
    "handoff_wiring_fix_version": "V3 actual run_full_stage4_teacher call",
    "handoff_entry_forward_values_ft": [float(x["entry_forward_ft"]) for x in ROLLOUT_CONFIGS],
    "requested_rollouts": int(len(ROLLOUT_CONFIGS)),
    "passing_rollouts": int(passing_rollouts),
    ...
    "phase_names": PHASES,
    "phase_counts": {
        PHASES[int(pid)]: int(count)
        for pid, count in phase_counts.items()
    },
    "target_stats": target_stats(actions),
    "alignment": (
        "trace state at row i paired with physical teacher command at row i+1; "
        "decision phase is phase(row i+1)"
    ),
    "canonical_action_representation": (
        "raw physical actuator commands; no mixed normalized action0/action1 semantics"
    ),
    "sample_weighting": (
        "sqrt inverse decision-phase frequency, normalized to mean 1"
    ),
    "rollout_obs_hashes": rollout_obs_hashes,
    "rollout_action_hashes": rollout_action_hashes,
    "distinct_observation_rollouts": int(distinct_obs_rollouts),
    "distinct_action_rollouts": int(distinct_action_rollouts),
    "rollout_diversity_ok": bool(rollout_diversity_ok),
    "ready_for_bc": False,
}

metadata["ready_for_bc"] = bool(
    passing_rollouts >= 4
    and obs.shape[0] >= 1000
    and rollout_diversity_ok
)
...
rule("C — ROLLOUT COLLECTION V4 CONCLUSION")

print(
    f"valid teacher rollouts: "
    f"{passing_rollouts}/{len(ROLLOUT_CONFIGS)}"
)
print(
    f"dataset shape: obs={obs.shape} "
    f"actions={actions.shape}"
)
print(
    f"distinct observation rollouts: "
    f"{distinct_obs_rollouts}/{passing_rollouts}"
)
print(
    f"distinct action rollouts: "
    f"{distinct_action_rollouts}/{passing_rollouts}"
)
print(
    f"ROLLOUT_DIVERSITY_OK={rollout_diversity_ok}"
)

for phase_name in PHASES:
    print(
        f"  {phase_name:30s}: "
        f"{metadata['phase_counts'].get(phase_name, 0)}"
    )

print("physical target ranges:")
for name, stats in metadata["target_stats"].items():
    print(
        f"  {name:36s} "
...
print(
    "READY FOR STAGE-4 BEHAVIORAL CLONING: "
    f"{metadata['ready_for_bc']}"
)

print("Saved:")
print(f"  {DATASET_DIR / 'teacher_rollout_summary.csv'}")
print(f"  {DATASET_DIR / 'stage4_bc_dataset.csv'}")
print(f"  {DATASET_DIR / 'stage4_obs_raw.npy'}")
print(f"  {DATASET_DIR / 'stage4_actions_physical.npy'}")
print(f"  {DATASET_DIR / 'stage4_phase_ids.npy'}")
print(f"  {DATASET_DIR / 'stage4_rollout_ids.npy'}")
print(f"  {DATASET_DIR / 'stage4_sample_weights.npy'}")
print(f"  {DATASET_DIR / 'dataset_metadata.json'}")
print()
print(
    "Do NOT train in this script. "
    "If READY=True, inspect phase counts and actuator ranges first; "
    "then build the fresh Stage-4 neural policy and perform policy distillation."
)
