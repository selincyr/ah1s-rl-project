...
RESULT_DIR = Path(
    "results_stage4_near_ground_diagnostic_v1"
)
...
rule("B — REPRODUCE QUALIFIED 300 -> 30 FT STAGE-4 TEACHER")
start = build_qualified_30ft_open(detailed=False)
state30 = start["state30"]
print(
    "30FT START | "
    f"ALT={state30['altitude_ft']:.3f} VS={state30['vertical_speed_fps']:+.3f} | "
    f"FWD={state30['forward_ft']:.3f} err={state30['position_error_ft']:+.3f} "
    f"V={state30['forward_speed_fps']:+.3f} | "
    f"X={state30['cross_track_ft']:+.3f} LAT={state30['lateral_speed_fps']:+.3f} | "
    f"HOLD={start['settle_hold_30_s']:.2f}s | "
    f"COLL={state30['physical_collective_cmd']:+.5f}"
)

initial_contact = contact_snapshot(start["fdm"])
if initial_contact:
    print("Contact-related properties visible at 30 ft:")
    for key, val in initial_contact.items():
        print(f"  {key} = {val}")
else:
    print("No candidate WOW/compression properties were readable; AGL/dynamics will remain the primary diagnostic.")


rule("C — CONTINUOUS NEAR-GROUND ALTITUDE LADDER (30 FT -> 9 FT)")
env2 = start["env2"]
fdm = start["fdm"]
lat0 = start["lat0"]
lon0 = start["lon0"]
...
all_trace = list(start["descent_trace"])
level_results = []
overall_safe = True
termination_reason = "completed_near_ground_ladder"

for level_index, target_alt in enumerate(NEAR_GROUND_TARGETS_FT, start=1):
    settle_hold = 0.0
    max_abs_pos = 0.0
    max_abs_cross = 0.0
    min_alt = +999.0
    ...
    print(f"\nLEVEL {level_index}/{len(NEAR_GROUND_TARGETS_FT)} | target={target_alt:.1f} ft")
    ...
    for step in range(int(NEAR_GROUND_LEVEL_MAX_TIME / dt)):
        ...
        settle_hold = settle_hold + dt if level_settle_now(state, target_alt) else 0.0
        max_abs_pos = max(max_abs_pos, abs(state["position_error_ft"]))
        max_abs_cross = max(max_abs_cross, abs(state["cross_track_ft"]))
        min_alt = min(min_alt, state["altitude_ft"])
        min_vs = min(min_vs, state["vertical_speed_fps"])
        min_collective = min(min_collective, state["physical_collective_cmd"])
        max_collective = max(max_collective, state["physical_collective_cmd"])
        residual_min = min(residual_min, ctrl["collective_residual"])
        residual_max = max(residual_max, ctrl["collective_residual"])

        row = {
            "phase": "near_ground_level",
        ...
        for key, val in contact.items():
            row["contact_" + key.replace("/", "_").replace("[", "").replace("]", "")] = float(val)
        ...
        if wow:
            overall_safe = False
            level_reason = "unexpected_weight_on_wheels_before_touchdown_test"
            termination_reason = level_reason
            break

        if t + 1e-9 >= next_print:
            print(
                f"  t={t:6.2f}s ALT={state['altitude_ft']:6.2f} "
                f"VS={state['vertical_speed_fps']:+6.3f}/{ctrl['vs_des_fps']:+5.2f} | "
                f"FWD={state['forward_ft']:7.2f} V={state['forward_speed_fps']:+6.3f} | "
                f"X={state['cross_track_ft']:+6.2f} LAT={state['lateral_speed_fps']:+6.3f} | "
                f"dA0={ctrl['collective_residual']:+6.3f} "
                f"COLL={state['physical_collective_cmd']:+.5f} VB={vertical_bias:+6.3f} | "
                f"HOLD={settle_hold:4.2f}s WOW={int(wow)}"
            )
            next_print += 10.0
    ...
    level_result = {
        ...
        "final_cross_track_ft": float(final_state["cross_track_ft"]),
        "final_lateral_speed_fps": float(final_state["lateral_speed_fps"]),
        "physical_collective_cmd": float(final_state["physical_collective_cmd"]),
        ...
        "collective_residual_min": float(residual_min),
        "collective_residual_max": float(residual_max),
        "wow_seen": bool(wow_seen),
    }
    level_results.append(level_result)

    print(
        f"  RESULT | PASS={level_pass} SAFE={overall_safe} REASON={level_reason} | "
        f"ALT={final_state['altitude_ft']:.2f} VS={final_state['vertical_speed_fps']:+.3f} | "
        f"FWD={final_state['forward_ft']:.2f} err={final_state['position_error_ft']:+.2f} "
        f"V={final_state['forward_speed_fps']:+.3f} | "
        f"X={final_state['cross_track_ft']:+.2f} LAT={final_state['lateral_speed_fps']:+.3f} | "
        f"COLL={final_state['physical_collective_cmd']:+.5f} VB={vertical_bias:+.3f}"
    )
...
rule("D — NEAR-GROUND / GROUND-EFFECT READINESS")
final_state = snapshot(fdm, lat0, lon0, mission_heading)
...
for r in level_results:
    print(
        f"target={r['target_alt_ft']:4.1f} | PASS={r['pass']} | "
        f"ALT={r['final_altitude_ft']:6.2f} VS={r['final_vertical_speed_fps']:+6.3f} | "
        f"FWDerr={r['final_position_error_ft']:+5.2f} X={r['final_cross_track_ft']:+5.2f} | "
        f"COLL={r['physical_collective_cmd']:+.5f} VB={r['vertical_bias_state']:+.3f}"
    )

print()
print(f"READY FOR TOUCHDOWN IDENTIFICATION: {ready}")
print(f"Termination: {termination_reason}")
print(
    f"Final near-ground state | ALT={final_state['altitude_ft']:.3f} "
    f"VS={final_state['vertical_speed_fps']:+.3f} "
    f"FWD={final_state['forward_ft']:.3f} err={final_state['position_error_ft']:+.3f} "
    f"V={final_state['forward_speed_fps']:+.3f} "
    f"X={final_state['cross_track_ft']:+.3f} LAT={final_state['lateral_speed_fps']:+.3f}"
)
...
save_rows_csv(RESULT_DIR / "near_ground_trace.csv", all_trace)
save_rows_csv(RESULT_DIR / "near_ground_level_summary.csv", level_results)
...
summary = {
    ...
    "qualified_300_to_30_low_alt_bias_floor": QUALIFIED_LOW_ALT_BIAS_FLOOR,
    "near_ground_targets_ft": NEAR_GROUND_TARGETS_FT,
    "near_ground_vmax_fps": NEAR_GROUND_VMAX_FPS,
    "near_ground_alt_to_vs_gain": NEAR_GROUND_ALT_TO_VS_GAIN,
    "ready_for_touchdown_identification": bool(ready),
    "termination_reason": str(termination_reason),
    "level_results": level_results,
    "final_state": {k: float(v) for k, v in final_state.items()},
    "contact_properties_at_30ft": initial_contact,
}
with open(RESULT_DIR / "diagnostic_summary.json", "w") as f:
    json.dump(summary, f, indent=2)

print("Saved:")
print(f"  {RESULT_DIR / 'near_ground_trace.csv'}")
print(f"  {RESULT_DIR / 'near_ground_level_summary.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print("Do NOT train Stage 4 here. If READY=True, next identify the touchdown/contact regime from the qualified ~9 ft state.")
close_handoff(start)
