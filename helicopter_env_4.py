...
class HelicopterEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.elevator_scale = 0.045
        self.aileron_scale = 0.040
        self.rudder_scale = 0.060
    ...
    def step(self, action):
        ...
        # ==========================================================
        # COMMON STABILITY TERMS
        # ==========================================================
        # The current curriculum is NOT forward flight. The policy is
        # explicitly taught to climb while keeping horizontal motion,
        # attitude and heading close to the trim condition.
        reward -= 2.00 * abs(heading_error)
        reward -= 0.18 * abs(forward_velocity)
        reward -= 0.22 * abs(lateral_velocity)

        reward -= 3.00 * abs(pitch)
        reward -= 4.00 * abs(roll)

        reward -= 0.35 * abs(roll_rate)
        reward -= 0.35 * abs(pitch_rate)
        ...
        # Extra soft penalties before the hard safety limits.
        if abs(roll) > 0.45:
            reward -= 12.0 * (
                abs(roll) - 0.45
            )
        ...
        if abs(forward_velocity) > 20.0:
            reward -= 0.35 * (
                abs(forward_velocity) - 20.0
            )

        if abs(lateral_velocity) > 15.0:
            reward -= 0.45 * (
                abs(lateral_velocity) - 15.0
            )
        ...
        # ==========================================================
            ...
            # Do not let the policy trade stability for excessive climb.
            ...
            # Small progress term so gaining altitude still matters.
            reward += 0.0025 * altitude
        ...
        # ==========================================================
            ...
            # Strongly suppress horizontal drift near the target altitude.
            reward -= 0.18 * abs(
                forward_velocity
            )
