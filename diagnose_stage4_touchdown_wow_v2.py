...
RESULT_DIR = Path(
    "results_stage4_contact_staircase_v2"
)
...
# =====================================================================
# STAGE-4 CONTACT-APPROACH COLLECTIVE STAIRCASE V2
# =====================================================================

# V1 showed that a constant touchdown VS target settles into new equilibria
# around 8.2-8.5 ft without reaching WOW.  This V2 does not retune the
# horizontal controller or the 9-ft capture.  It uses a monotone, conservative
# physical-collective staircase from the verified 9-ft hover until actual
# JSBSim WOW/compression is observed.
...
print(
    ...
    f"COLL={cap['physical_collective_cmd']:.6f} | "
...
rule("C — STEPWISE CONTACT SEARCH TO ACTUAL WOW")
result, level_rows, trace_rows, post_rows = (
    run_contact_staircase(
        curve_points,
        local_slope,
        detailed=True,
    )
...
rule("D — CONTACT STAIRCASE V2 CONCLUSION")
fc = result.get("first_contact")

if fc is not None:
    print(
        "FIRST WOW CONTACT | "
        f"level={fc['level_index']} "
        f"reduction={fc['collective_reduction']:.3f} | "
    ...
    print("FIRST WOW CONTACT | not detected")

print(
    f"SAFE={result['safe']} "
    f"TERM={result['termination']} | "
    f"CONTACT={result['contact_detected']} "
    f"contactVSok={result['contact_vs_ok']} | "
    f"bounce={result['bounce_detected']} "
    f"WOWpersistent={result['wow_persistent_s']:.2f}s | "
    f"finalWOW={result['final_wow']} "
    f"finalCOMP={result['final_max_compression_ft']:.4f}"
)

print(
    "READY FOR TOUCHDOWN HOLD / FINAL LANDING CONTROLLER CALIBRATION: "
    f"{result['ready_for_touchdown_hold_calibration']}"
)

save_rows_csv(
    RESULT_DIR / "staircase_levels.csv",
    level_rows,
)
save_rows_csv(
    RESULT_DIR / "staircase_traces.csv",
    trace_rows,
)
save_rows_csv(
    RESULT_DIR / "post_contact_observation.csv",
    post_rows,
)
...
summary = {
    ...
    "staircase_reductions": STAIR_REDUCTIONS,
    "result": result,
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'staircase_levels.csv'}")
print(f"  {RESULT_DIR / 'staircase_traces.csv'}")
print(f"  {RESULT_DIR / 'post_contact_observation.csv'}")
print(f"  {RESULT_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "This V2 only searches conservatively for the first actual JSBSim WOW "
    "contact from the already-validated 9-ft hover and records the contact "
    "AGL, vertical speed, compression, geometry, and bounce behavior."
)
