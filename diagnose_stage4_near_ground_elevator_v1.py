...
RESULT_DIR = Path(
    "results_stage4_near_ground_elevator_diagnostic_v1"
)
...
rule("A — REQUALIFY LOCKED STAGE1 -> STAGE2 -> STAGE3")
audit = build_stage4_handoff(detailed=True, require_full_hold=True)
audit_state = audit["state"]
print(
    "HANDOFF | "
    f"PASS={audit['handoff_pass']} | same_fdm={audit['same_fdm']} | "
    f"clock_reset={audit['clock_reset_on_attach']} | "
    f"FWD={audit_state['forward_ft']:.3f} V={audit_state['forward_speed_fps']:+.3f} | "
    f"X={audit_state['cross_track_ft']:+.3f} ALT={audit_state['altitude_ft']:.3f} | "
...
rule("B — REPRODUCE 30 FT -> 25 FT AND CHARACTERIZE BACKWARD-DRIFT TRIGGER")
trigger_run = build_25ft_then_trigger20()
s25 = trigger_run["state25"]
trig = trigger_run["trigger_state"]

print(
    f"25FT QUALIFIED | ALT={s25['altitude_ft']:.3f} VS={s25['vertical_speed_fps']:+.3f} | "
    f"FWD={s25['forward_ft']:.3f} err={s25['position_error_ft']:+.3f} "
    f"V={s25['forward_speed_fps']:+.3f} | "
    f"X={s25['cross_track_ft']:+.3f} LAT={s25['lateral_speed_fps']:+.3f}"
)
print(
    f"TRIGGER | t20={trigger_run['trigger_elapsed_s']:.2f}s | "
    f"ALT={trig['altitude_ft']:.3f} VS={trig['vertical_speed_fps']:+.3f} | "
    f"FWD={trig['forward_ft']:.3f} err={trig['position_error_ft']:+.3f} "
    f"V={trig['forward_speed_fps']:+.3f} | "
    f"X={trig['cross_track_ft']:+.3f} LAT={trig['lateral_speed_fps']:+.3f} | "
    f"ELEV={trig['physical_elevator_cmd']:+.6f}"
)
close_handoff(trigger_run)


rule("C — NEAR-GROUND POSITIVE PHYSICAL-ELEVATOR AUTHORITY PULSES")
pulse_rows = []
pulse_trace_rows = []

for case_idx, delta_phys in enumerate(PULSE_DELTAS_PHYS, start=1):
    run = build_25ft_then_trigger20()
    env2 = run["env2"]
    fdm = run["fdm"]
    lat0 = run["lat0"]
    lon0 = run["lon0"]
    ...
    for step in range(int(PULSE_SECONDS / dt)):
        ...
        state, used, base_elev, mapped_elev, diag_elev = raw_stage4_elevator_offset_cycle(
            env2,
            fdm,
            action,
            elevator_delta_phys=delta_phys,
            lat0=lat0,
            lon0=lon0,
            mission_heading=mission_heading,
        )
        t = (step + 1) * dt
        elevator_values.append(diag_elev)
        ...
        pulse_trace_rows.append({
            ...
            "time_s": float(t),
            "mapped_elevator_before_offset": float(mapped_elev),
            "diagnostic_physical_elevator": float(diag_elev),
            "action0": float(used[0]),
            "action1": float(used[1]),
            "action2": float(used[2]),
    ...
    d_v = final["forward_speed_fps"] - initial["forward_speed_fps"]
    d_fwd = final["forward_ft"] - initial["forward_ft"]
    d_vs = final["vertical_speed_fps"] - initial["vertical_speed_fps"]
    ...
    row = {
        ...
        "trigger_forward_ft": float(initial["forward_ft"]),
        "trigger_forward_speed_fps": float(initial["forward_speed_fps"]),
        "delta_forward_speed_fps": float(d_v),
        "delta_forward_ft": float(d_fwd),
        "delta_vertical_speed_fps": float(d_vs),
        "delta_altitude_ft": float(d_alt),
        "elevator_mean": float(np.mean(elevator_values)) if elevator_values else float("nan"),
        "max_abs_pitch_deg": float(max_abs_pitch),
        "max_abs_roll_deg": float(max_abs_roll),
        "max_abs_cross_track_ft": float(max_abs_cross),
    ...
    print(
        f"Case {case_idx:02d} dELEV={delta_phys:+.4f} | SAFE={safe} TERM={term} | "
        f"ELEV={row['elevator_mean']:+.6f} | "
        f"dVfwd@2s={d_v:+.4f} dFWD={d_fwd:+.3f} | "
        f"dVS={d_vs:+.3f} dALT={d_alt:+.3f} | "
        f"PITCHmax={max_abs_pitch:.2f} ROLLmax={max_abs_roll:.2f} | "
        f"Xmax={max_abs_cross:.2f} POSmax={max_abs_pos:.2f}"
    )
...
if len(safe_pulses) >= 3:
    x = np.asarray([r["physical_elevator_delta"] for r in safe_pulses], dtype=float)
    y = np.asarray([r["delta_forward_speed_fps"] for r in safe_pulses], dtype=float)
    ...
    print()
    print(
        f"FIT dVfwd(2s) ~= {fit_slope:+.6f} * physical_elevator_delta "
        f"{fit_intercept:+.6f}   (R^2={fit_r2:.4f})"
    )


rule("D — SUSTAINED NEAR-GROUND POSITIVE-ELEVATOR CHECKS")
sustained_rows = []
sustained_trace_rows = []

for case_idx, delta_phys in enumerate(SUSTAINED_DELTAS_PHYS, start=1):
    run = build_25ft_then_trigger20()
    env2 = run["env2"]
    fdm = run["fdm"]
    lat0 = run["lat0"]
    ...
    for step in range(int(SUSTAINED_SECONDS / dt)):
        ...
        t = (step + 1) * dt
        elev_values.append(diag_elev)

        if t >= SUSTAINED_SECONDS - 2.0:
            vfwd_tail.append(state["forward_speed_fps"])
    ...
    final = snapshot(fdm, lat0, lon0, mission_heading)
    vfwd_mean2 = float(np.mean(vfwd_tail)) if vfwd_tail else float("nan")
    row = {
        "case": int(case_idx),
        "physical_elevator_delta": float(delta_phys),
        "safe": bool(safe),
        ...
        "vfwd_mean_last2s": float(vfwd_mean2),
        ...
        "elevator_mean": float(np.mean(elev_values)) if elev_values else float("nan"),
    }
    sustained_rows.append(row)

    print(
        f"Case {case_idx:02d} dELEV={delta_phys:+.4f} | "
        f"SAFE={safe} CORRIDOR={corridor} TERM={term} | "
        f"FWD={final['forward_ft']:.3f} err={final['position_error_ft']:+.3f} "
        f"V={final['forward_speed_fps']:+.4f} Vmean2={vfwd_mean2:+.4f} | "
        f"ALT={final['altitude_ft']:.2f} VS={final['vertical_speed_fps']:+.3f} | "
        f"X={final['cross_track_ft']:+.2f} | "
        f"ELEVmean={row['elevator_mean']:+.6f}"
    )

    close_handoff(run)


rule("E — NEAR-GROUND ELEVATOR AUTHORITY CONCLUSION")
positive_authority_useful = bool(
    safe_pulses
    and any(
        r["physical_elevator_delta"] > 0.0
        and r["delta_forward_speed_fps"] > 0.03
...
safe_sustained = [
    ...
    if r["safe"]
    and r["corridor"]
    and abs(r["final_position_error_ft"]) <= PRESENTATION_MAX_POSITION_ERROR_FT
]
...
if safe_sustained:
    preferred = min(
        safe_sustained,
        key=lambda r: (
            abs(r["vfwd_mean_last2s"]),
            abs(r["final_position_error_ft"]),
            r["physical_elevator_delta"],
        ),
    )

ready = bool(
    positive_authority_useful
    and preferred is not None
    and np.isfinite(fit_r2)
    and fit_r2 >= 0.95
)

print(f"near-ground positive elevator authority measurable: {positive_authority_useful}")
print(f"pulse-response R^2: {fit_r2:.4f}")
print(f"safe sustained candidate inside 5-ft corridor: {preferred is not None}")
if preferred is not None:
    print(
        "preferred sustained evidence case: "
        f"dELEV={preferred['physical_elevator_delta']:+.4f}, "
        f"Vmean(last2s)={preferred['vfwd_mean_last2s']:+.4f}, "
        f"finalErr={preferred['final_position_error_ft']:+.3f}, "
        f"ELEVmean={preferred['elevator_mean']:+.6f}"
    )

print()
print(f"READY FOR NEAR-GROUND LONGITUDINAL MAPPING CALIBRATION: {ready}")

save_rows_csv(RESULT_DIR / "elevator_authority_pulse_sweep.csv", pulse_rows)
save_rows_csv(RESULT_DIR / "elevator_authority_pulse_traces.csv", pulse_trace_rows)
save_rows_csv(RESULT_DIR / "elevator_sustained_sweep.csv", sustained_rows)
save_rows_csv(RESULT_DIR / "elevator_sustained_traces.csv", sustained_trace_rows)

summary = {
    "stage4_training": "NONE",
    "diagnostic_only": True,
    "trigger_forward_ft": TRIGGER_FORWARD_FT,
    "pulse_seconds": PULSE_SECONDS,
    "pulse_deltas_physical_elevator": PULSE_DELTAS_PHYS,
    "sustained_seconds": SUSTAINED_SECONDS,
    "sustained_deltas_physical_elevator": SUSTAINED_DELTAS_PHYS,
    "fit_slope": float(fit_slope),
    "fit_intercept": float(fit_intercept),
    "fit_r2": float(fit_r2),
    "positive_authority_useful": bool(positive_authority_useful),
    "preferred_sustained_case": preferred,
    "ready_for_near_ground_longitudinal_mapping_calibration": bool(ready),
}
with open(RESULT_DIR / "diagnostic_summary.json", "w") as f:
    json.dump(summary, f, indent=2)

print("Saved:")
print(f"  {RESULT_DIR / 'elevator_authority_pulse_sweep.csv'}")
print(f"  {RESULT_DIR / 'elevator_authority_pulse_traces.csv'}")
print(f"  {RESULT_DIR / 'elevator_sustained_sweep.csv'}")
print(f"  {RESULT_DIR / 'elevator_sustained_traces.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. This test only measures whether the existing "
    "one-sided [-0.183,-0.180] elevator mapping needs safe positive-release "
    "authority in the near-ground regime."
)
