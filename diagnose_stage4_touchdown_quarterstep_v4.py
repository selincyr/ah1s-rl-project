...
RESULT_DIR = Path(
    "results_stage4_touchdown_quarterstep_v4"
)
...
rule("C — QUARTER-STEP CONTACT CONTINUATION")
result, anchor_rows, extension_rows, trace_rows, post_rows = (
    run_quarterstep_search(
        curve_points,
        local_slope,
    )
)
...
rule("D — QUARTER-STEP TOUCHDOWN V4 CONCLUSION")
...
if fc is not None:
    print(
        ...
        f"reduction={fc['collective_reduction']:.5f} | "
...
save_rows_csv(
    RESULT_DIR / "anchor_levels.csv",
    anchor_rows,
)
save_rows_csv(
    RESULT_DIR / "quarterstep_extension_levels.csv",
    extension_rows,
)
save_rows_csv(
    RESULT_DIR / "quarterstep_traces.csv",
    trace_rows,
)
save_rows_csv(
...
summary = {
    ...
    "extension_reductions": EXTENSION_REDUCTIONS,
    "result": result,
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'anchor_levels.csv'}")
print(f"  {RESULT_DIR / 'quarterstep_extension_levels.csv'}")
print(f"  {RESULT_DIR / 'quarterstep_traces.csv'}")
print(f"  {RESULT_DIR / 'post_contact_observation.csv'}")
print(f"  {RESULT_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "This V4 reproduces the confirmed reduction=0.0160 equilibrium and "
    "continues only in 0.00025 collective-reduction increments until actual "
    "WOW/compression is measured or a safety/equilibrium gate stops the search."
)
