...
rule("B — REPRODUCE V4 FIRST WOW CONTACT")
contact_check = build_first_contact_open(
    curve_points,
    local_slope,
)

fc = contact_check["first_contact_state"]
print(
    "FIRST CONTACT REPRODUCED | "
    f"reduction={contact_check['contact_reduction']:.5f} | "
    ...
    f"COLL={contact_check['contact_collective']:.6f} "
    f"COMP={contact_check['contact_compression_ft']:.4f}"
)
close_handoff(contact_check)


rule("C — POST-CONTACT LANDED-HOLD SWEEP")
rows = []
all_traces = []

for case_idx, offset in enumerate(
    LANDED_COLL_OFFSETS,
    start=1,
):
    print(
        f"Candidate {case_idx}/{len(LANDED_COLL_OFFSETS)} | "
        f"contact collective offset={offset:+.5f}"
    )

    result, trace = run_landed_hold_candidate(
    ...
    print(
        f"  PASS={result['pass']} SAFE={result['safe']} "
        f"TERM={result['termination']} | "
        ...
        f"COMP={result['final_compression_ft']:.4f} "
        f"COLL={result['target_landed_collective']:.6f}"
    )


rule("D — LANDED-HOLD SELECTION")
ranked = sorted(rows, key=landed_rank_key)

for rank, row in enumerate(ranked, start=1):
    print(
        f"#{rank} C{row['case']:02d} | "
        f"PASS={row['pass']} SAFE={row['safe']} | "
        f"offset={row['collective_offset']:+.5f} | "
        f"HOLD={row['landed_hold_s']:.2f}s "
        f"WOW={row['final_wow']} bounce={row['bounce']} | "
...
if passing:
    ...
    print(
        ...
        f"offset={selected['collective_offset']:+.5f}"
    )

    rule("E — FRESH DETAILED LANDED-HOLD REPEAT")
    fresh_result, fresh_trace = run_landed_hold_candidate(
        collective_offset=selected["collective_offset"],
        points=curve_points,
        local_slope=local_slope,
        detailed=True,
    )
    ...
    print(
        ...
        f"AGL={fresh_result['final_altitude_ft']:.3f} "
        ...
        f"COMP={fresh_result['final_compression_ft']:.4f} "
        f"COLL={fresh_result['target_landed_collective']:.6f}"
    )
else:
    print()
    print("NO PASSING LANDED-HOLD CANDIDATE.")


rule("F — STAGE-4 LANDED-HOLD V1 CONCLUSION")
ready_full_teacher = bool(
...
print(
    "READY TO BUILD FULL STAGE-4 TEACHER: "
    f"{ready_full_teacher}"
)

save_rows_csv(
    RESULT_DIR / "landed_hold_sweep.csv",
    rows,
)
save_rows_csv(
    RESULT_DIR / "landed_hold_traces.csv",
    all_traces,
)
if fresh_result is not None:
    save_rows_csv(
        RESULT_DIR / "best_landed_hold_trace.csv",
        fresh_trace,
    )
...
summary = {
    ...
    "contact_reduction": 0.01925,
    "landed_collective_offsets": LANDED_COLL_OFFSETS,
    "rows": rows,
    "fresh_result": fresh_result,
    "ready_to_build_full_stage4_teacher": bool(
        ready_full_teacher
    ),
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'landed_hold_sweep.csv'}")
print(f"  {RESULT_DIR / 'landed_hold_traces.csv'}")
if fresh_result is not None:
    print(f"  {RESULT_DIR / 'best_landed_hold_trace.csv'}")
print(f"  {RESULT_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "If READY=True, touchdown/contact physics and 5-s landed hold are both "
    "qualified, and the next step is assembling the complete Stage-4 teacher "
    "before distillation."
)
