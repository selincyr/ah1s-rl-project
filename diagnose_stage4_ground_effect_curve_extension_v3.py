...
RESULT_DIR = Path(
    "results_stage4_ground_effect_curve_extension_v3"
)
...
rule("B — REPRODUCE V2 CONFIRMED LOWER CURVE ANCHOR")
...
for idx, coll in enumerate(ANCHOR_LEVELS, start=1):
    result = hold_collective_to_equilibrium(
        env2,
        fdm,
        lat0,
        lon0,
    ...
    row = {
        "phase": "anchor",
        "level_index": int(idx),
        "physical_collective": float(coll),
        ...
        "equilibrium_altitude_range_ft": float(
            eq.get("altitude_range_ft", float("nan"))
        ),
    ...
    print(
        f"Anchor {idx:02d}/{len(ANCHOR_LEVELS)} "
        f"COLL={coll:+.6f} | "
    ...
    if not result["safe"] or not result["settled"]:
        anchor_ok = False
        break
...
if not anchor_ok:
    close_handoff(run)
    raise RuntimeError(
        "Could not reproduce the previously confirmed V2 lower-curve anchor."
    )


rule("C — TARGETED LOWER-CURVE EXTENSION")
overall_safe = True
curve_valid = True
termination = "extension_complete"

for idx, coll in enumerate(EXTENSION_LEVELS, start=1):
    result = hold_collective_to_equilibrium(
        env2,
        fdm,
        lat0,
    ...
    row = {
        ...
        "equilibrium_vs_abs_peak_fps": float(
            eq.get("vs_abs_peak_fps", float("nan"))
        ),
        "equilibrium_altitude_range_ft": float(
            eq.get("altitude_range_ft", float("nan"))
        ),
        ...
        "final_forward_ft": float(s["forward_ft"]),
        "final_position_error_ft": float(
            s["position_error_ft"]
        ),
        "final_forward_speed_fps": float(
        ...
        "max_abs_position_error_ft": float(
            result["max_abs_position_error_ft"]
        ),
        "max_abs_cross_track_ft": float(
            result["max_abs_cross_track_ft"]
        ),
    }
    extension_rows.append(row)

    print(
        f"Level {idx:02d}/{len(EXTENSION_LEVELS)} "
        f"COLL={coll:+.6f} | "
        f"SETTLED={result['settled']} SAFE={result['safe']} "
        ...
        f"FWDerr={row['final_position_error_ft']:+.2f} "
        f"X={row['final_cross_track_ft']:+.2f} | "
        f"minALT={row['min_altitude_ft']:.2f} "
    ...
    # Once a genuinely confirmed equilibrium is close enough to the 9-ft
    # capture region, do not continue lower in this pre-touchdown diagnostic.
    if (
        np.isfinite(row["equilibrium_altitude_mean_ft"])
        and row["equilibrium_altitude_mean_ft"] <= 10.25
    ):
        termination = "capture_range_reached"
        break


rule("D — LOWER-CURVE EXTENSION V3 CONCLUSION")
confirmed_extension = [
    r for r in extension_rows
    if r["safe"]
    and r["settled"]
...
combined_confirmed = [
    r for r in anchor_rows + extension_rows
    if r["safe"]
    and r["settled"]
    and np.isfinite(r["equilibrium_altitude_mean_ft"])
]
...
monotonic = False
if len(combined_confirmed) >= 3:
    alts = [
        r["equilibrium_altitude_mean_ft"]
        for r in combined_confirmed
    ]
    colls = [
...
lowest_eq_alt = (
    combined_confirmed[-1]["equilibrium_altitude_mean_ft"]
    if combined_confirmed
    else float("nan")
)
lowest_eq_coll = (
    combined_confirmed[-1]["physical_collective"]
    if combined_confirmed
    else float("nan")
)

ready_capture_controller = bool(
    curve_valid
    and overall_safe
    and monotonic
    and len(confirmed_extension) >= 1
    and np.isfinite(lowest_eq_alt)
    and lowest_eq_alt <= 10.75
)

ready_direct_9ft_schedule = bool(
    ready_capture_controller
    and lowest_eq_alt <= 10.25
)

print(f"curve valid: {curve_valid}")
print(
    f"confirmed extension points: "
    f"{len(confirmed_extension)}"
)
print(
    f"combined monotonic relation: {monotonic}"
)
print(
    f"lowest confirmed equilibrium: "
    f"ALT={lowest_eq_alt:.3f} ft, "
    f"COLL={lowest_eq_coll:.6f}"
)
print(f"termination: {termination}")
print()
print(
    "READY FOR 9-FT CAPTURE CONTROLLER CALIBRATION: "
    f"{ready_capture_controller}"
)
print(
    "READY FOR DIRECT INTERPOLATED 9-FT FEEDFORWARD SCHEDULE: "
    f"{ready_direct_9ft_schedule}"
)

save_rows_csv(
    RESULT_DIR / "anchor_reproduction.csv",
    anchor_rows,
)
save_rows_csv(
    RESULT_DIR / "lower_curve_extension.csv",
    extension_rows,
)
save_rows_csv(
    RESULT_DIR / "lower_curve_extension_traces.csv",
    trace_rows,
)
...
summary = {
    ...
    "termination": str(termination),
    "confirmed_extension_points": confirmed_extension,
    "combined_monotonic": bool(monotonic),
    "lowest_confirmed_equilibrium_altitude_ft": float(
        lowest_eq_alt
    ),
    "lowest_confirmed_equilibrium_collective": float(
        lowest_eq_coll
    ),
    "ready_for_9ft_capture_controller_calibration": bool(
        ready_capture_controller
    ),
    "ready_for_direct_interpolated_9ft_feedforward_schedule": bool(
        ready_direct_9ft_schedule
    ),
}
...
print("Saved:")
print(f"  {RESULT_DIR / 'anchor_reproduction.csv'}")
print(f"  {RESULT_DIR / 'lower_curve_extension.csv'}")
print(f"  {RESULT_DIR / 'lower_curve_extension_traces.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "This script only extends the already-confirmed lower end of the "
    "ground-effect collective curve toward the 9-ft capture region."
)
