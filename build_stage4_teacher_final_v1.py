...
RESULT_DIR = Path(
    "results_stage4_teacher_final_v1"
)
...
FULL_RESULT_DIR = Path("results_stage4_teacher_final_v1")
...
rule("A — LOCKED FULL STAGE-4 TEACHER FINAL VALIDATION")

result, trace = run_full_stage4_teacher(
    landed_target_collective=LOCKED_LANDED_TARGET_COLLECTIVE,
    detailed=True,
)


rule("B — LOCKED STAGE-4 TEACHER FINAL CONCLUSION")
...
print(
    "FINAL LANDED | "
    f"AGL={result['final_altitude_ft']:.3f} "
    f"VS={result['final_vertical_speed_fps']:+.3f} | "
    f"FWD={result['final_forward_ft']:.3f} "
    f"err={result['final_position_error_ft']:+.3f} "
    f"V={result['final_forward_speed_fps']:+.3f} | "
    f"X={result['final_cross_track_ft']:+.3f} "
    f"LAT={result['final_lateral_speed_fps']:+.3f} | "
    f"WOW={result['final_wow']} "
...
print(
    "READY FOR STAGE-4 TEACHER ROLLOUT COLLECTION: "
    f"{ready_for_collection}"
)

final_summary = dict(result)
final_summary.update({
    "stage4_training": "NONE",
    "teacher_locked": bool(ready_for_collection),
    "locked_landed_target_collective": float(
        LOCKED_LANDED_TARGET_COLLECTIVE
    ),
    "ready_for_stage4_teacher_rollout_collection": bool(
        ready_for_collection
    ),
})

save_rows_csv(
    FULL_RESULT_DIR / "final_trace.csv",
    trace,
)
...
print("Saved:")
print(f"  {FULL_RESULT_DIR / 'final_trace.csv'}")
print(f"  {FULL_RESULT_DIR / 'final_summary.json'}")
print()
print(
    "Do NOT train Stage 4 in this script. "
    "This file is the deterministic locked teacher validation. "
    "If READY=True, freeze these teacher parameters and use this behavior "
    "as the source for rollout collection / policy distillation."
)
