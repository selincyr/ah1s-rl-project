...
RESULT_DIR = Path(
    "results_stage4_low_alt_physical_collective_sustained_v3"
)
...
rule("B — NATIVE A0=-1 EQUILIBRIUM REPRODUCTION")
...
rule("C — FOCUSED SUSTAINED PHYSICAL COLLECTIVE SWEEP")
rows = []
trace_rows = []

for case_idx, delta_phys in enumerate(V3_SUSTAINED_DELTAS, start=1):
    ...
    for step in range(int(V3_SUSTAINED_SECONDS / dt)):
        state_before = snapshot(fdm, lat0, lon0, mission_heading)
        ...
        reason = v3_safety_reason(state)
        if reason:
            if "corridor_exit" in reason:
                corridor = False
            else:
                safe = False
            term = reason
...
rule("D — FOCUSED SUSTAINED V3 CONCLUSION")
usable = [
    r for r in rows
    if r["safe"]
    and r["corridor"]
    and np.isfinite(r["vs_mean_last2s"])
    and r["vs_mean_last2s"] <= -0.06
    and r["min_altitude_ft"] >= V3_MIN_AGL_FT
]
...
ready_mapping = bool(preferred is not None)

print(f"usable sustained physical collective candidate found: {ready_mapping}")
...
print(
    ...
    f"CALIBRATION: {ready_mapping}"
)

save_rows_csv(
    RESULT_DIR / "focused_sustained_sweep.csv",
    rows,
)
save_rows_csv(
    RESULT_DIR / "focused_sustained_traces.csv",
    trace_rows,
)
...
summary = {
    ...
    "sustained_deltas": V3_SUSTAINED_DELTAS,
    "sustained_seconds": V3_SUSTAINED_SECONDS,
...
print("Saved:")
print(f"  {RESULT_DIR / 'focused_sustained_sweep.csv'}")
print(f"  {RESULT_DIR / 'focused_sustained_traces.csv'}")
print(f"  {RESULT_DIR / 'diagnostic_summary.json'}")
print()
print(
    "Do NOT train Stage 4 here. "
    "This V3 only closes the missing sustained-authority band identified by V2."
)
